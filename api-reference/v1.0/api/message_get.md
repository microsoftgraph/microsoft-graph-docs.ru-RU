...
| Prefer: outlook.body-content-type | string (строка) | Формат возвращаемых свойств **body** и **uniqueBody**. Возможные значения: "text" или "html". Заголовок `Preference-Applied` возвращается как подтверждение, если заголовок `Prefer` указан. Если заголовок не указан, свойства **body** и **uniqueBody** возвращаются в формате HTML. Необязательный параметр. |
...
В следующем примере используются параметр `$select` для получения заголовков интернет-сообщений. 
<!-- {
  "blockType": "request",
...
Ниже приведен пример отклика. Примечание. Представленные здесь заголовки ответов объекта могут быть усечены для краткости. При фактическом вызове будут возвращены все заголовки.
<!-- {
  "blockType": "response",
  "truncated": true,
  "@odata.type": "microsoft.graph.message"
