...
>**Примечание:** Комнаты, помеченные как "Скрытые из GAL", невозможно получить через эту конечную точку.
...
По сравнению с [функциями findRooms](/graph/api/user-findrooms) и [findRoomLists](/graph/api/user-findroomlists) эта операция возвращает более широкие полезные данные для комнат и списков помещений. [Ознакомьтесь с подробными](../resources/place.md#using-the-places-api) сведениями о том, как они сравниваются.
...
Используется `$top` для настройки размера страницы. Размер страницы по умолчанию — 100.
