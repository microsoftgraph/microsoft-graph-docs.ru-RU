...
Этот метод поддерживает необязательные параметры запросов OData для настройки ответа.
...
- Имеется ограниченная поддержка параметров `$filter` и `$orderby`:
  * Единственное поддерживаемое выражение `$filter` предназначено для отслеживания изменений, касающихся одного или двух определенных пользователей: `$filter=id+eq+{value}` или `$filter=id+eq+{value1}+or+id+eq+{value2}`. `$filter=id+eq+{value1}+or+id+eq+{value2}` 
