...
Этот критерий для  свойства владельца позволяет пользователю, заявив, что он может делать обновления через другие не собственные приложения, такие как Microsoft Graph Explorer. При использовании Graph Explorer для обновления ресурса **schemaExtension** включаем свойство владельца в  тело запроса PATCH. Дополнительные сведения см. в разделе [Расширения](/graph/known-issues#extensions) в [разделе Известные проблемы с Microsoft Graph](/graph/known-issues).
...
При успешном выполнении этот метод возвращает код отклика `204 No Content`. Попытка выполнить этот запрос из приложения, которым вы не владеете (и без настройки свойства владельца к приложению приложения, которым  вы владеете) `403 Forbidden` возвращает код ответа.
