...
Как и в предыдущем примере, в этом примере используется `entityUrl` для .`topic` Однако в этом случае он связывается с сообщением в чате. В сообщении может содержаться карточка с кнопкой утверждения.
