...
> **Примечание:** В настоящее время для вызова этого API требуется разрешение `Directory.Read.All` или выше. При использовании разрешения Group.Read.All отдельно или в сочетании с разрешением `User.` возвращается код ошибки. Мы знаем об этой проблеме.
