...
Этот метод поддерживает параметры `$select` `$filter` запроса oData и OData для настройки ответа. Например, для поиска пакетов доступа с определенным именем включайте фильтр, например `$filter=contains(tolower(displayName),'team')` в запрос. Общие сведения см. в статье [Параметры запроса OData](/graph/query-parameters).
