...
Создание [звонка](../resources/call.md) позволяет боту создать новый исходящий одноранговый или групповой звонок или присоединиться к существующему собранию. Необходимо зарегистрировать [вызывающий бот](/microsoftteams/platform/concepts/calls-and-meetings/registering-calling-bot) и просмотреть список необходимых разрешений, как упоминалось ниже.
...
> **Примечание:** Для вызова с размещенным в приложении носителем требуется разрешение Calls.AccessMedia.All в дополнение к одному из разрешений, перечисленных в таблице выше.
...
##### <a name="request"></a>Запрос
В следующем примере показан запрос, который выполняет одноранговый вызов между ботом и указанным пользователем. В этом примере носитель размещается локально приложением. Значения маркера авторизации, URL-адреса обратного вызова, идентификатора приложения, имени приложения, идентификатора пользователя, имени пользователя и клиента должны быть заменены фактическими значениями для работы примера.
...
Эти сведения можно получить из [API get Online Meetings](../api/onlinemeeting-get.md) (только собрания на основе VTC).

Значения маркера авторизации, URL-адреса обратного вызова, идентификатора приложения, имени приложения, идентификатора пользователя, имени пользователя и клиента должны быть заменены вместе со сведениями, полученными из  [API get Online Meetings](../api/onlinemeeting-get.md) (только для собраний на основе VTC) фактическими значениями, чтобы пример совмещал работу.
...
### <a name="example-6-join-scheduled-meeting-with-application-hosted-media"></a>Пример 6. Присоединение к запланированному собранию с размещенным в приложении носителем
Обновите конфигурацию мультимедиа с [помощью AppHostedMediaConfig](../resources/apphostedmediaconfig.md) , как показано ниже.
...
### <a name="example-7-create-peer-to-peer-pstn-call-with-service-hosted-media"></a>Пример 7. Создание однорангового вызова ТСОП с размещенным в службе носителем
...
### <a name="example-8-create-peer-to-peer-pstn-call-with-application-hosted-media"></a>Пример 8. Создание однорангового вызова ТСОП с размещенным в приложении носителем
