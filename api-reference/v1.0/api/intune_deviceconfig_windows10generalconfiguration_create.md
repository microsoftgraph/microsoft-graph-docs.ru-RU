...
|diagnosticsDataSubmissionMode|Строка|Возвращает или задает значение, позволяющее устройству отправлять данные диагностики и телеметрии использования, такие как Watson. Возможные значения: `userDefined`, `none`, `basic`, `enhanced`, `full`.|
...
|personalizationDesktopImageUrl|Строка|Начинающийся с http или https URL-адрес изображения JPG, JPEG или PNG, которое необходимо скачать и разместить на рабочем столе, или адрес локального изображения в файловой системе, которое необходимо разместить на рабочем столе.|
|personalizationLockScreenImageUrl|Строка|Начинающийся с http или https URL-адрес изображения JPG, JPEG или PNG, которое необходимо скачать и разместить на экране блокировки, или адрес локального изображения в файловой системе, которое необходимо разместить на экране блокировки.|
...
|bluetoothBlockPrePairing|Логический|Указывает, следует ли заблокировать автоматическое сопряжение отдельных периферийных устройств Bluetooth с главным устройством.|
...
|edgeCookiePolicy|Строка|Указывает, какие файлы cookie следует блокировать в браузере Edge. Возможные значения: `userDefined`, `allow`, `blockThirdParty`, `blockAll`.|
...
|edgeBlockPasswordManager|Логический|Указывает, следует ли заблокировать менеджер паролей.|
...
|edgeSyncFavoritesWithInternetExplorer|Логический|Указывает, следует ли включить синхронизацию избранного между Internet Explorer и Microsoft Edge. Браузеры обмениваются данными о добавлении, удалении и изменении избранных элементов и их порядка.|
...
|defenderSystemScanSchedule|Строка|День сканирования системы Защитником. Возможные значения: `userDefined`, `everyday`, `sunday`, `monday`, `tuesday`, `wednesday`, `thursday`, `friday`, `saturday`.|
|defenderFilesAndFoldersToExclude|Коллекция строк|Файлы и папки, которые необходимо исключить из сканирования и защиты в режиме реального времени.|
|defenderFileExtensionsToExclude|Коллекция строк|Расширения файлов, которые необходимо исключить из сканирования и защиты в режиме реального времени.|
|defenderScanMaxCpu|Int32|Максимальный процент загрузки ЦП во время сканирования. Допустимые значения: от 0 до 100|
|defenderMonitorFileActivity|Строка|Значение для отслеживания действий с файлами. Возможные значения: `userDefined`, `disable`, `monitorAllFiles`, `monitorIncomingFilesOnly`, `monitorOutgoingFilesOnly`.|
|defenderProcessesToExclude|Коллекция строк|Процессы, которые необходимо исключить из сканирования и защиты в режиме реального времени.|
|defenderPromptForSampleSubmission|Строка|Позволяет указать, нужно ли спрашивать пользователя перед отправкой образцов. Возможные значения: `userDefined`, `alwaysPrompt`, `promptBeforeSendingPersonalData`, `neverSendData`, `sendAllDataWithoutPrompting`.|
...
|defenderScanArchiveFiles|Логический|Указывает, следует ли сканировать архивные файлы.|
|defenderScanDownloads|Логический|Указывает, следует ли сканировать загрузки.|
|defenderScanNetworkFiles|Логический|Указывает, следует ли сканировать файлы, открытые из сетевой папки.|
|defenderScanIncomingMail|Логический|Указывает, следует ли сканировать входящую почту.|
|defenderScanMappedNetworkDrivesDuringFullScan|Логический|Указывает, следует ли сканировать подключенные сетевые диски во время полного сканирования.|
|defenderScanRemovableDrivesDuringFullScan|Логический|Указывает, следует ли сканировать съемные диски во время полного сканирования.|
|defenderScanScriptsLoadedInInternetExplorer|Логический|Указывает, следует ли сканировать сценарии, загружаемые в браузере Internet Explorer.|
...
|defenderScanType|Строка|Тип сканирования системы Защитником. Возможные значения: `userDefined`, `disabled`, `quick`, `full`.|
...
|defenderCloudBlockLevel|Строка|Определяет уровень облачной защиты. Возможные значения: `notConfigured`, `high`, `highPlus`, `zeroTolerance`.|
|lockScreenAllowTimeoutConfiguration|Логический|Указывает, следует ли показывать настройку, позволяющую определить время до отключения экрана на экране блокировки мобильных устройств с Windows 10. Если для этой политики установлено значение Allow, значение, заданное свойством lockScreenTimeoutInSeconds, игнорируется.|
...
|lockScreenBlockCortana|Логический|Указывает, может ли пользователь взаимодействовать с Кортаной при помощи голоса при заблокированной системе.|
...
|passwordRequired|Логический|Указывает, обязательно ли использование пароля.|
...
|passwordRequiredType|Строка|Требуемый тип пароля. Возможные значения: `deviceDefault`, `alphanumeric`, `numeric`.|
...
|privacyAdvertisingId|Строка|Указывает, можно ли использовать идентификатор рекламы. Добавлено в Windows 10 версии 1607. Возможные значения: `notConfigured`, `blocked`, `allowed`.|
|privacyAutoAcceptPairingAndConsentPrompts|Логический|Указывает, следует ли запретить автоматическое принятие примечаний о сопряжении и конфиденциальности при запуске приложений.|
...
|startMenuAppListVisibility|Строка|Эта политика позволяет свернуть список приложений, полностью удалить этот список или отключить соответствующий переключатель в приложении "Параметры". Возможные значения: `userDefined`, `collapse`, `remove`, `disableSettingsApp`.|
...
|startMenuHideRecentJumpLists|Логический|Эта политика позволяет скрыть списки последних переходов в меню "Пуск" и на панели задач и отключить соответствующий переключатель в приложении "Параметры".|
...
|startMenuHideSwitchAccount|Логический|Эта политика позволяет скрыть параметр для смены учетной записи на плитке пользователя в меню "Пуск".|
...
|startMenuMode|Строка|Позволяет администраторам определить вид меню "Пуск". Возможные значения: `userDefined`, `fullScreen`, `nonFullScreen`.|
|startMenuPinnedFolderDocuments|Строка|Определяет, виден ли ярлык папки "Документы" в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderDownloads|Строка|Определяет, виден ли ярлык папки "Загрузки" в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderFileExplorer|Строка|Определяет, виден ли ярлык Проводника в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderHomeGroup|Строка|Определяет, виден ли ярлык папки "Домашняя группа" в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderMusic|Строка|Определяет, виден ли ярлык папки "Музыка" в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderNetwork|Строка|Определяет, виден ли ярлык папки "Сеть" в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderPersonalFolder|Строка|Определяет, виден ли ярлык личной папки в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderPictures|Строка|Определяет, виден ли ярлык папки "Изображения" в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderSettings|Строка|Определяет, виден ли ярлык папки "Параметры" в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
|startMenuPinnedFolderVideos|Строка|Определяет, виден ли ярлык папки "Видео" в меню "Пуск". Возможные значения: `notConfigured`, `hide`, `show`.|
...
|windowsSpotlightBlockConsumerSpecificFeatures|Логический|Позволяет ИТ-администраторам заблокировать рекомендации в меню "Пуск", уведомления, связанные с членством в группе, установку приложений после запуска при первом включении компьютера и плитки перенаправления.|
...
|windowsSpotlightConfigureOnLockScreen|Строка|Задает тип экрана "Интересное". Возможные значения: `notConfigured`, `disabled`, `enabled`.|
...
|safeSearchFilter|Строка|Определяет необходимый уровень фильтрации для безопасного поиска. Возможные значения: `userDefined`, `strict`, `moderate`.|
...
|storageRequireMobileDeviceEncryption|Логический|Указывает, обязательно ли шифрование данных на мобильном устройстве.|
...
|wirelessDisplayBlockProjectionToThisDevice|Логический|Указывает, могут ли другие устройства находить этот ПК для проецирования контента.|
...
|wirelessDisplayRequirePinForPairing|Логический|Указывает, обязательно ли использовать ПИН-код для сопряжения с новыми устройствами.|
|windowsStoreBlocked|Логический|Указывает, следует ли запретить использовать Microsoft Store.|
|appsAllowTrustedAppsSideloading|Строка|Указывает, можно ли загружать неопубликованные приложения из пакетов AppX с доверенным сертификатом. Возможные значения: `notConfigured`, `blocked`, `allowed`.|
...
|developerUnlockSetting|Строка|Указывает, следует ли разрешить разблокировку для разработки. Возможные значения: `notConfigured`, `blocked`, `allowed`.|
...
|windowsStoreEnablePrivateStoreOnly|Логический|Указывает, следует ли включить частный магазин.|
