...
- Имеется ограниченная поддержка параметра `$filter`:
  * Единственное поддерживаемые выражения для отслеживания изменений для `$filter` определенных ресурсов по их ид:  `$filter=id+eq+{value}` или `$filter=id+eq+{value1}+or+id+eq+{value2}` . Максимальное количество url-адресов, которые можно указать, ограничено.
