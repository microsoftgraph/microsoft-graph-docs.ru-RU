...
|jobTitle|Строка|Должность пользователя. Поддерживает параметр $filter.|
...
|userType|Строка|Значение строки, с помощью которого можно классифицировать типы пользователей в справочнике, например, "Участник" и "Гость". Поддерживает параметр $filter.          |
...
|calendar|[Календарь](calendar.md)|Основной календарь пользователя. Только для чтения.|
...
|inferenceClassification | [inferenceClassification](inferenceClassification.md) | Классификация релевантности сообщений пользователя, основанная на явных указаниях, переопределяющих заданную релевантность или важность. |
...
|ownedObjects|Коллекция [directoryObject](directoryobject.md)|Объекты справочника, принадлежащие пользователю. Только для чтения. Допускается значение null.|
