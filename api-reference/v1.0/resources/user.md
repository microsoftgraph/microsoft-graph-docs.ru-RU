...
|assignedLicenses|Коллекция [assignedLicense](assignedlicense.md)|Лицензии, назначенные пользователю. Возвращается только с помощью оператора `$select`. Значение null не допускается. Поддерживает `$filter`.           |
...
|city|String|Город, в котором находится пользователь. Максимальная длина: 128 символов. Поддерживает `$filter`.|
|companyName | String | Название организации, с которой связан пользователь. Это свойство может быть полезно для описания компании внешнего пользователя. Длина имени компании не должна превышать 64 символов.<br><br>Возвращается только с помощью оператора `$select`.|
...
|country|String|Страна или регион, в котором находится пользователь, например "США" или "Соединенное Королевство". Максимальная длина: 128 символов. Поддерживает `$filter`.|
|createdDateTime | DateTimeOffset |Дата создания объекта пользователя. Поддерживает `$filter` с операторами `eq`, `lt`и `ge`.|
...
|department|String|Название отдела, в котором работает пользователь. Максимальная длина: 64 символа. Поддерживает `$filter`.|
...
|givenName|String|Простое имя пользователя. Возвращается по умолчанию. Максимальная длина: 64 символа. Поддерживает `$filter`.|
...
|jobTitle;|String|Должность пользователя. Максимальная длина: 128 символов. Возвращается по умолчанию. Поддерживает `$filter`.|
...
|officeLocation|String|Расположение офиса, в котором находится рабочее место пользователя. Возвращается по умолчанию.|
...
|preferredLanguage|String|Язык, предпочтительный для пользователя. Он должен быть представлен в формате ISO 639-1, например "ru-RU". Возвращается по умолчанию.|
...
|state|String|Область, республика, край или округ в адресе пользователя. Максимальная длина: 128 символов. Поддерживает `$filter`.|
|streetAddress|String|Почтовый адрес места работы пользователя. Максимальная длина: 1024 символа.|
|surname|String|Фамилия пользователя. Возвращается по умолчанию. Максимальная длина: 64 символа. Поддерживает `$filter`.|
