...
|assignedLicenses|Коллекция [assignedLicense](assignedlicense.md)|Лицензии, назначенные пользователю, включая наследуемые (на основе групп) лицензии. Возвращается только с помощью оператора `$select`. Значение null не допускается. Поддерживает `$filter`.           |
...
|country|Строка|Страна или регион, в котором находится пользователь, например "США" или "Соединенное Королевство". Максимальная длина: 128 символов. Поддерживает `$filter`.|
|createdDateTime | DateTimeOffset |Дата создания объекта пользователя. Поддерживает `$filter` с операторами `eq`, `ne`, `le` и `ge`.|
...
|jobTitle;|String|Должность пользователя. Максимальная длина: 128 символов. Возвращается по умолчанию. Поддерживает `$filter` (операторы `eq` и `startsWith`).|
