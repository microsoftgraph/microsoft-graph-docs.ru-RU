...
|assignedLicenses|Коллекция [assignedLicense](assignedlicense.md)|Назначенные пользователю лицензии, в том числе наследуемые (на основе групп).  Значение null не допускается. Возвращается только с помощью оператора `$select`. Поддерживает `$filter` (`eq`, `not` и подсчитывает пустые коллекции).           |
...
|onPremisesSecurityIdentifier|String|Содержит локальный идентификатор безопасности (SID) локальной группы, синхронизированной с облаком. Только для чтения.<br><br>Возвращается только с помощью оператора `$select`.  Поддерживает `$filter` (`eq` в том числе `null` значений). |
|onPremisesSyncEnabled|Логический| `true`, если этот пользовательский объект сейчас синхронизируется с локальной Active Directory (AD),иначе пользователь не синхронизируется и может управляться в Azure Active Directory (Azure AD). Только для чтения. <br><br>Возвращается только с помощью оператора `$select`. Поддерживает `$filter` (`eq`, `ne`, `not`, `in` и `eq` для значений `null`).|
|onPremisesUserPrincipalName|String| Содержит локальное `userPrincipalName`, синхронизированное из локального каталога. Свойство заполняется только для клиентов, синхронизирующих свой локальный каталог с Azure Active Directory через Azure AD Connect. Только для чтения. <br><br>Возвращается только с помощью оператора `$select`. Поддерживает `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`).|
|otherMails|Коллекция строк| Список дополнительных адресов электронной почты для пользователя. Например: `["bob@contoso.com", "Robert@fabrikam.com"]`. <br>ПРИМЕЧАНИЕ. Это свойство не может содержать диакритические знаки. <br><br>Возвращается только с помощью оператора `$select`. Поддерживает `$filter` (`eq`, `not`, `ge`, `le`, `in`, `startsWith`, `endsWith`, и подсчитывает пустые коллекции).|
...
|extensions|Коллекция [extension](extension.md)|Коллекция открытых расширений, определенных для пользователя. Только для чтения. Поддерживает `$expand`. Допускается значение null.|
