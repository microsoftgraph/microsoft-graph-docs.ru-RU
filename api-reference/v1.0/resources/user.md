...
|[checkMemberGroups](../api/user-checkmembergroups.md)|Коллекция объектов string|Проверка членства в списке групп. Это транзитивная проверка.|
|[checkMemberObjects](../api/user-checkmemberobjects.md)|Коллекция объектов string|Проверка участия в списке группы, роли каталога или объектах административных единиц. Это транзитивная функция.|
...
| [getMemberGroups](../api/user-getmembergroups.md) |Коллекция объектов string | Возвращает все группы, в которых состоит пользователь. Это транзитивная проверка. |
...
|pastProjects|Коллекция объектов string|Список предыдущих проектов пользователя.|
...
|proxyAddresses|Коллекция String|Пример: `["SMTP: bob@contoso.com", "smtp: bob@sales.contoso.com"]`. В выражениях фильтра для свойств с несколькими значениями требуется оператор **any**. Только для чтения. Значение null не допускается. Поддерживает параметр $filter.|
