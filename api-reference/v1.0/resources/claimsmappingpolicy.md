...
description: Представляет политики сопоставления утверждений для протоколов WS-Fed, SAML, OAuth 2.0 и OpenID Подключение для маркеров, выданных конкретному приложению.
...
|Определение|Коллекция String| Коллекция строк, содержащая строку JSON, которая определяет правила и параметры для этой политики. [Дополнительные сведения о схеме](#properties-of-a-claims-mapping-policy-definition) JSON для этого свойства см. в разделе "Свойства определения политики сопоставления утверждений". Обязательный.|
