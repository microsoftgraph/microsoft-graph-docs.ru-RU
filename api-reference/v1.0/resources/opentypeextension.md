...
Один из способов обеспечить уникальность имен расширений — использовать метод обратной службы доменных имен (DNS), который зависит от _принадлежащего вам домена_, например `com.contoso.ContactInfo`. **Не используйте** домены Майкрософт (`com.microsoft` или `com.onmicrosoft`) в имени расширения.
