...
| latestSupportedTlsVersion | Строка | Указывает последнюю версию TLS, поддерживаемую конечной точкой уведомлений. Позволяет подписчикам использовать устаревшую версию TLS в течение ограниченного периода. Возможные значения: **v1_0**, **v1_1**, **v1_2**, **v1_3**. Необязательно, значение по умолчанию — v1_2. Если конечная точка клиента поддерживает TLS 1.2 или более поздние версии, это свойство не требуется. Если конечная точка клиента поддерживает только TLS 1.0 или 1.1, для этого свойства следует задать соответствующую версию. В противном случае операция завершится сбоем. |
