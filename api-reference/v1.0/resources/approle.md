...
Представляет роль приложения, которая может быть запрошена клиентским приложением, вызывающей другое приложение, или которая может использоваться для назначения приложения пользователям или группам в указанной роли приложения. Свойство **appRoles** <!-- of the [servicePrincipal](serviceprincipal.md) entity and --> объекта [Application](application.md) — коллекция **аппроле**.
...
|основания|String| Только для чтения. Указывает, определена ли роль приложения для объекта Application. <!-- or on the ServicePrincipal object -->. _Не_ следует включать в запросы POST или patch. |
