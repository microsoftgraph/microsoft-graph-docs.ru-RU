...
Можно получить доступ к набору объектов Excel (например, Table, Range или Chart) с помощью стандартных интерфейсов API REST, чтобы выполнять в книге операции создания, чтения, обновления и удаления (CRUD). Например, `GET https://graph.microsoft.com/v1.0/me/drive/items/{id}/workbook/worksheets`  
...
1. Постоянный сеанс — все изменения, вносимые в книгу, сохраняются. Это наиболее эффективный и производительный режим работы. 
