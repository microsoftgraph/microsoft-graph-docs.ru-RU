...
Коллекция предложений встречи (если они есть) или причина их отсутствия.
...
| locationsUnavailable | Свойство **isRequired** параметра [locationConstraint](locationconstraint.md) указано как обязательное, но для рассчитанных периодов времени нет доступного местоположения. |
...
|:---------------|:--------|:----------|
|emptySuggestionsReason|String|Причина для того, чтобы не возвращать какие-либо предложения встреч. Допустимые значения: `attendeesUnavailable`, `attendeesUnavailableOrUnknown`, `locationsUnavailable`, `organizerUnavailable` или `unknown`. Это свойство — пустая строка, если свойство **meetingTimeSuggestions** включает какое-либо предложение встречи.|
|meetingTimeSuggestions|Коллекция [meetingTimeSuggestion](meetingTimeSuggestion.md)|Массив предложений.|
