...
Ресурс **DriveItem** представляет элемент на диске. Все объекты файловой системы верхнего уровня в OneDrive возвращаются как ресурсы элемента. 

К экземплярам **DriveItem** можно получить доступ по их свойству **id** с помощью синтаксиса `/items/{item-id}` или по их пути в файловой системе с помощью синтаксиса `/drive/root:/path/to/file`.

У элементов есть аспекты, смоделированные как свойства, которые предоставляют данные об идентификаторах и возможностях элемента. У папок есть [**аспект folder**](folder.md), а у файлов — [**аспект file**](file.md). У изображений, кроме аспекта file, есть [**аспект image**](image.md). У изображений, полученных с помощью камеры (фотографий), есть [**аспект photo**](photo.md), который определяет элемент как фотографию со свойствами времени и устройства съемки.

Элементы с аспектом **folder** выполняют роль контейнеров элементов, поэтому у них есть ссылка `children`, указывающая на коллекцию элементов в папке.
...
**Примечание.** Значение @microsoft.graph.downloadUrl — краткосрочный URL-адрес, который не сохраняется в кэше. URL-адрес будет доступным только на короткое время, прежде чем станет недействительным.

## <a name="relationships"></a>Связи

| Связь       | Тип                                       | Описание                                                                                                                                                                       |
|:-------------------|:-------------------------------------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| children           | Коллекция [driveitem](driveitem.md)       | Коллекция, содержащая объекты Item для непосредственных дочерних элементов Item. Дочерние элементы есть только у элементов, представляющих папки. Только для чтения. Допускается значение null.                                        |
...
| разрешения        | Коллекция объектов [permission](permission.md)     | Набор разрешений для элемента. Только для чтения. Допускается значение null.                                                                                                                         |
...
| [Поиск изменений](../api/item_delta.md)                   | `GET /drive/root/delta`                  |
| [Список эскизов](../api/item_list_thumbnails.md)      | `GET /drive/items/{item-id}/thumbnails`  |
...
В OneDrive для бизнеса или библиотеках документов SharePoint свойство **cTag** не возвращается, если в роли элемента выступает ресурс [Folder](folder.md).
