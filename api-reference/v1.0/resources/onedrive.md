...
* _Аспекты_ (например, **file** и **photo**) содержат сложные значения. Наличие аспекта **file** или **folder** указывает поведение и свойства ресурса **DriveItem**.
...
Ресурсы [DriveItem](driveitem.md) представляют собой объекты в файловой системе диска.
Доступ к ним можно получить по свойству **id** с использованием синтаксиса `/items/{item-id}` или по пути в файловой системе с использованием синтаксиса `/root:/path/to/item/`.
...
Пользователь, у которого есть личная (не рабочая или учебная) учетная запись OneDrive, может добавить один или несколько общих элементов с другого диска в свое хранилище OneDrive.
Эти общие элементы отображаются в коллекции **children** как экземпляры **DriveItem** с аспектом [remoteItem](remoteitem.md).
...
Одно из самых распространенных действий для OneDrive и библиотек документов SharePoint — предоставление общего доступа к содержимому.
С помощью Microsoft Graph ваше приложение может создавать [ссылки для общего доступа](../api/driveitem_createlink.md), [добавлять разрешения и отправлять приглашения](../api/driveitem_invite.md) для доступа к элементам на диске.
