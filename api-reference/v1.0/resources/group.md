...
|groupTypes|Коллекция String| Задает тип группы и членства в ней.  <br><br>Если коллекция содержит объект `Unified`, то эта группа является группой Office 365. В противном случае она является группой безопасности.  <br><br>Если коллекция включает объект `DynamicMembership`, то в этой группе используется динамическое членство. В противном случае членство является статическим.  <br><br>Возвращается по умолчанию. Поддерживает параметр $filter.|
