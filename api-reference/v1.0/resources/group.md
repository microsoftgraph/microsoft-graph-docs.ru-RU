...
|proxyAddresses|Коллекция String| Для выражений фильтра в случае многозначных свойств требуется оператор **any**. Только для чтения. Значение null не допускается. Поддерживает параметр $filter. |
...
|settings|Коллекция объектов [groupSetting](groupsetting.md)| Только для чтения. Допускает значение null.|
