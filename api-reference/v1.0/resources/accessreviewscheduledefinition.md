...
description: Представляет планирование проверки Azure AD доступа.
author: zhusijia26
...
| createdDateTime  |DateTimeOffset  | Метка времени создания ряда проверки доступа. Поддерживает `$select` и `$orderBy`. Только для чтения. |
...
|Связь|Тип|Описание|
|:---|:---|:---|
| instances|[Коллекция accessReviewInstance](accessreviewinstance.md) | Если **accessReviewScheduleDefinition** является повторяющейся проверкой доступа, экземпляры представляют каждое повторение. Повторная проверка будет иметь только один экземпляр. Экземпляры также представляют каждый уникальный ресурс, который просматривается в **accessReviewScheduleDefinition**. Если проверка содержит несколько ресурсов и несколько экземпляров, каждый ресурс будет иметь уникальный экземпляр для каждого повторения. |
