...
- Добавление собственных данных в качестве настраиваемых заголовков интернет-сообщений. Добавляйте настраиваемые заголовки только при создании сообщения и присваивайте им названия, начинающиеся с «x-». После отправки сообщения заголовки изменить невозможно. Чтобы получить заголовки сообщений, в операции [get message](../api/message_get.md) примените параметр запроса `$select`.
- Добавление собственных данных в качестве настраиваемых свойств в [расширениях](../../../concepts/extensibility_overview.md).
- Используйте запрос [разностный запрос](../../../concepts/delta_query_overview.md) для отслеживания вносимых добавлений, удалений и обновлений при помощи функции [delta](../api/message_delta.md).
...
|[copy](../api/message_copy.md)|[Сообщение](message.md)|Копирование сообщения в папку.|
|[createForward](../api/message_createforward.md)|[Сообщение](message.md)|Создание черновика пересылаемого сообщения. После этого вы сможете [обновить](../api/message_update.md) или [отправить](../api/message_send.md) черновик.|
|[createReply](../api/message_createreply.md)|[Сообщение](message.md)|Создание черновика ответного сообщения. После этого вы сможете [обновить](../api/message_update.md) или [отправить](../api/message_send.md) черновик.|
|[createReplyAll](../api/message_createreplyall.md)|[Сообщение](message.md)|Создание черновика сообщения для ответа всем пользователям. После этого вы сможете [обновить](../api/message_update.md) или [отправить](../api/message_send.md) черновик.|
...
|[move](../api/message_move.md)|[Сообщение](message.md)|Перемещение сообщения в папку. При этом в целевой папке создается новая копия сообщения.|
...
|bodyPreview|String (строка)|Первые 255 символов в тексте сообщения. В текстовом формате.|
...
|changeKey|String (строка)|Версия сообщения.|
|conversationId|String (строка)|Идентификатор беседы, к которой принадлежит электронное сообщение.|
...
|from|[recipient](recipient.md)|Владелец почтового ящика и отправитель сообщения. Значение должно соответствовать фактически используемому почтовому ящику.|
|hasAttachments|Boolean (логический)|Указывает на наличие вложений в сообщении. Это свойство не включает встроенные вложения, поэтому, если сообщение содержит только встроенные вложения, это свойство имеет значение false. Чтобы проверить наличие встроенных вложений, проанализируйте свойство **body** на наличие атрибута `src`, например `<IMG src="cid:image001.jpg@01D26CD8.6C05F070">`.|
|id|String (строка)|Уникальный идентификатор сообщения (обратите внимание, что это значение может меняться при перемещении и изменении сообщения).|
|importance|importance| Важность сообщения: `Low`, `Normal`, `High`.|
|inferenceClassification | inferenceClassificationType | Классификация сообщения для пользователя на основе подразумеваемой релевантности или важности либо явного переопределения. Возможные значения: `focused` или`other`. |
|internetMessageHeaders | Коллекция [internetMessageHeader](internetmessageheader.md) | Коллекция заголовков сообщений, определенных в документе [RFC5322](https://www.ietf.org/rfc/rfc5322.txt). Набор включает заголовки сообщений, которые указывают маршрут доставки сообщения от отправителя получателю. Он также может содержать настраиваемые заголовки сообщений, в которых содержатся данные приложения для сообщения. |
|internetMessageId |Строка |Идентификатор сообщения в формате, установленном документом [RFC2822](http://www.ietf.org/rfc/rfc2822.txt). |
|isDeliveryReceiptRequested|Логический|Указывает, запрашивается ли уведомление о прочтении сообщения.|
|isDraft|Boolean (логический)|Указывает, является ли сообщение черновиком. Сообщение считается черновиком, если оно еще не отправлено.|
...
|parentFolderId|String (строка)|Уникальный идентификатор родительского ресурса mailFolder для сообщения.|
