...
>[!NOTE]
> Если какой-либо из проверок доступа, соответствующих фильтру, является повторяющейся проверкой доступа, один объект **акцессревиев** будет возвращен для представления каждого повторяющегося ряда в целом, в дополнение к любому текущему, предшествующему и следующему экземпляру. Например, если имеется ежемесячная проверка доступа участников группы "гость", то есть Ежеквартальная проверка доступа участников группы "гость" для группы "б" и однократный доступ на проверку участников группы "гость", каждый из этих повторений начался, а абонент запросы на рецензии на доступ к шаблону бизнес-процесса, просматривая участников группы, будут возвращены три объекта, представляющие три ряда, а также три объекта для текущего экземпляра проверки доступа, а потенциально три объекта для следующего будущие экземпляры. Чтобы получить экземпляры повторяющейся проверки доступа или экземпляра проверки доступа, запланированный на определенный месяц или квартал, вызывающий может впоследствии перемещаться по связи **экземпляра** объекта повторяющегося объекта **акцессревиев** . Связь **экземпляра** связывается с объектами **акцессревиев** для текущего или прошлых экземпляров проверки поповторяющегося доступа.
...
|:--------------------------------------|:---------------------------------------------------------|
|Делегированное (рабочая или учебная учетная запись)     | Акцессревиев. Read. ALL, Акцессревиев. ReadWrite. Membership, Акцессревиев. ReadWrite. ALL  |
