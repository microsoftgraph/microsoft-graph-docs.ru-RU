...
Шаблон каталога параметр представляет шаблон параметры, с которых могут быть созданы параметры клиента. Эта операция позволяет получать свойства объекта directorySettingTemplate, включая доступные параметры и значения по умолчанию.

> **Примечание**: версия /beta этот интерфейс API является только относится к группам. Для *получения groupSettingTemplate*переименован версии /v1.0 этот интерфейс API.
