...
|mail|String|SMTP-адрес пользователя, например "gregory@contoso.onmicrosoft.com". Изменения этого свойства также будут приводить к обновлению коллекции **proxyAddresses** пользователя, чтобы включить значение в качестве SMTP-адреса. <br><br>Возвращается по умолчанию. Поддерживает параметр $filter.|
