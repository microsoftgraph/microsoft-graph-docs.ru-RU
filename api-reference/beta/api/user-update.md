...
Обновление свойств объекта [user](../resources/user.md). Не все свойства могут обновляться пользователями-членами или гостями с разрешениями по умолчанию без ролей администратора. [Сравните разрешения](/azure/active-directory/fundamentals/users-default-permissions#compare-member-and-guest-default-permissions) по умолчанию для членов и гостей, чтобы увидеть свойства, которые они могут управлять.
...
> - Обновление свойств **businessPhones**, **mobilePhone** или **otherMails** других пользователей разрешается только для пользователей, не являющихся администраторами, или для пользователей, которым назначена одна из следующих ролей: читатель каталога, приглашающий гостей, читатель Центра сообщений или читатель отчетов. Дополнительные сведения см. в материале Администратор Helpdesk (Password) в встроенных ролях [Azure AD.](/azure/active-directory/roles/permissions-reference)  Это относится к приложениям с предоставленными разрешениями User.ReadWrite.All или Directory.ReadWrite.All (делегированными или для приложений).
> - Ваша личная учетная запись Майкрософт должна быть привязана к клиенту AAD, чтобы обновить свой профиль с помощью делегированного разрешения User.ReadWrite в личной учетной записи Майкрософт.
...
| companyName | String | Название организации, с которой связан пользователь. Это свойство может быть полезно для описания компании внешнего пользователя. Максимальная длина имени компании — 64 символа. |
...
|почта;|String|Адрес SMTP для пользователя, `jeff@contoso.onmicrosoft.com` например. Изменение этого свойства также приведет к обновлению коллекции пользователя **proxyAddresses**, которая будет включать это значение как SMTP-адрес. <br><br>Возвращается по умолчанию. Поддерживает `$filter`.|
...
|passwordPolicies|String|Задает политики паролей для пользователя. Это значение является переумежанием с одним возможным значением, которое позволяет заданным более слабым паролям, чем `DisableStrongPassword` указана политика по умолчанию. `DisablePasswordExpiration` можно также укаварить. Эти два могут быть указаны вместе; например: `DisablePasswordExpiration, DisableStrongPassword` .|
...
|preferredLanguage|String|Предпочитаемый язык для пользователя. Следует следовать коду ISO 639-1; например `en-US` .|
...
|usageLocation|String|Двухбуквенный код страны (по стандарту ISO 3166). Необходим для пользователей, которым будут назначены лицензии, в связи с законодательным требованием проверять доступность служб в разных странах.  Примеры включают: `US` , `JP` и `GB` . Значение null не допускается.|
...
|userType|Строка|Строковая величина, которую можно использовать для классификации типов пользователей в каталоге, таких как `Member` и `Guest` .          |
