...
|roleDefinitionId|String|Идентификатор объекта unifiedRoleDefinition, для который предназначено назначение. Обязательно. Только для чтения.|
...
Ниже приведен пример отклика. Запрос возвращает объект ответа, в котором отображается состояние ранее допустимых изменений назначения как `Revoked`. Субъект больше не будет видеть свою ранее допустимую роль.
