...
>**Примечание:** При попытке сохранить полное локальное представление элементов в папке или диске необходимо использовать `delta` для начального перечисления.
Другие подходы, такие как разбиение по `children` коллекции для папки, не возвращают каждый отдельный элемент, если при перечислении происходят записи. Используйте `delta` единственный способ, чтобы убедиться, что вы прочитали все нужные данные.
...
По умолчанию ответ на запрос изменений включает информацию об общем доступе для всех изменившихся элементов в запросе, даже если они наследуют разрешения от родительского элемента и сами непосредственно не имеют изменений общего доступа. Обычно это приводит к последующему вызову, чтобы получить сведения о разрешениях для каждого элемента, а не только тех, чьи сведения об общем доступе изменились. Чтобы получить более детальные сведения о произошедших изменениях разрешений, добавьте в запрос заголовок `Prefer: hierarchicalsharing`.
