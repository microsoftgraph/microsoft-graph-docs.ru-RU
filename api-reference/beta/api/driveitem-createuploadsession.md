...
Например, `item` свойство позволяет задать следующие параметры:<!-- { "blockType": "resource", "@odata.type": "microsoft.graph.driveItemUploadableProperties" } -->
```json
{
...
|:-----------|:------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| *if-match* | etag  | Если этот заголовок запроса включен, а предоставленное значение eTag (или cTag) не совпадают с текущим тегом ETag элемента, возвращается ответ `412 Precondition Failed` об ошибке. |
...
Фрагменты файла должны быть отправлены последовательно по порядку.
