...
[Пометить timeOffReason](../resources/timeoffreason.md) как неактивное, задав **свойство isActive.** Каждая команда должна включать по крайней мере один **разOffReason**.

Этот метод не удаляет указанный экземпляр **timeOffReason.** [Экземпляры timeOffItem,](../resources/timeoffitem.md) которые были назначены по этой причине, остаются назначены по этой причине.
