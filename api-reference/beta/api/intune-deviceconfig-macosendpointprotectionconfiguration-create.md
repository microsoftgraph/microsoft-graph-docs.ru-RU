...
|advancedThreatProtectionRealTime|[включить](../resources/intune-shared-enablement.md)|Определяет, следует ли включить защиту в режиме реального времени для Microsoft Defender для конечной точки (ранее Microsoft Defender ATP) на macOS. Возможные значения: `notConfigured`, `enabled`, `disabled`.|
