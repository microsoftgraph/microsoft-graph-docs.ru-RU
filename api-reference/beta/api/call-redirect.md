...
|maskCallee|Boolean|Указывает, должен ли вызываемая стороны быть скрыта от вызываемой стороны. Если установлено true, то идентификатор вызываемого пользователя — это идентификатор бота. Значение по умолчанию: false.|
|maskCaller|Boolean|Указывает, следует ли скрывать вызываемую от вызываемой стороны. Если установлено true, то идентификатором вызывающего пользователя является идентификатор бота. Значение по умолчанию: false.|
