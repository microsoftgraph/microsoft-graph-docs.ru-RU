...
- Можно использовать параметр запроса `$select` , как в любом запросе GET, чтобы указать только свойства, необходимые для достижения наилучшей производительности. Свойство **id** возвращается всегда.
...
В случае успешного выполнения этот метод возвращает код `200 OK` отклика и объект [коллекции orgContact](../resources/orgcontact.md) в тексте отклика. Оклик также содержит URL-адрес `@odata.nextLink` или `@odata.deltaLink`.
...
Ниже приведен пример запроса. Так как параметр отсутствует `$select` , отслеживается и возвращается набор свойств по умолчанию.
...
Ниже приведен пример отклика при использовании параметра `@odata.deltaLink`, полученного в начале запроса. Обратите внимание `mail` , что свойство не включено, что означает, `displayName` `jobTitle` что оно не изменилось с момента последнего разностного запроса и включено, что означает, что их значения изменились.
