...
- Вы можете использовать `$select` параметр запроса, как в любом запросе GET, чтобы указать только те свойства, которые необходимы для достижения максимальной производительности. Свойство **id** возвращается всегда.
...
В случае успешного выполнения этот метод возвращает `200 OK` код отклика и объект коллекции [orgContact](../resources/orgcontact.md) в тексте отклика. Оклик также содержит URL-адрес `nextLink` или `deltaLink`.
...
> **Примечание:** Таким образом, невозможно определить, изменяется ли свойство, изучив ответ. Кроме того, разностные ответы имеют большой размер, так как они содержат все значения свойств, как показано в [примере 2](#example-2-selecting-three-properties).
...
Ниже приведен пример запроса. Так как параметр отсутствует `$select` , набор свойств по умолчанию отслеживается и возвращается.
...
Ниже приведен пример отклика при использовании параметра `deltaLink`, полученного в начале запроса. Обратите внимание, что `mail` свойство не включается, что означает, что он не изменился с момента последнего запроса Дельта `displayName` `jobTitle` , а включается, что означает, что их значения изменились.
