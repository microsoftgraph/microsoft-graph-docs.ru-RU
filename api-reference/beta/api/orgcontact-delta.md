...
- Вы можете использовать параметр `$select` запроса, как в любом запросе GET, чтобы указать только те свойства, которые необходимы для достижения максимальной производительности. Свойство **id** возвращается всегда.
...
Ниже приведен пример запроса. Так как `$select` параметр отсутствует, набор свойств по умолчанию отслеживается и возвращается.
...
Ниже приведен пример отклика при использовании параметра `deltaLink`, полученного в начале запроса. Обратите внимание `mail` , что свойство не включается, что означает, что он не изменился с момента последнего запроса Дельта; `displayName` они `jobTitle` включены, а это означает, что их значения изменились.
