...
> [!Note]
> Действие записи поддерживается только для [вызовов](../resources/call.md) , инициированных с помощью [сервицехостедмедиаконфиг](../resources/servicehostedmediaconfig.md). Это действие не записывает весь вызов. Максимальная длина записи составляет 5 минут. Запись не сохраняется пермаментли платформой Cloud Communications и удаляется вскоре после завершения вызова. Bot должен быстро скачать запись (используя значение **рекординглокатион** , указанное в завершенном уведомлении) после завершения операции записи.
