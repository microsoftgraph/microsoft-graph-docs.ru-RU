...
|autoFillForceAuthentication|Boolean|Указывает, следует ли принудительно использовать проверку подлинности пользователей перед автозаполнением паролей и данных кредитных карт в Safari и других приложениях на защищенном устройстве.|
...
|kioskModeAllowTouchscreen|Boolean|Указывает, можно ли использовать сенсорный экран в режиме киоска. Функциональные возможности этого свойства избыточны при стандарте ОС и неподготовлены. Вместо этого используйте экран KioskModeBlockTouchscreen.|
...
|airPrintBlockCredentialsStorage|Boolean|Указывает, заблокировано ли хранилище имени пользователя и пароля для ключей в Airprint (iOS 11.0 и более поздних).|
