...
|:---|:---|:---|
|id|String|Идентификатор для предопределяемой величины. Может иметь длину до 64 символов и включать символы Unicode. Может включать пробелы, но некоторые специальные символы не допускаются. Не удается изменить позже. Деликатный. Обязательный.|
