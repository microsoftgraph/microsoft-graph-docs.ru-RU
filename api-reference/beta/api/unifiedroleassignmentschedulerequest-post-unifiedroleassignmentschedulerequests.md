...
ms.openlocfilehash: b21347c0ff5c4d8d4175968556fca0f1d5167ff1
ms.sourcegitcommit: 43a7c971a97ce1e4c55cbae089820bfce7dfe42b
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 03/29/2022
ms.locfileid: "64510488"
---
...
|isValidationOnly|Boolean|Указывает, является ли вызов проверкой или фактическим вызовом. Только задайте это свойство, если необходимо проверить, подчиняется ли активация дополнительным правилам, таким как MFA, перед отправкой запроса.|
...
В случае успешного `201 Created` выполнения этот метод возвращает код ответа и объект [unifiedRoleAssignmentScheduleRequest](../resources/unifiedroleassignmentschedulerequest.md) в тексте ответа.

Когда вызываемого пользователя не вызывалось для многофакторной проверки подлинности во время записи в сеансе, запрос с действием SelfActivate `400 Bad request` сбой и возвращает код ответа.
...
В следующем запросе администратор `fdd7a751-b60b-444a-984c-02652fe8fa1c` создает запрос, чтобы назначить роль, определенную главной, идентифицированной **по id**`07706ff1-46c7-4847-ae33-3003830675a1`. Область их роли — это все объекты каталога в клиенте, а назначение является постоянным, то есть не истекает.
...
В следующем запросе пользователь, идентифицированный **principalId**`c6ad1942-4afa-47f8-8d48-afb5d8d69d2f`, активирует свою собственную роль, определенную .`9b895d92-2cd3-44c7-9d02-a6ac2d5ea5c3` Область их роли — все объекты каталога в клиенте, а назначение — пять часов. Чтобы выполнить этот запрос, вызываемого пользователя необходимо применять многофакторную проверку подлинности (MFA) и запускать запрос в сеансе, в котором он был опровержение для MFA.
