...
Создание [переопределения](../resources/manage-focused-inbox.md) "Отправляемой почты" для отправитель, идентифицированный SMTP-адресом. Последующие сообщения с этого SMTP-адреса всегда будут классифицироваться так, как указано в переопределении.

**Примечание**

- Если переопределения уже существуют с тем же SMTP-адресом, то поля **classifyAs** и имени этого переопределения обновляются с помощью предоставленных значений. 
