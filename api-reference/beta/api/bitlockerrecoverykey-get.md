...
По умолчанию эта операция не возвращает свойство **Key** , представляющее фактический ключ восстановления. Чтобы включить свойство **Key** в ответ, используйте `$select` параметр запроса OData. Включение `$select` параметра запроса инициирует аудит Azure AD для операции и создает журнал аудита. Журнал [аудита Azure AD](https://docs.microsoft.com/azure/active-directory/reports-monitoring/concept-audit-logs) можно найти в категории кэйманажемент.
