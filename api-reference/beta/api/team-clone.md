...
После возвращения клона POST необходимо получить операцию, возвращаемую [](../resources/teamsasyncoperation.md) расположением: заглавной, чтобы узнать, "запущена" или "успешно" или "не удалось". Вы должны продолжать получать, пока состояние не будет "запущено". Рекомендуемая задержка между GETs — 5 секунд.
...
|:---------------|:--------|:----------|
|classification|String (необязательный)|Описывает классификацию для группы (например, низкое, среднее или высокое влияние бизнеса). Допустимые значения для этого свойства определяются путем создания значения параметра [ClassificationList на](../resources/directorysetting.md) основе [определения шаблона](../resources/directorysettingtemplate.md). Если классификация не указана, классификация будет скопирована из исходной группы или группы.|
