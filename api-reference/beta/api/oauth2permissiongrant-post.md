...
Создайте делегированное предоставление разрешений, представленное [объектом oAuth2PermissionGrant](../resources/oauth2permissiongrant.md) .
...
В следующей таблице показаны свойства, необходимые при создании [объекта oAuth2PermissionGrant](../resources/oauth2permissiongrant.md).
...
| scope | String | Разделенный пробелами список значений утверждений для делегированных разрешений, которые должны быть включены в маркеры доступа для приложения ресурсов (API). Например, `openid User.Read GroupMember.Read.All`. Каждое значение утверждения должно соответствовать полю значения одного из делегированных разрешений, определенных API, перечисленным в свойстве **publishedPermissionScopes**  субъекта-службы [ресурсов](../resources/serviceprincipal.md). |
| startTime | DateTimeOffset | В настоящее время значение времени начала игнорируется, но это значение является обязательным. Обязательный. |
| истечение срока действия | DateTimeOffset | В настоящее время значение конечного времени игнорируется, но оно является обязательным. Обязательный. |

## <a name="response"></a>Отклик

В случае успешного выполнения этот метод возвращает код отклика серии 200 и новый объект [oAuth2PermissionGrant](../resources/oauth2permissiongrant.md) в теле отклика.
