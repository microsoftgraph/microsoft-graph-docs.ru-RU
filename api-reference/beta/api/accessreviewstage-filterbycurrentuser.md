...
description: Возвращаем все объекты accessReviewStage для данного рецензента.
...
Возвращайте все [объекты accessReviewStage](../resources/accessReviewStage.md) в заданном [accessReviewInstance](../resources/accessreviewinstance.md) , где вызываемая пользователь является рецензентом на одном или более [объектах accessReviewInstanceDecisionItem](../resources/accessreviewinstancedecisionitem.md) .
...
|:---|:---|:---|
|on|accessReviewStageFilterByCurrentUserOptions|Фильтрует результаты на основе вызываемой пользователя. Разрешено значение `reviewer`. Это возвращает все объекты accessReviewStage в accessReviewInstance, где вызываемая пользователь является рецензентом. Обязательный элемент.|

Эта функция также поддерживает `$select`параметры запросов , и `$skip` `$filter``$orderBy``$top` OData, чтобы помочь настроить ответ. Общие сведения см. в статье [Параметры запроса OData](/graph/query-parameters).
...
В случае успешной работы эта функция возвращает код `200 OK` ответа и [коллекцию accessReviewStage](../resources/accessreviewstage.md) в тексте ответа.
