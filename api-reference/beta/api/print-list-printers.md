...
* Операторы `$select` `$expand` и операторы поддерживаются для свойства `shares` навигации, но не для `jobs`.
...
В случае успешного выполнения этот метод возвращает код `200 OK` отклика и коллекцию объектов [принтера](../resources/printer.md) в тексте отклика.
