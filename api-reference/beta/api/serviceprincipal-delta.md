...
Получать только что созданные, обновленные или удаленные основные службы, не выполняя полное чтение всей коллекции ресурсов.
...
Чтобы начать отслеживание изменений, необходимо внести запрос, включив функцию delta для ресурса servicePrincipal. 
...
- Имеется ограниченная поддержка параметра `$filter`:
  * Единственное поддерживаемые выражения для отслеживания изменений для `$filter` определенных ресурсов по их ид:  `$filter=id+eq+{value}` или `$filter=id+eq+{value1}+or+id+eq+{value2}` . Допустимые числа ид ограничены максимальной длиной URL-адреса.
...
В случае успеха этот метод возвращает код отклика и объект `200 OK` [коллекции servicePrincipal](../resources/serviceprincipal.md) в тексте отклика. Ответ также включает URL-адрес nextLink или URL-адрес deltaLink. 
