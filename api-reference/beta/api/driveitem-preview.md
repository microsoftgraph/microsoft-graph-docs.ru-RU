...
|   Имя      |  Тип         | Описание
|:------------|:--------------|:-----------------------------------------------
| Средство просмотра      | строка        | Необязательный параметр. Предварительная версия приложения для использования. `onedrive` или `office`. Если значение null, подходящее средство просмотра выбирается автоматически.
| chromeless  | boolean       | Необязательное. Если `true` (по умолчанию), внедренное представление не будет содержать все элементы управления.
| Свойство allowEdit   | boolean       | Необязательное. Если `true`, файл можно редактировать в пользовательском Интерфейсе внедренных.
