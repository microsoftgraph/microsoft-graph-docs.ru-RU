...
В следующей таблице перечислены три сценария, в которых можно получить открытое расширение из поддерживаемого экземпляра ресурса.

|**Сценарий GET**|**Поддерживаемые ресурсы**|**Текст ответа**|
|:-----|:-----|:-----|
|Получение определенного расширения из экземпляра известного ресурса.| [Административная единица](../resources/administrativeunit.md), [устройство](../resources/device.md), [событие](../resources/event.md), [Группа](../resources/group.md), [событие группы](../resources/event.md), [Отправка по группе](../resources/post.md), [сообщение](../resources/message.md), [Организация](../resources/organization.md), [личный контакт](../resources/contact.md), [пользователь](../resources/user.md) | Только открытый добавочный номер.|
|Получение экземпляра известного ресурса, дополненного определенным расширением.|Административная единица, устройство, событие, группа, событие группы, отправка по группе, сообщение, организация, личный контакт, пользователь |Экземпляр ресурса, дополненный открытым расширением.|
|Поиск экземпляров ресурсов и их дополнение определенным расширением. | Событие, событие группы, отправка в группу, сообщение, личный контакт |Экземпляры ресурсов, развернутые с открытым расширением.|

## <a name="permissions"></a>Разрешения

В зависимости от ресурса, содержащего добавочный номер и запрошенный тип разрешения (делегированное или приложение), разрешение, указанное в следующей таблице, является минимальным требованием для вызова этого API. Дополнительные сведения, включая сведения о том, как выбрать разрешения, см. в статье [Разрешения](/graph/permissions-reference).
...
В этом разделе указан синтаксис для каждого из трех перечисленных выше сценариев `GET`.
...
Для типов ресурсов Event, Group POST, Message и Personal Contact можно использовать тот же запрос REST, что и при извлечении экземпляра ресурса, искать расширение, которое соответствует фильтру для свойства **ID** , и развертывать экземпляр с расширением. Ответ включает в себя большинство свойств ресурса.
...
Для типов ресурсов "устройство", "Группа", "Организация" и "пользователь" `$select` также необходимо использовать параметр, чтобы включить свойство **ID** и любые другие свойства, необходимые для экземпляра ресурса:
...
>**Примечание:** Приведенный выше синтаксис показывает некоторые распространенные способы идентификации экземпляра ресурса или коллекции, чтобы получить из него расширение. Весь другой синтаксис, позволяющий идентифицировать эти экземпляры или коллекции ресурсов, поддерживает доступ к открытым расширениям из них аналогичным образом.
