...
Вы можете отфильтровать сообщения и получить только те из них, которые [](../resources/mention.md) содержат упоминания пользователя, выполнившего вход. См. [пример](#request-2) ниже. По умолчанию операция `GET /me/messages` не возвращает свойство **упоминаний** . Используйте параметр `$expand` запроса для [поиска сведений о каждом упоминаемом в сообщении](../api/message-get.md#example-2-get-all-mentions-in-a-specific-message).
...
Чтобы получить все сообщения в почтовом ящике пользователя, включающее **в** себя упоминание пользователя:
...
Параметр запроса в `$filter` свойстве **mentionsPreview** можно использовать для получения сообщений, в которых упоминается вошедшающий пользователь.
...
В случае успешного выполнения этот метод возвращает код `200 OK` отклика и коллекцию объектов [сообщений](../resources/message.md) в тексте отклика.

## <a name="examples"></a>Примеры
### <a name="example-1-list-all-messages"></a>Пример 1. Перечисление всех сообщений
#### <a name="request"></a>Запрос
В первом примере по умолчанию возвращается 10 основных сообщений в почтовом ящике пользователя, выполнившего вход. `$select` используется для получения подмножества свойств каждого сообщения в ответе. 
...
#### <a name="request"></a>Запрос
В следующем примере фильтруются все сообщения в почтовом ящике пользователя, выполнившего вход, для тех, кто упоминает пользователя. Он также использует `$select` для возврата подмножества свойств каждого сообщения в ответе. 

В следующем примере также включается кодирование URL-адресов для пробелов в строке параметра запроса.
...
### <a name="example-3-use-prefer-header-to-get-the-message-body-and-uniquebody-is-text-format"></a>Пример 3. Использование заголовка prefer для получения текста сообщения, а uniqueBody — текстовый формат
#### <a name="request"></a>Запрос
В третьем примере показано`Prefer: outlook.body-content-type="text"`, как использовать заголовок для получения  текста и свойств **uniqueBody** каждого сообщения в текстовом формате.
