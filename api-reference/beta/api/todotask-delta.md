...
Для отслеживания изменений в ресурсах **тодотаск** в **тодотасклист** , начиная с последнего цикла отслеживания изменений, необходимо выполнить один или несколько вызовов функции **Delta** для получения набора добавочных изменений. В приведенном ниже примере показано, как начать следующий цикл отслеживания изменений, используя URL-адрес, `deltaLink` возвращенный при вызове последней функции **Дельта** последнего цикла, который содержит элемент `deltaToken` . Этот вызов функции **Delta** ограничен максимальным числом элементов **тодотаск** в тексте отклика до 2.
