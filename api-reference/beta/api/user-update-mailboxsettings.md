...
Обновляя предпочтительный часовой пояс для пользователя, можете указать его в формате Windows или [часового пояса IANA](https://www.iana.org/time-zones) (также известен как часовой пояс Олсона). Можно также настроить часовой пояс как показано в следующем [примере 2](#request-2) .
