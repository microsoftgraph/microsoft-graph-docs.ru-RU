...
Помните, что у пользователя не может `alternateMobile` быть номера без `mobile` номера. `mobile` Если вы хотите удалить номер из пользователя, который также имеет `alternateMobile` номер, сначала [Обновите](phoneauthenticationmethod-update.md) его `mobile` до нового номера, а затем удалите `alternateMobile` номер.
