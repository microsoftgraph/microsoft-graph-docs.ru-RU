...
Создавая черновик в одном вызове **POST,** вы можете:
...
В тексте запроса укажу представление объекта [сообщения](../resources/message.md) в JSON.
...
В случае успеха этот метод возвращает код отклика и `201 Created` [объект сообщения](../resources/message.md) в тексте отклика.
...
В следующем примере показан черновик сообщения от Randi Welch к Samantha Чтобы отправить сообщение. Сообщение также содержит упоминание другого пользователя, Dana Swope.
