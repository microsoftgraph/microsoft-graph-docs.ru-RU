...
Область действия поискового запроса определяется с помощью свойства **entityTypes** полезных данных запроса **query**. В таблице ниже описаны доступные типы запросов и поддерживаемые разрешения на доступ к данным.
...
|[list](list.md)|Sites.Read.All, Sites.ReadWrite.All| SharePoint и OneDrive | Списки. Обратите внимание, что библиотеки документов также возвращаются в виде списков. |
...
- **from**  — целое значение, указывающее начальную точку (начиная с 0) для перечисления результатов поиска на странице. Значение по умолчанию – 0.
...
**listItem** и **externalItem**  — единственные поддерживаемые сущности, которые позволяют настройку расширенных полей в схеме. Из всех других сущностей расширенные свойства невозможно получить. Например, при создании поля для **externalItem** в схеме поиска, или при наличии пользовательского столбца в **listItem** , эти свойства можно извлечь из поиска. Чтобы получить расширенное свойство файла, укажите в запросе тип **listItem**.
...
Свойства, доступные для поиска, зависят от типа объекта. Подробнее см. в:
...
Свойства, используемые в поисковом запросе и отклике, были переименованы или удалены. В большинстве случаев первоначальные свойства устаревают и заменяются текущими свойствами, как указано в следующей таблице.

Обновите существующие приложения так, чтобы в них использовались текущие имена свойств и типов, а отклик содержал имена текущих свойств.
Для обеспечения обратной совместимости первоначальные свойства и типы доступны и могут использоваться до **31 декабря 2020 г.** , после этого они будут удалены.

| Ресурс                           | Тип изменения   | Первоначальное свойство | Текущее свойство|
|:-----------------------------------|:--------------|:------------------|:----------------|
| [searchRequest](./searchrequest.md)| Переименованное свойство | **stored_fields** | **fields**      |
| [searchQuery](./searchquery.md)    | Переименованное свойство | **query_string** | **queryString** |
| [searchQueryString](./searchquerystring.md) | Устаревший ресурс | Неприменимо | Неприменимо |
| [searchHit](./searchhit.md)        | Переименованное свойство | **_id** | **hitId** |
| [searchHit](./searchhit.md)        | Переименованное свойство | **_score** | **rank** |
| [searchHit](./searchhit.md)        | Удаленное свойство | **_sortField** | Неприменимо |
