...
**listItem** и **externalItem** — единственные поддерживаемые сущности, которые позволяют настройку расширенных полей в схеме. Из всех других сущностей расширенные свойства невозможно получить. Например, если имеется поле для **externalItem** в схеме поиска или пользовательский столбец в **listItem**, можно получит эти свойства из результатов поиска. Чтобы получить расширенное свойство файла, укажите в запросе тип **listItem**.
...
Агрегаты в настоящее время поддерживаются только для следующих типов SharePoint и OneDrive: [driveItem](driveitem.md), [listItem](listitem.md), [list](list.md), [site](site.md). Вскоре они будут поддерживаться для уточняемых свойств в объекте [externalItem](externalItem.md) соединителей Microsoft Graph.
