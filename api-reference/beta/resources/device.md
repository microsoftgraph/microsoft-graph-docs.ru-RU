...
|alternativeSecurityIds| microsoft.graph.alternativeSecurityId коллекции | Только для внутреннего использования. Значение NULL не допускается. |
...
|deviceId|String (идентификатор)| Уникальный идентификатор, задаваемый службой Azure Device Registration Service при регистрации. |
...
|Имя| Строка | Понятное имя устройства. Возвращается только при входе пользователя с учетной записью Майкрософт в составе рим проекта. |
|Состояние | Строка| Устройство является сетевым и автономным. Возвращается только при входе пользователя с учетной записью Майкрософт в составе рим проекта. |
|Платформа |Строка|Платформа устройства. Возвращается только при входе пользователя с учетной записью Майкрософт в составе рим проекта. Возвращается только при входе пользователя с учетной записью Майкрософт в составе рим проекта.|
|Kind| Строка| Форм-фактора устройства. Возвращается только при входе пользователя с учетной записью Майкрософт в составе рим проекта. |
|Model| Строка| Модель устройства. Возвращается только при входе пользователя с учетной записью Майкрософт в составе рим проекта. |
|Производитель| Строка| Производителя устройства. Возвращается только при входе пользователя с учетной записью Майкрософт в составе рим проекта. |
...
|команды | Microsoft.Graph.Command коллекции | Набор команд, отправленные на это устройство|
