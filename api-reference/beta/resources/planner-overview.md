...
При [создании нового плана](../api/planner-post-plans.md) сделайте группу его контейнером, задав свойство `container` для объекта плана. Планы должны содержаться поддерживаемым ресурсом.
...
Чтобы [создать задачу в плане](../api/planner-post-tasks.md), при ее создании укажите в качестве свойства `planId` объекта задачи идентификатор плана.
...
* За новым объектом `plannerTask` будет следовать соответствующий объект `plannerTaskDetails`.
* За новым объектом `plannerPlan` будет следовать соответствующий объект `plannerPlanDetails`.
...
1. Вызывающий объект запускает запрос синхронизации изменений, получая `@odata.nextLink` и пустую коллекцию изменений.
...
3. Вызывающий объект переходит по ссылке `@odata.nextLink`, предоставленной в исходном запросе синхронизации изменений, чтобы получить новую ссылку `@odata.deltaLink` для любых изменений с предыдущего этапа.
4. Вызывающий объект применяет изменения в возвращенном отклике изменений к объектам в кэше.
5. Вызывающий объект переходит по новой ссылке deltaLink, чтобы получить следующую ссылку deltaLink и изменения с момента создания текущей ссылки `@odata.deltaLink`.
