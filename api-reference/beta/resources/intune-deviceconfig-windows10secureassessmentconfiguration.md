...
|configurationAccountType|[secureAssessmentAccountType](../resources/intune-deviceconfig-secureassessmentaccounttype.md)|Тип учетной записи, используемой ConfigurationAccount. Возможные значения: `azureADAccount`, `domainAccount`, `localAccount`, `localGuestAccount`.|
...
|localGuestAccountName|String|Указывает отображаемый текст для локальной гостевой учетной записи, отображаемой на экране входа. Обычно это имя оценки. Когда пользователь щелкает локальную гостевую учетную запись на экране входа, запускается приложение оценки с указанным URL-адресом оценки. Безопасные оценки можно настроить только при входе в локальную гостевую учетную запись на устройствах под управлением Windows 10 версии 1903 или более поздней. Важное примечание. Это свойство должно быть задано с помощью assessmentAppUserModelID, чтобы обеспечить правильную работу локальной гостевой учетной записи для безопасной оценки.|
|assessmentAppUserModelId|String|Указывает идентификатор модели пользователя приложения, запускаемого при входе пользователя в безопасную оценку с помощью локальной гостевой учетной записи. Важное примечание. Это свойство должно быть задано с помощью localGuestAccountName, чтобы обеспечить правильную работу локальной гостевой учетной записи для безопасной оценки.|
