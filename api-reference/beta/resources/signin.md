...
|azureResourceId|String|Содержит полностью квалифицированный ИД Azure Resource Manager ресурса Azure, доступ к нему во время входной присоединения.|
...
|federatedCredentialId|String|Содержит идентификатор федераированной учетной записи удостоверения приложения, если для входа использовался федераированный учетный документ удостоверения.|
|flaggedForReview|Логическое|При сбойном входе пользователь может нажать кнопку на портале Azure, чтобы отметить неудачное событие для администраторов клиентов. Если пользователь нажал кнопку, чтобы пометить неудачный вход, это значение `true`.|
...
|id|String|Идентификатор, представляющий действие входной записи. Наследуется [от сущности](entity.md). `$filter` Поддерживает (`eq`только оператор).|
...
|isTenantRestricted|Логическое|Показывает, был ли знак в событии объектом политики ограничения клиента Azure AD.|
...
|resourceDisplayName|String|Имя ресурса, на который пользователь подписался. `$filter` Поддерживает (`eq`только оператор).|
...
|servicePrincipalId|String|Идентификатор приложения, используемый для регистрации. Это поле заполняется при входе в приложение. `$filter` Поддерживает (`eq`и `startsWith` только операторов).|
...
|signInEventTypes|Коллекция строк|Указывает категорию знака в том, что представляет событие. Для входных входов `interactiveUser` `nonInteractiveUser` пользователя категория может быть или соответствовать значению свойства **isInteractive** на ресурсе signin. Для входов управляемых входов удостоверений категория `managedIdentity`. Для входов основного знака службы категория **servicePrincipal**. Возможные значения: `interactiveUser`, `nonInteractiveUser`, `servicePrincipal`, `managedIdentity`, `unknownFutureValue`. `$filter` Поддерживает (`eq`только оператор).|
...
|userAgent|String|Сведения агента пользователя, относящиеся к входу. `$filter` Поддерживает (`eq`и `startsWith` только операторов).|
