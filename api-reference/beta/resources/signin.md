...
|:---------------|:--------|:----------|
|alternateSignInName|String|Альтернативное удостоверение для регистрации при использовании номера телефона для регистрации.|
...
|authenticationMethodsUsed|Коллекция строк|Используемые методы проверки подлинности. Возможные значения: `SMS` , `Authenticator App` , или `App Verification code` `Password` `FIDO` `PTA` `PHS` .|
...
|authenticationRequirement | Строка | Это самый высокий уровень проверки подлинности, необходимый для успешного входов.|
...
|correlationId|String|Идентификатор, который отправляется клиентом при инициале регистрации. Это используется для устранения неполадок с соответствующим действием при входе в службу поддержки.|
...
|id|String|Идентификатор, представляющий действие при входе.|
...
|resourceDisplayName|String|Имя ресурса, в который вписались пользователи.|
...
|riskLevelAggregated|riskLevel|Сводный уровень риска. Возможные значения: `none` , `low` , или `medium` `high` `hidden` `unknownFutureValue` . Значение `hidden` означает, что пользователь или вход не разрешены в службе защиты идентификации Azure AD. **Примечание.** Сведения для этого свойства доступны только для пользователей Azure AD Premium P2. Возвращаются все остальные `hidden` клиенты.|
|riskLevelDuringSignIn|riskLevel|Уровень риска при входе. Возможные значения: `none` , `low` , или `medium` `high` `hidden` `unknownFutureValue` . Значение `hidden` означает, что пользователь или вход не разрешены в службе защиты идентификации Azure AD. **Примечание.** Сведения для этого свойства доступны только для пользователей Azure AD Premium P2. Возвращаются все остальные `hidden` клиенты.|
|riskState|riskState|Состояние риска для рискованных пользователей, вход в учетную записи или событие риска. Возможные значения: `none` , `confirmedSafe` , или `remediated` `dismissed` `atRisk` `confirmedCompromised` `unknownFutureValue` .|
|servicePrincipalId|String|Идентификатор приложения, используемый для входов. Это поле заполняется при входе в приложение.|
...
|userAgent|String|Сведения об агенте пользователя, связанные со входом.|
