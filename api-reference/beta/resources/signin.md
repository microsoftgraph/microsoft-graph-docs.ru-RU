...
|:---------------|:--------|:----------|
|алтернатесигниннаме|Строка|Альтернативное удостоверение входа при входе в систему с помощью номера телефона.|
|appDisplayName|Строка|Имя приложения отображается на портале Azure.|
...
|correlationId|Строка|Идентификатор, который отправляется клиентом при запуске входа. Используется для устранения неполадок с соответствующими действиями при входе в службу поддержки.|
...
|ipAddress|Строка|IP-адрес клиента, с которого произошел вход.|
...
|riskEventTypes_v2|Коллекция строк|Список типов событий риска, связанных с входом. Возможные значения: `unlikelyTravel`, `anonymizedIPAddress`, `maliciousIPAddress`, `unfamiliarFeatures` `malwareInfectedIPAddress`,, `suspiciousIPAddress`, `leakedCredentials`, `investigationsThreatIntelligence` `generic`, или `unknownFutureValue`.|
...
|сервицепринЦипалид|Строка|Идентификатор приложения, используемый для входа в систему. Это поле заполняется при входе с использованием приложения.|
