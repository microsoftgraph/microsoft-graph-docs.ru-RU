...
* User.Read — используется для отправки общих вызовов API профилей. Пример: `https://graph.microsoft.com/beta/me/profile/`. Для использования разрешения User.Read требуется согласие пользователя.

## <a name="view-my-profile"></a>Просмотр моего профиля

С помощью описанных в этом разделе запросов можно получить данные о людях, наиболее релевантных для пользователя, выполнившего вход в систему (`/me`). Для выполнения этих запросов у вошедшего пользователя должно быть разрешение User.Read. 
