...
Представляет учетную запись Microsoft Azure Active Directory (Azure AD). Наследуется от [directoryObject](directoryobject.md).
...
| country | String | Страна или регион, в котором находится пользователь, например `US` или `UK`. Максимальная длина составляет 128 символов. <br><br>Поддерживает `$filter` (`eq`, `ne`, `NOT`, `ge`, `le`, `in`, `startsWith`). |
...
| hireDate | DateTimeOffset | Дата найма пользователя. Тип Timestamp представляет сведения о времени и дате с использованием формата ISO 8601 и формата времени UTC. Например, полночь 1 января 2014 г. в формате UTC представляется в виде `2014-01-01T00:00:00Z`. <br><br> Возвращается только с помощью оператора `$select`. <br> **Примечание.** Свойство относится к Microsoft Office SharePoint Online. Рекомендуем использовать собственное свойство **employeeHireDate**, чтобы устанавливать и обновлять значения даты найма с помощью API Microsoft Graph. |
...
| lastPasswordChangeDateTime | DateTimeOffset | Время последней смены пароля пользователем Azure AD. Тип Timestamp представляет сведения о времени и дате с использованием формата ISO 8601 и формата времени UTC. Например, полночь 1 января 2014 г. в формате UTC представляется в виде `2014-01-01T00:00:00Z`. Только для чтения. <br><br>Возвращается только с помощью оператора `$select`.  |
...
| userType | Строка | Строковое значение, с помощью которого можно классифицировать типы пользователей в каталоге, например `Member` и `Guest`. <br><br>Поддерживает `$filter` (`eq`, `ne`, `NOT`, `in`). |
