...
| responsibilities | Коллекция String | Список обязанностей пользователя. <br><br>Возвращается только с помощью оператора $select. |
...
| state | Строка | Область, республика, край или округ в адресе пользователя. <br><br>Возвращается только с помощью оператора $select. Поддерживает параметр $filter. |
...
| userType | String | Строковое значение, с помощью которого можно классифицировать типы пользователей в каталоге, например "Участник" и "Гость". <br><br>Возвращается только с помощью оператора $select. Поддерживает параметр $filter. |
...
В дополнение к **mailboxSettings**, **пользователь** включает связь с помощью [усерсеттингс](../resources/usersettings.md) к [регионаландлангуажесеттингс](../resources/regionalandlanguagesettings.md), подмножество параметров языка и региональных форматов, которые могут использоваться любым приложением для предоставления пользователю лучшего языка и региональных форматов. Используйте **усерсеттингс** для согласованного взаимодействия между приложениями, которые применяют профиль пользователя Azure AD для отображения одних и тех же пользовательских параметров.
