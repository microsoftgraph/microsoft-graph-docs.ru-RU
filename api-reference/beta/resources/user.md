...
| [activateServicePlan](../api/user-activateserviceplan.md) (упразднено) | Нет | Активация службы с заданными `servicePlanId` и `skuId` для определенного [пользователя](user.md). |
| **Drive** |||
...
> [!TIP]
> Расширения каталогов и связанные с ними данные возвращаются по умолчанию; расширения схемы и связанные с ними данные возвращаются только в `$select`; а открытые расширения и связанные данные возвращаются только в `$expand`.
...
|extensions|Коллекция [extension](extension.md)|Коллекция открытых расширений, определенных для пользователя. Поддерживает `$expand`. Допускается значение null.|
