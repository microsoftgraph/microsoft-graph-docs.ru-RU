...
| pastProjects | Коллекция объектов string | Список предыдущих проектов пользователя. <br><br>Возвращается только с помощью оператора $select. |
...
| responsibilities | Коллекция объектов string | Список обязанностей пользователя. <br><br>Возвращается только с помощью оператора $select. |
...
|events|Коллекция [event](event.md)|События пользователя. По умолчанию отображаются события в стандартном календаре. Только для чтения. Может иметь значение null.|
