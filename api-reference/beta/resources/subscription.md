...
|:---------|:-----|:------------|
| changeType | строка | Указывает тип изменения в ресурсе, на который оформлена подписка и при возникновении которого будет создано уведомление. Поддерживаемые значения: `created`, `updated`, `deleted`. Вы можете объединить несколько значений, указав их в списке с разделителями-запятыми. Обязательный. <br><br>Примечание. Уведомления о корневых элементах диска поддерживают только `updated` changeType. Уведомления о пользователе и группе поддерживают `updated` и `deleted` changeType. |
| notificationUrl | строка | URL-адрес конечной точки, принимающей уведомления. Этот URL-адрес должен использовать протокол HTTPS. Обязательный. |
...
| resource | string | Указывает ресурс, для которого будут отслеживаться изменения. Не включайте базовый URL-адрес (`https://graph.microsoft.com/beta/`). Обязательный. |
...
| latestSupportedTlsVersion | Строка | Задает последнюю версию протокола TLS, поддерживаемую конечной точкой уведомления, заданной параметром **notificationUrl**. Допустимые значения: `v1_0`, `v1_1`, `v1_2`, `v1_3`. </br></br>Для подписчиков, чьи конечная точка уведомлений поддерживает версию ниже рекомендуемой версии (TLS 1,2), задание этого свойства с помощью настройки [временной шкалы](https://developer.microsoft.com/graph/blogs/microsoft-graph-subscriptions-deprecating-tls-1-0-and-1-1/) позволяет им временно использовать устаревшую версию протокола TLS перед выполнением обновления до TLS 1,2. Для этих подписчиков, а не для задания этого свойства на временной шкале, произойдет ошибка при выполнении операций подписки. </br></br>Для подписчиков, конечная точка уведомлений которых уже поддерживает TLS 1,2, задание этого свойства является необязательным. В таких случаях Microsoft Graph по умолчанию имеет значение `v1_2`. |
