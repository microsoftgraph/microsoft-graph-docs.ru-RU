...
Выражения путей к ресурсам, поддерживаемые для каждого ресурса, которые можно использовать в свойстве **ресурса** подписки, задокументированы в [статье Обзор](webhooks.md).
...
| resource | string | Указывает ресурс, для которого будут отслеживаться изменения. Не включайте базовый URL-адрес (`https://graph.microsoft.com/beta/`). Обязательное. |
