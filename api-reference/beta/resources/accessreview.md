...
Тип ресурса **ауторевиевсеттингс** внедряется в параметры проверки доступа, а вместе с **ауторевиевенаблед**указывает поведение компонента при завершении проверки доступа. У ресурса есть одно свойство **нотревиеведресулт**. Если вы не хотите, чтобы решение по проверке было записано, пока проверяющий не выйдет явный выбор, задайте для **ауторевиевенаблед** значение false. Если вы хотите, чтобы система предоставляла решение, даже если проверяющий не сделает выбор, установите **ауторевиевенаблед** в `true` и включите **ауторевиевсеттингс** с помощью свойства **нотревиеведресулт** . После завершения анализа в зависимости от свойства **нотревиеведресулт** решение будет записано как `Approve` в, так и. `Deny`   

| Свойство                     | Тип     | Описание                          |
| :--------------------------- | :------  | :----------                          |
| `notReviewedResult`          |`String`  | Должно иметь один из типов `Approve`, `Deny` или `Recommendation`.  `accessRecommendationsEnabled` В этом случае `Recommendation`необходимо также задать значение true. |
