...
| Связь | Тип   |Описание|
|:---------------|:--------|:----------|
| `reviewers`               |Коллекция [userIdentity](useridentity.md)                     | Коллекция проверяющих для проверки доступа, если тип проверки доступа Ревиевертипе имеет значение `delegated` . |
...
Тип ресурса **акцессревиевсеттингс** предоставляет дополнительные параметры при создании проверки доступа, чтобы управлять поведением компонента при запуске проверки доступа. Этот тип имеет следующие свойства. 
...
Тип ресурса **ауторевиевсеттингс** внедряется в параметры проверки доступа, а вместе с **ауторевиевенаблед**указывает поведение компонента при завершении проверки доступа. У ресурса есть одно свойство **нотревиеведресулт**. Если вы не хотите, чтобы решение по проверке было записано, пока проверяющий не выйдет явный выбор, задайте для **ауторевиевенаблед** значение false. Если вы хотите, чтобы система предоставляла решение, даже если проверяющий не сделает выбор, установите **ауторевиевенаблед** в `true` и включите **Ауторевиевсеттингс** с помощью свойства **нотревиеведресулт** . После завершения анализа в зависимости от свойства **нотревиеведресулт** решение будет записано как в, `Approve` так и `Deny` .   

| Свойство                     | Тип     | Описание                          |
| :--------------------------- | :------  | :----------                          |
| `notReviewedResult`          |`String`  | Должно иметь один из типов `Approve`, `Deny` или `Recommendation`.  `Recommendation` `accessRecommendationsEnabled` В этом случае необходимо также задать значение true. |


## <a name="accessreviewrecurrencesettings-resource-type"></a>Тип ресурса Акцессревиеврекурренцесеттингс

Тип ресурса **акцессревиеврекурренцесеттингс** внедряется в параметрах проверки доступа и указывает, что проверка доступа повторяется через регулярные промежутки времени.  Этот тип имеет следующие свойства.

| Свойство                     | Тип                                                                                                          | Описание |
| :--------------------------- | :------------------------------------------------------------------------------------------------------------ | :---------- |
| `recurrenceType`|`String`    | Интервал повторения, который должен быть одним из:,,, `onetime` `weekly` `monthly` `quarterly` , "халфеарли" или `annual` .                                                                   |
