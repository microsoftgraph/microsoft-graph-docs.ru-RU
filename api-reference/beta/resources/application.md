...
|id|String|Уникальный идентификатор для приложения. Наследуется от [directoryObject](directoryobject.md). Ключ. Значение null не допускается. Только для чтения. |
...
|publisherDomain| String | Домен подтвержденным publisher для приложения. Только для чтения.|
