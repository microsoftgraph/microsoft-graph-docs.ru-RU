...
| подписки      | Коллекция [подписки][] | Набор подписок для элемента. Поддерживается только в корневой папке диска.
...
URL-адрес будет доступен только в течение короткого периода времени (1 час) до того, как он станет недействительным.
Удаление разрешений на доступ к файлам для пользователя может не сразу сделать этот URL-адрес недействительным.
