...
|displayName         |строка  |Задает описание элемента.                 |
|type                |числовых |Тип запасов для элемента. 1 = складская номенклатура, 2 = сервисный товар. Это обязательное свойство.|
|заблокировано             |boolean |Указывает, что транзакции с элементом не могут быть опубликованы, например, потому что элемент находится в карантине. Имеет значение **true**, если элемент блокируется.|
