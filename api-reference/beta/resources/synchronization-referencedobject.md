...
|:---------------------------|:--------------------------|:---------------|
|референцедобжектнаме        |Строка                     |Имя объекта, на который указывает ссылка. Должен сопоставлять один из объектов в [определении каталога](synchronization-directorydefinition.md).|
|референцедпроперти          |Строка                     |**В настоящее время не поддерживается**. Имя свойства в объекте, на который указывает ссылка, значение, которое используется в качестве ссылки.|
