...
- **Microsoft Defender для конечной точки** — поддерживает следующие методы **tiIndicators:**
  ...
  > [!NOTE]
  >Следующие типы индикаторов поддерживаются Microsoft Defender для targetProduct конечных точек:
  > - Files
  > - IP-адреса: Microsoft Defender для конечной точки поддерживает только IPv4/IPv6 — задает свойство в свойствах networkDestinationIPv4 или networkDestinationIPv6 в microsoft Graph tiPi API **tiIndicator**.
  > - URL-адреса/домены

   Существует ограничение в 15000 индикаторов для клиента для Microsoft Defender для конечной точки.
...
|:-------------|:------------|:------------|
|action|string| Действие, применяемая, если индикатор соответствует из средства безопасности targetProduct. Возможные значения: `unknown`, `allow`, `block`, `alert`. **Обязательно.**|
...
|azureTenantId|String| Штампуется системой при прохохо-стике индикатора. ID Azure Active Directory клиента отправки клиента. **Обязательно.**|
...
|description|Строка| Краткое описание (100 символов или менее) угрозы, представленной индикатором. **Обязательно.**|
...
|id|Строка|Создается системой при прохо-мыве индикатора. Созданный GUID/уникальный идентификатор. Только для чтения.|
...
|targetProduct|Строка|Строковая величина, представляющая один продукт безопасности, к которому должен применяться индикатор. Допустимые значения: `Azure Sentinel` , `Microsoft Defender ATP` . **Required**|
...
|tlpLevel|[tlpLevel](#tlplevel-values)| Значение Протокола светофора для индикатора. Возможные значения: `unknown`, `white`, `green`, `amber`, `red`. **Обязательно.**|

### <a name="indicator-observables---email"></a>Наблюдаемые индикаторы — электронная почта

| Свойство     | Тип        | Описание |
|:-------------|:------------|:------------|
|emailEncoding|Строка|Тип коди-кодии текста, используемый в электронной почте.|
|emailLanguage|Строка|Язык электронной почты.|
|emailRecipient|Строка|Адрес электронной почты получателя.|
|emailSenderAddress|Строка|Адрес электронной почты злоумышленника&#124;жертвы.|
|emailSenderName|Строка|Отображается имя злоумышленника&#124;жертвы.|
|emailSourceDomain|Строка|Домен, используемый в электронной почте.|
|emailSourceIpAddress|Строка|Исходный IP-адрес электронной почты.|
|emailSubject|Строка|Тема строки электронной почты.|
|emailXMailer|Строка|Значение X-Mailer, используемого в электронной почте.|
...
|fileHashValue|Строка| Значение hash файла.|
|fileMutexName|Строка| Имя Mutex, используемого в обнаружениях на основе файлов.|
|fileName|String|Имя файла, если индикатор основан на файле. Несколько имен файлов могут быть делимитированы запятой. |
|filePacker|Строка|Упаковщик, используемый для создания файла, о чем идет речь.|
|filePath|Строка|Путь файла, указывающий компромисс. Может быть Windows или *nix.|
|fileSize|Int64|Размер файла в bytes.|
|fileType|Строка| Текстовое описание типа файла. Например, "Word Document" или "Binary".|
...
|networkCidrBlock|Строка| Представление нотации ciDR Block для сети, ссылаясь на этот индикатор. Используйте только в том случае, если источник и пункт назначения не могут быть определены. |
|networkDestinationAsn|Int32|Идентификатор автономной системы назначения сети, на который ссылается индикатор.|
|networkDestinationCidrBlock|Строка|Представление нотации ciDR Block в сети назначения в этом индикаторе.|
|networkDestinationIPv4|Строка|Назначение IP-адресов IPv4.|
|networkDestinationIPv6|Строка|Назначение IP-адресов IPv6.|
|networkDestinationPort|Int32|Пункт назначения порта TCP.|
|networkIPv4|Строка| IP-адрес IPv4. Используйте только в том случае, если источник и пункт назначения не могут быть определены. |
|networkIPv6|Строка| IP-адрес IPv6. Используйте только в том случае, если источник и пункт назначения не могут быть определены. |
...
|networkSourceCidrBlock|Строка|Представление нотации блока CIDR для исходных сетей в этом индикаторе|
|networkSourceIPv4|Строка|Источник IP-адресов IPv4.|
|networkSourceIPv6|Строка|Источник IP-адресов IPv6.|
...
|userAgent|Строка|User-Agent строка из веб-запроса, которая может указывать на компромисс.|
...
| Member | Описание |
|:-------|:------------|
|Действия|Сообщается, что злоумышленник использует скомпрометированную систему для действий, таких как распределенная атака отказа в обслуживании.|
...
|Зеленый| Область общего доступа: Community. Индикаторы могут быть общими для сообщества безопасности.|
