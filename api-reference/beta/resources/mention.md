...
|:---------------|:--------|:----------|
|application | String | Имя приложения, в котором создается упоминание. Необязательное свойство. Не используется и по умолчанию задано значение NULL для **сообщения**. |
|клиентреференце | String | Уникальный идентификатор, представляющий родительский объект для экземпляра ресурса. Необязательное свойство. Не используется и по умолчанию задано значение NULL для **сообщения**. |
