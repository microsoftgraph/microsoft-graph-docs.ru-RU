...
|azureTenantId |String|Идентификатор клиента Azure Active Directory. Обязательное. |
...
|createdDateTime |DateTimeOffset|Время создания оповещения поставщиком оповещения. Тип Timestamp представляет сведения о времени и дате с использованием формата ISO 8601 (всегда применяется формат UTC). Например, значение полуночи 1 января 2014 г. в формате UTC выглядит так: `'2014-01-01T00:00:00Z'`. Обязательное.|
...
|severity |alertSeverity|Серьезность оповещения, заданная поставщиком. Возможные значения: `unknown`, `informational`, `low`, `medium`, `high`. Обязательное.|
...
|status |alertStatus|Оповещение о состоянии жизненного цикла (этапа). Возможные значения: `unknown`, `newAlert`, `inProgress`, `resolved`. (Поддерживает [обновление](../api/alert-update.md)). Обязательное.|
...
|title |String|Заголовок оповещения. Обязательное.|
