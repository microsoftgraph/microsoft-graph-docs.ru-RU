...
|azureTenantId |String|Идентификатор Azure Active Directory клиента. Обязательный атрибут. |
...
|описание|String|Описание оповещения.|
...
|id |String|GUID/уникальный идентификатор, созданный поставщика. Только для чтения. Обязательный атрибут.|
...
|уровень серьезности |alertSeverity|Серьезность оповещения — задание с поставщиком/поставщика. Возможные значения: `unknown`, `informational`, `low`, `medium`, `high`. Обязательный атрибут.|
...
|status |alertStatus|Состояние оповещений жизненного цикла (рабочей области). Возможные значения: `unknown`, `newAlert`, `inProgress`, `resolved`. (поддерживает [обновление](../api/alert-update.md)). Обязательный атрибут.|
|теги|Коллекция String|Определяемые пользователем меток, которые могут применяться к оповещения и может выступать в качестве условий фильтра (например «HVA», «ЗАФИКСИРОВАЛА», и т.д.) (поддерживает [обновление](../api/alert-update.md)).|
|title |String|Название оповещения. Обязательный атрибут.|
...
|vendorInformation |[securityVendorInformation](securityvendorinformation.md)|Сложный тип, содержащий сведения о безопасности продуктов и услуг поставщика, поставщик и subprovider (например, поставщика = Майкрософт; поставщика = ATP Защитник Windows; subProvider = AppLocker). Обязательный атрибут.|
