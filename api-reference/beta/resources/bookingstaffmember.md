...
|:---------------|:--------|:----------|
|availabilityIsAffectedByPersonalCalendar|Логическое|True означает, что если сотрудник является пользователем Microsoft 365, API Bookings проверяет наличие сотрудника в личном календаре в Microsoft 365, прежде чем сделать заказ. |
...
|role|string| Роль сотрудника в бизнесе. Возможные значения: `guest` `administrator` , , , , , `viewer` `externalGuest` и `scheduler` `member` . Обязательный.|
