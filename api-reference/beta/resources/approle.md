...
|:---------------|:--------|:----------|
|алловедмембертипес|Коллекция String|Указывает, может ли эта роль приложения назначаться пользователям и группам (с помощью параметра `["User"]` ), другим приложениям (по значению `["Application"]` или обоим `["User", "Application"]` ). Роли приложений, поддерживающие назначение для других субъектов служб приложений, также называются [разрешениями приложений](/graph/auth/auth-concepts#microsoft-graph-permissions). Значение Application поддерживается только для ролей приложения, определенных для сущностей **приложения** . |
|description|Строка|Описание роли приложения. Он отображается при назначении роли приложения и, если роль приложения работает как разрешение приложения, во время предоставления согласия.|
