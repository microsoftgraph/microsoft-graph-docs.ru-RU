...
|findMyFriendsBlocked|Boolean|Указывает, следует ли блокировать изменения в поиске друзей, когда устройство находится в режиме надзора.|
...
|kioskModeAllowAutoLock|Boolean|Указывает, следует ли запретить автоблокировку устройства в режиме киоска. Функциональные возможности этого свойства избыточны при стандарте ОС и неподготовлены. Используйте KioskModeBlockAutoLock.|
...
|kioskModeBuiltInAppId|String|ИД встроенных приложений для использования в режиме терминала. Используется, когда kioskModeManagedAppId и KioskModeAppStoreUrl не заданы.|
...
|softwareUpdatesEnforcedDelayInDays|Int32|Задает количество дней, в течение которое обновление программного обеспечения будет оторолироваться для контролируемого устройства. Допустимые значения: от 0 до 90.|
...
|pkiBlockOTAUpdates|Boolean|Указывает, блокируются ли обновления PKI, перенанося через нее. Установка для этого ограничения false не отключает проверки CRL и OCSP (iOS 7.0 и более поздних).|
...
|airPrintBlocked|Boolean|Указывает, заблокирована ли airPrint (iOS 11.0 и более поздних).|
