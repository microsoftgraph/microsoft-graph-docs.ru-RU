...
|useDeviceContext|Boolean|Указывает, следует ли устанавливать MSI с двойным режимом в контексте устройства. Если значение равно true, приложение будет установлено для всех пользователей. Если задано значение false, приложение будет установлено для каждого пользователя. Если значение равно NULL, служба будет использовать контекст установки пакета MSI по умолчанию. В случае использования MSI с двойным режимом значение по умолчанию будет применяться для каждого пользователя.  Не может быть задано для приложений, отличных от двух режимов.  Невозможно изменить после первоначального создания приложения.|
