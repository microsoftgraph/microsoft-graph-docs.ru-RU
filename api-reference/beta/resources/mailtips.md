...
| customMailTip | Строка | Пользовательская подсказка, которую можно настроить в почтовом ящике получателя. |
...
| recipientScope | Строка | Область получателя. Возможные значения: `none`, `internal`, `external`, `externalPartner`, `externalNonParther`. Например, администратор может сделать другую организацию ее "партнером". Область полезна, если администратор хочет, чтобы определенные почтовые советы были доступны для определенных областей. Кроме того, отправителям полезно сообщить им, что их сообщение может покинуть организацию, помогая принимать правильные решения о формулировках, тонах и содержимом.|
