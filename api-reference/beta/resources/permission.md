...
**grantedTo** и **grantedToIdentities** будут обесцениться в будущем, а ответ будет перенесен на **grantedToV2** и **grantedToIdentitiesV2** соответственно под соответствующими именами свойств.
...
| grantedToV2                      | [SharePointIdentitySet][]                 | Для разрешений типа пользователя сведения о пользователях и приложениях для этого разрешения. Только для чтения. |
| grantedToIdentitiesV2            | [Collection(SharePointIdentitySet)][]     | Для разрешений типа link: сведения о пользователях, которым предоставлено разрешение. Только для чтения. |
...
| roles                            | Collection(String)                        | Тип разрешения, `read` например. Полный список ролей см. ниже. Только для чтения. |
...
| hasPassword                      | Логическое                                   | Указывает, установлен ли пароль для этого разрешения. Это свойство отображается только в ответе. Необязательно. Только для чтения. Только OneDrive personal. |
| grantedTo (отстает)           | [IdentitySet](identityset.md)             | Для разрешений типа пользователя сведения о пользователях и приложениях для этого разрешения. Только для чтения. |
| grantedToIdentities (обесценив) | Collection([IdentitySet](identityset.md)) | Для разрешений типа сведения о пользователях, которым было предоставлено разрешение. Только для чтения. |
