...
|подединица|Коллекция объектов string|Иерархия подразделений, в которой расположен принтер. Элементы должны находиться в иерархическом порядке. Например, если Кампус делится на несколько разделов, иерархия может выглядеть следующим образом:`["East Wing", "Block A"]`|
...
|румдескриптион|String|Описание комнаты, в которой находится принтер.|
|organization;|Коллекция объектов string|Организационная иерархия, к которой относится принтер. Элементы должны находиться в иерархическом порядке.|
|подразделение|Коллекция объектов string|Подразделение, в котором находится принтер. Элементы должны находиться в иерархическом порядке.|
|stateOrProvince|String|Область или край, в котором находится принтер.|
