...
Если на странице есть изображение, подходящее для предварительного просмотра, то свойство **href** объекта **previewImageUrl** будет содержать ссылку на общедоступный [ресурс изображения](#image-or-other-file-resource). Эту ссылку можно использовать в коде HTML. В противном случае **href** возвращает значение null.
...
Получение имени и ссылки **pagesUrl** для всех разделов, имя которых содержит термин *spring*. Сортировка разделов по дате последнего изменения.
...
Получение первых 20 страниц, отсортированных по свойству **createdByAppId** и последнему времени создания. По умолчанию API возвращает 20 записей.
...
| orderby | <p>`orderby=title,createdTime desc`</p><p>
            [Свойства](#onenote-entity-properties) для сортировки с необязательным порядком сортировки **asc** (по умолчанию) или **desc**. Вы можете сортировать по любому свойству сущности в запрошенной коллекции.</p><p>По умолчанию для записных книжек, разделов и групп разделов используется порядок сортировки `name asc`, а для страниц — `lastModifiedTime desc` (сначала отображается последняя измененная страница).</p><p>Разделяйте несколько свойств запятыми и перечисляйте их в порядке применения. Имена свойств чувствительны к регистру.</p> |  
| search | <p>`search=cell div`</p><p>Доступен только для пользовательских записных книжек.</p><p>Термин или фраза, которую нужно найти в заголовке и тексте страницы, замещающем тексте изображений и распознанном тексте. По умолчанию поисковые запросы возвращают результаты, отсортированные по релевантности.</p><p>OneNote использует полнотекстовые поиск Bing для поддержки поиска фраз, выделения корней, допуска правописания, релевантности и ранжирования, разбиения слов, нескольких языков и других функций полнотекстового поиска. Строки поиска нечувствительны к регистру.</p><p>Применяется только к страницам записных книжек, принадлежащих пользователю. Индексированное содержимое является личным и доступно только владельцу. Страницы, защищенные паролем, не индексируются. Применяется только к конечной точке `pages`.</p> |  
...
| skip | <p>`skip=10`</p><p>Количество пропускаемых записей в наборе результатов. Обычно используется для деления результатов на страницы.</p> |  
| top | <p>`top=50`</p><p>Количество возвращаемых записей в наборе результатов, до 100. Значение по умолчанию — 20.</p> |  
