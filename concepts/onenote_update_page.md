...
Отправьте объект изменений JSON в тексте сообщения. Если запрос выполнен успешно, Microsoft Graph возвращает код состояния HTTP 204.
...
**Получение HTML-кода страницы и всех определенных значений *data-id***</p>
`../pages/{id}/content`   
...
Узнайте больше о [корневом URL-адресе службы](../api-reference/v1.0/resources/onenote-api-overview.md#root-url).
...
|------|------|  
| #{data-id} | <p>Этот идентификатор при желании определяется для элементов входного HTML-кода при [создании](onenote-create-page.md) или [обновлении страницы](onenote_update_page.md). Добавьте перед этим значением префикс #.</p><p> Пример: `'target':'#intro'` указывает на элемент `<div data-id="intro" ...>`</p> |  
| id | <p>Это [сгенерированный идентификатор](#generated-ids) из Microsoft Graph, необходимый для большинства операций замены. Не добавляйте перед ним префикс #.</p><p> Пример: `'target':'div:{33f8a2...}{37}'` указывает на элемент `<div id="div:{33f8a2...}{37}" ...>`</p><p>Не следует путать эти идентификаторы со значениями **id**, определенными во [входном HTML-коде](onenote_input_output_html.md). Все значения **id**, отправляемые во входном HTML-коде, отклоняются.</p> |  
...
**action**  
Действие, которое требуется выполнить с целевым элементом. См. [поддерживаемые действия для элементов](#supported-elements-and-actions).
...
**position**  
Расположение относительно целевого элемента, куда нужно добавить указанное содержимое. Если этот атрибут не указан, по умолчанию используется значение **after**.
...
> **Примечание.** API отклоняет все значения **id**, определенные во [входном HTML-коде](onenote_input_output_html.md) запросов create-page и update-page.

Ниже показаны сгенерированные идентификаторы для абзаца и изображения в [выходном HTML-коде](onenote_input_output_html.md) страницы.
...
| div<br /> (внутри разделителя) | **да** (только идентификатор) | **да** | **да** |   
...
[Добавление дочерних элементов](#append-child-elements)&nbsp;&nbsp;|&nbsp;&nbsp;[Вставка элементов того же уровня](#insert-sibling-elements)&nbsp;&nbsp;|&nbsp;&nbsp;[Замена элементов](#replace-elements)&nbsp;&nbsp;|&nbsp;&nbsp;[Выполнение запросов PATCH](#complete-patch-request-examples)


<a name="append-examples"></a>

### <a name="append-child-elements"></a>Добавление дочерних элементов
Действие **append** добавляет дочерний элемент к элементу **body**, **div** (внутри разделителя), **ol** или **ul**. Атрибут **position** определяет, где следует добавлять дочерний элемент: до или после целевого элемента. По умолчанию задано положение **after**.
...
В приведенном ниже примере элемент списка добавляется в качестве последнего дочернего элемента целевого списка. В нем определяется свойство **list-style-type**, так как для элемента используется нестандартный стиль.
...
В приведенном ниже примере на страницу добавляется два узла того же уровня. Над элементом **para1** добавляется изображение, а под элементом **para2** — абзац.
...
В этом примере показано, как обновить таблицу, используя ее сгенерированный идентификатор. Замена элементов **tr** и **td** не поддерживается, но вы можете заменить таблицу целиком.
...
| Заголовок Content-Type | <p>`application/json` для массива объектов изменений JSON, отправленного либо непосредственно в тексте сообщения, либо в обязательной части Commands [составных запросов](#multipart-request-with-binary-content).</p><p>Составные запросы необходимы при отправке двоичных данных. В них используется тип контента `multipart/form-data; boundary=part-boundary`, где *{part-boundary}*  — это строка, обозначающая начала и конец каждой части данных.</p> |  
 

| Данные ответа | Описание |  
|------|------|  
| Код успешного завершения | Код состояния HTTP 204. Данные JSON не возвращаются по PATCH-запросу. |  
| Ошибки | В статье [Коды ошибок для API OneNote в Microsoft Graph](onenote_error_codes.md) описываются ошибки OneNote, которые может возвращать Microsoft Graph. |  
...
- Значение `users/{id}` предназначено для содержимого OneNote, которым указанный (в URL-адресе) пользователь поделился с текущим пользователем. Использование [API Graph для Azure AD](https://msdn.microsoft.com/library/azure/ad/graph/api/api-catalog)
...
Дополнительную информацию о разрешениях и принципе их работы см. в разделе [Разрешения OneNote](permissions_reference.md).
