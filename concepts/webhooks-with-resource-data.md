...
В настоящее время Microsoft Teams [chatMessage](/graph/api/resources/chatmessage), а также  ресурсы [presence](/graph/api/resources/presence) Microsoft Teams поддерживают уведомления об изменении, которые включают данные ресурсов. В частности, вы можете настроить подписку, относящуюся к одному из следующих элементов:
...
Дополнительные сведения о том, как получать уведомления в жизненном цикле (предварительная версия), а также отвечать на них, см. в статье [Уменьшение числа пропущенных подписок и уведомлений об изменениях (предварительная версия)](webhooks-lifecycle.md)
