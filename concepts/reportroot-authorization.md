...
---
# <a name="authorization-for-apis-to-read-office-365-usage-reports"></a>Авторизация для API с целью чтения отчетов об использовании Office 365

Данные отчетов, доступные через API отчетов Microsoft Graph, являются конфиденциальными. В частности, отчеты об использовании Office 365 защищаются как разрешениями, так и ролями Azure Active Directory (Azure AD). Сведения этой статьи относятся к API-интерфейсу отчетов, считывающему отчеты об использовании Office 365.

API для чтения отчетов об использовании Office 365 поддерживают два типа авторизации:
