...
2.  Microsoft Graph отправит отклик, содержащий нужный ресурс и [маркер состояния](#state-tokens).
...
Объекты @removed могут возвращаться в отклике на исходный разностный запрос и в отслеживаемых откликах (deltaLink). Клиенты, использующие разностные запросы, должны иметь возможность обрабатывать эти объекты в откликах.
...
- [Получение добавочных изменений событий в представлении календаря](../Concepts/delta_query_events.md)
