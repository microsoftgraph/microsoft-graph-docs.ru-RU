...
Дельта-запрос позволяет запрашивать добавления, удаления или обновления сообщений в папке с помощью серии вызовов функции [дельта](/graph/api/message-delta). Дельта-данные позволяют поддерживать и синхронизировать локальное хранилище сообщений пользователя без необходимости каждый раз получать весь набор сообщений пользователя с сервера.
...
Посмотрите этот [пример](#example-synchronize-messages-in-a-folder), чтобы узнать, как использовать URL-адреса `@odata.nextLink` и `@odata.deltaLink`.
...
## <a name="example-synchronize-messages-in-a-folder"></a>Пример синхронизации сообщений в папке
