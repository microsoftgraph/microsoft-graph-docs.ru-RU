...
С этим разрешением нет признаков того, что сообщение было отправлено от имени другого пользователя. Свойства `sender` и `from` имеют одинаковое значение.
...
В этом примере Александр Воронов пытается отправить электронное письмо от Полины Кольцовой, но емуне предоставлено разрешение **Отправить от имени** или **Отправить как**. Ответ содержит ошибку `ErrorSendAsDenied`.
