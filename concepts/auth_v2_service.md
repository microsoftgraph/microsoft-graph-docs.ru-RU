# <a name="get-access-without-a-user"></a>Получение доступа без вмешательства пользователя
Некоторые приложения работают на сервере в случаях, когда вход пользователя не предусмотрен. Такие приложения часто называют фоновыми службами или управляющими программами. Примером таких приложений можно назвать службу архивации электронной почты, которая выходит из спящего режима и работает в течение ночи. Как правило, фоновые службы используют поток предоставления учетных записей клиента OAuth 2.0 для получения маркеров доступа из Azure AD. В этой статье рассматриваются основные действия по настройке фоновой службы и использованию потока предоставления учетных данных клиента OAuth для получения маркера доступа из Azure AD для вызова Microsoft Graph. 

## <a name="authentication-and-authorization-steps"></a>Этапы проверки подлинности и авторизации
Ниже перечислены основные действия, необходимые для проверки подлинности фоновой службы и получения маркера из конечной точки Azure AD версии 2.0 для вызова Microsoft Graph.
...
На приведенном ниже рисунке показана регистрация веб-приложения, настроенного для фоновой службы. ![Регистрация приложения-службы](./images/v2-service-registration.png)

В случае фоновой службы необходимо зарегистрировать приложение для платформы "Веб" и скопировать следующие значения:
...
- URL-адрес перенаправления, с помощью которого служба будет получать отклики с маркерами от Azure AD;
...
Сведения о том, как настроить приложение на портале Майкрософт для регистрации приложений, см. в статье [Регистрация приложения](./auth_register_app_v2.md).

При использовании потока предоставления учетных данных клиента OAuth 2.0 приложение проходит проверку подлинности непосредственно в конечной точке `/token` Azure AD версии 2.0, используя идентификатор приложения, назначенный службой Azure AD, и секрет приложения, созданный вами на портале. 

## <a name="2-configure-permissions-for-microsoft-graph"></a>2. Настройка разрешений для Microsoft Graph
Для приложений, работающих без пользователя, Microsoft Graph предоставляет разрешения приложений. Вы настраиваете эти разрешения заранее во время регистрации приложения. Разрешения приложений всегда требуют согласия администратора. Администратор может согласиться предоставить эти разрешения на [портале Azure](https://portal.azure.com) во время установки приложения в организации. Вы также можете создать в приложении интерфейс регистрации, где администраторы могут соглашаться на предоставление заданных вами разрешений. Когда согласие администратора фиксируется в Azure AD, приложение может запрашивать маркеры, не требуя повторного согласия. Дополнительные сведения о разрешениях, доступных в Microsoft Graph, см. в [справочнике по разрешениям](./permissions_reference.md).

Чтобы настроить разрешения приложения на [портале Майкрософт для регистрации приложений](https://apps.dev.microsoft.com/), в разделе **Разрешения Microsoft Graph** нажмите кнопку **Добавить** рядом с заголовком **Разрешения приложений**, а затем выберите разрешения, необходимые вашему приложению, в диалоговом окне **Выбор разрешения**.

На приведенном ниже снимке экрана показано диалоговое окно **Выбор разрешения** для разрешений приложений в Microsoft Graph. 

![Диалоговое окно "Выбор разрешения" для разрешений приложений в Microsoft Graph.](./images/v2-application-permissions.png)

> **Важно!** Рекомендуем настроить минимальный набор разрешений, необходимый приложению. Это намного удобнее для администраторов, чем необходимость соглашаться на длинный список разрешений.
>
...
| redirect_uri |Обязательный |URI перенаправления, на который должен отправляться отклик для обработки приложением. Он должен в точности совпадать с одним из URI перенаправления, зарегистрированных на портале, и быть закодирован как URL-адрес. Может содержать дополнительные сегменты пути. |
| state |Рекомендуемый |Значение, которое включается в запрос и возвращается в отклике с маркером. Это может быть строка с любым содержимым. Параметр state используется для кодирования сведений о состоянии пользователя в приложении до запрашивания проверки подлинности, например о просматриваемых странице и представлении. |

### <a name="consent-experience"></a>Предоставление согласия
В Azure AD только администратор клиента может войти для выполнения запроса. Администратору будет предложено утвердить все прямые разрешения приложения, указанные вами на портале регистрации приложений. Ниже представлен пример диалогового окна согласия, которое Azure AD представляет администратору:

![Диалоговое окно согласия администратора.](./images/admin-consent.png)

### <a name="response"></a>Отклик
Если администратор утверждает разрешения для приложения, успешный отклик выглядит так:
...
| state |Значение, которое включается в запрос и возвращается в отклике с маркером. Это может быть строка с любым содержимым. Параметр state используется для кодирования сведений о состоянии пользователя в приложении до запрашивания проверки подлинности, например о просматриваемых странице и представлении. |
| admin_consent |Задано значение **true**. |


> **Попробуйте сами.** Вы можете попробовать это самостоятельно, вставив приведенный ниже запрос в браузере. Если вы войдете от имени глобального администратора клиента Azure AD, откроется диалоговое окно согласия администратора для приложения. (Ваше приложение может быть не таким, как на представленном выше снимке экрана.)
> 
...
В потоке предоставления учетных данных клиента OAuth 2.0 вы используете значения идентификатора и секрета приложения, сохраненные во время его регистрации, чтобы запросить маркер доступа непосредственно из конечной точки `/token` Azure AD версии 2.0.
...
#### <a name="token-response"></a>Отклик с маркером
...
Фоновые службы работают на сервере в случаях, когда вход пользователя не предусмотрен, и используют поток предоставления учетных данных клиента OAuth 2.0 для проверки подлинности в Azure AD и получения маркера. В случае конечной точки версии 2.0 вы можете подробнее исследовать этот сценарий с помощью следующих ресурсов:
...
В случае конечной точки Azure AD вы можете подробнее исследовать этот сценарий с помощью следующих ресурсов:
