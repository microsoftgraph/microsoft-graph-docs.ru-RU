...
Сведения о регистрации приложений см. в статье [Регистрация приложения с помощью платформы удостоверений Майкрософт](auth-register-app-v2.md). 
...
- Device.Read — разрешает идентификацию списка устройств пользователя

- Device.Command — разрешает взаимодействие с устройством пользователя

- UserActivity.ReadWrite.CreatedByApp — разрешает подписку приложения на получение уведомлений

- Notifications.ReadWrite.CreatedByApp — разрешает доступ к уведомлениям и их доставку

- wns.connect — разрешает подключение к службе уведомлений Windows
