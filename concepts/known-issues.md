...
3. Идентичный — для всех отдельных запросов в свойстве **dependsOn** указана одна и та же зависимость. **Примечание**. Запросы, созданные с помощью этого шаблона, будут запускаться последовательно.
