...
### <a name="linux-based-devices-cant-be-updated-by-an-app-with-application-permissions"></a>Устройства на базе Linux не могут быть обновлены приложением с разрешениями приложения.

Когда приложение с разрешениями приложения пытается обновить какие-либо свойства объекта устройства, где свойство **operationSystem** имеет значение `linux`, кроме свойства **extensionAttributes**, API [обновления устройства](/graph/api/device-update) возвращает `400 Bad request`код ошибки с сообщением об ошибке «Свойства, отличные от ExtendedAttribute1..15, могут быть изменены только на устройствах Windows.". Используйте делегированные разрешения для обновления свойств устройств под управлением Linux.
...
Дополнительные сведения см. в разделе [Регулирование и пакетная обработка](/graph/throttling#throttling-and-batching).
...
### <a name="teamworkappsettings-permissions-are-not-visible-in-the-azure-portal"></a>Разрешения TeamworkAppSettings не отображаются на портале Azure
Разрешения TeamworkAppSettings.Read.All и TeamworkAppSettings.ReadWrite.All в настоящее время развертываются и могут не отображаться на портале Azure. Чтобы предоставить согласие на эти разрешения, используйте запрос авторизации следующим образом.
...
### <a name="encode-number--symbols-in-userprincipalname"></a>Кодируйте символ номера (#) в userPrincipalName

Параметр **userPrincipalName** гостевых пользователей, добавленных через Azure AD B2B, часто содержит символ номера (#). Использование `$filter` на **userPrincipalName**, содержащем символ #, например `GET /users?$filter=userPrincipalName eq 'AdeleV_contoso.com#EXT#@fabrikam.com'`, возвращает ошибку HTTP `400 Bad request`. Чтобы выполнить фильтрацию по **userPrincipalName**, закодируйте символ # в виде его эквивалента в UTF-8 (`%23`), например `GET /users?$filter=userPrincipalName eq 'AdeleV_contoso.com%23EXT%23@fabrikam.com'`.

### <a name="access-to-user-resources-is-delayed-after-creation"></a>Доступ к ресурсам пользователей задерживается после создания

Пользователей можно создавать мгновенно с помощью метода POST для объекта user. Для доступа к службам Microsoft 365 пользователю нужна соответствующая лицензия. Из-за распределенного характера службы файлы, сообщения и события этого пользователя станут доступны посредством API Microsoft Graph через 15 минут после назначения лицензии. В течение этого времени приложения будут получать ошибку HTTP `404 Not Found`.

### <a name="access-to-a-users-profile-photo-is-limited"></a>Доступ к фотографии профиля пользователя ограничен

1. Просмотреть и обновить фотографию профиля пользователя можно, только если у пользователя есть почтовый ящик. Если в таком случае не удастся прочитать или изменить фотографию, возникнет следующая ошибка:
    ...
    ``` 
2. Все фотографии, которые, *возможно*, были сохранены с использованием свойства **thumbnailPhoto** (с помощью API Azure AD Graph [не рекомендуется] или путем синхронизации с AD Connect), больше недоступны с помощью свойства **photo** ресурса [user](/graph/api/resources/user) в Microsoft Graph.
3. Управление фотографиями пользователей с помощью [ресурса profilePhoto](/graph/api/resources/profilephoto) из API Microsoft Graph в настоящее время не поддерживается в клиентах Azure AD B2C.
