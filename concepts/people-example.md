---
title: Использование API людей в Microsoft Graph для получения сведений о наиболее релевантных для вас людях
description: 'Приложения Microsoft Graph могут использовать API людей для получения сведений о наиболее релевантных для пользователя людях. '
ms.date: 4/9/2019
author: anthona
...
ms.locfileid: "60780473"
---
# <a name="use-the-people-api-in-microsoft-graph-to-get-information-about-the-people-most-relevant-to-you"></a>Использование API людей в Microsoft Graph для получения сведений о наиболее релевантных для вас людях
...
Особенно удобно использовать API людей в сценариях выбора людей при написании электронных писем, создании собраний и т. д. Например, вы можете использовать API людей в сценариях написания электронных писем.

## <a name="including-a-person-as-relevant-or-working-with"></a>Добавление человека в качестве релевантного или "сотрудника"
 
Чтобы человек был добавлен как релевантный или "сотрудник" для владельца профиля в Delve, чтобы он отображался в карточке профиля владельца или возвращался от API людей, между ним и владельцем профиля должна быть _общедоступная_ связь. На приведенном ниже рисунке показаны пользователь A, индекс связи с другими пользователями (пользователь Б) и общедоступный профиль с подмножеством связей пользователя.
...
Если владелец профиля связывается с кем-то, и между этими людьми _нет общедоступной связи_, например, подключение к организационной диаграмме или общая группа, факт данной связи _не будет виден_ другим.
...
## <a name="disabling-working-with"></a>Отключение "сотрудников"
...
* Для группы Azure AD в организации:
  - Отключено для указанной группы Azure AD. Это удобно для добавления "сотрудников" для организации, за исключением участников группы Azure AD.

Дополнительные сведения см. в статье [Настройка элементов управления конфиденциальности аналитики людей](insights-customize-people-insights-privacy.md).

## <a name="authorization"></a>Authorization

Чтобы ваше приложение в Microsoft Graph могло вызывать API людей, ему потребуются указанные ниже разрешения.
