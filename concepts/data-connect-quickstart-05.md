...
1. Найдите **тип ресурса Фабрика** данных и используйте для его создания следующие значения, а затем выберите **Create**.
    ...
    6. На **вкладке Конфигурация Git** убедитесь, что вы либо настраивали Git, либо выбрали параметр _Configure Git позже_.
...
1. По умолчанию фабрика данных Azure будет использовать время запуска интеграции, которое автоматически разрешит область. Поскольку для Подключение данных требуется, чтобы ваш источник и назначение, а также время запуска интеграции существовали в одном Microsoft 365 регионе, рекомендуется создать новое время запуска интеграции с фиксированным регионом.
    ...
    1. Выберите **runtimesNew** >  **интеграции**.
    2. Выберите **Azure, самоуправление и** выберите **Продолжить**.
    ...
    4. Чтобы заполнить форму на окончательном экране, используйте следующие сведения, а затем выберите **Create**.
        ...
        - **Виртуальная конфигурация сети (предварительный просмотр)**: отключена
...
1. Создайте новый конвейер, выбрав значок **плюс** , а затем **конвейер**.
    ...
    - В диалоговом октаге, который отображается, введите ранее созданный **ID** приложения Azure AD и секретный **ID** в полях основных  ИД служб и основных ключевых ключевых служб соответственно, а затем выберите **Create**.
    - Выберите время запуска интеграции, созданное ранее в **Подключение** при отсеве интеграции.
    ...
    - После создания Microsoft 365 для поля **Table** выберите BasicDataSet_v0 **. Message_v0**.
    - Переключение **с Office365Table** на **источник > Pipeline**. Используйте следующие значения для фильтра **Date**.
      ...
      - Выберите **схему импорта** в разделе _Выходные столбцы_ .
...
1. После просмотра **выполняется действие** , перейдите в раздел _Действия выполняется_ , который расположен в нижней части страницы.

1. Наведите **курсор над именем Activity** и выберите параметр очки. В этом случае будет приведена **вкладка Details** .
...
1. На этом этапе запуск действий внутренне приостановлен до тех пор, пока кто-то вручную не одобрит запрос на согласие через Центр администрирования Microsoft 365 или через PowerShell.
