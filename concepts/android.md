...
Если вы скачали [пример приложения подключения для Android](https://github.com/microsoftgraph/android-java-connect-sample), пропустите этот шаг. 

Создайте новый проект в Android Studio. Большинство значений по умолчанию в мастере можно оставить без изменений. Обязательно выполните следующее:

- На странице "Target Android Devices" (Целевые устройства с Android) выберите параметр **Phone and Tablet** (Телефон и планшет).
- В качестве минимальной версии пакета SDK укажите **API 16: Android 4.1 (Jelly Bean)**.
- На странице "Add an Activity to Mobile" (Добавить действие для мобильного устройства) выберите **Basic Activity** (Основное действие).
...
3. Введите имя приложения и нажмите кнопку **Создать**. 
    ...
    Если установлен флажок **Инструкции по настройке**:
 
    А. Выберите **Мобильные и классические приложения**, чтобы определить тип создаваемого приложения.

    Б. Выберите **Android**, чтобы определить используемую технологию для мобильных приложений.
...
> **Примечание.** Если при создании нового приложения вы установили флажок **Инструкции по настройке** на портале регистрации приложений, эти действия можно пропустить. Дополнительные сведения об API Graph см. в разделе [Вызов Microsoft Graph с помощью пакета SDK для Microsoft Graph](#call-microsoft-graph-using-the-microsoft-graph-sdk).

Рассмотрим [пример приложения подключения для Android](https://github.com/microsoftgraph/android-java-connect-sample), чтобы узнать о добавленном коде Microsoft Graph и MSAL.
...
1. Откройте файл **AuthenticationManager** и сначала найдите параметр объявления объекта **PublicClientApplication**, а затем — параметр создания экземпляра в методе **getInstance**.
...
2. В классе **ConnectActivity** найдите обработчик событий для события нажатия кнопки **mConnectButton**. Найдите метод **onClick** и просмотрите соответствующий код.
    ...
    Метод **connect** позволяет вести журнал личных сведений, а также получает экземпляр вспомогательного класса **AuthenticationManager** и коллекцию пользователей объекта платформы MSAL. Если пользователей нет, запускается поток авторизации и аутентификации Azure AD для нового пользователя. В противном случае токен аутентификации будет получен автоматически.
...
3. Найдите обработчик событий, который обрабатывает отклик перенаправления Azure AD, созданный при закрытии пользователем диалогового окна аутентификации. Этот обработчик находится в классе **ConnectActivity**.
...
4. Найдите метод обратного вызова аутентификации, который кэширует токен аутентификации, используемый в вызовах Graph API.
...
В примере приложения подключения добавлена кнопка **Connect** (Подключиться) для основного действия. При ее нажатии в первый раз приложение открывает страницу проверки подлинности в браузере устройства. Теперь необходимо обработать код, отправляемый сервером авторизации на URI перенаправления, и обменять его на токен доступа.
...
2. Добавьте зависимости в пакет SDK для Microsoft Graph и GSON.
...
3. Добавьте токен аутентификации в новые запросы, используя вспомогательный метод **AuthenticateRequest**. Этот метод реализует тот же метод, что и интерфейс аутентификации Microsoft Graph **IAuthenticationProvider**.
...
4. Создайте черновик электронного сообщения и отправьте его, используя указанные ниже методы из вспомогательного класса **GraphServiceController**.
...
3. Выберите устройство или эмулятор Android в диалоговом окне развертывания.
4. Нажмите кнопку **Floating Action** (Астатическое действие), выбирая основное действие.
5. Войдите с использованием личной, рабочей или учебной учетной записи и предоставьте необходимые разрешения.
6. Чтобы продолжить, выберите свое приложение в диалоговом окне выбора приложения.

Проверьте папку "Входящие" в почтовом ящике, выбранном в разделе [Вызов Microsoft Graph](#call-microsoft-graph). Должно прийти сообщение из учетной записи, которая использовалась для входа в приложение.

## <a name="next-steps"></a>Дальнейшие действия

- Опробуйте [песочницу Microsoft Graph](https://developer.microsoft.com/graph/graph-explorer).
- Примеры распространенных операций вы можете найти в примерах кода для Android, доступных [здесь](https://github.com/microsoftgraph/android-java-snippets-sample) и [здесь](https://github.com/microsoftgraph?utf8=%E2%9C%93&query=android) на сайте GitHub.
