...
### <a name="riskyusers-apis"></a>API-интерфейсы RiskyUsers

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Дополнение |Бета| Введена [riskyUsers API](/graph/api/resources/riskyuser?view=graph-rest-beta), представляющий Azure AD пользователей в группу риска, как выявлено системой защиты удостоверения Azure AD. |


### <a name="signin-apis"></a>API-интерфейсы вход

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Изменение   |Бета| Переименовать свойство `conditionalAccessPolicies` для `appliedConditionalAccessPolicy`.|
|Дополнение |Бета| Введена риска дополнительных свойств в [входить API](/graph/api/resources/signin?view=graph-rest-beta), включая `riskDetail`, `riskLevelAggregated`, `riskLevelDuringSignIn`, `riskEventTypes`, и `riskState`.|
|Дополнение |Бета| Представлены дополнительные свойства входа в [входить API](/graph/api/resources/signin?view=graph-rest-beta), включая `authenticationProcessingDetails`, `originalRequestID`, `isInteractive`, `tokenIssuerName`, `tokenIssuerType`, `correlationId`, и `processingTimeinMilliseconds`.|
|Удаление   |Бета| Удалены свойство `isRisky`.|
...
| **Тип изменения** | **Версия** | **Описание**                  |
|:------------|:--------|:-----------------------------------------|
| Дополнение    | Бета-версия   | Возможность [запроса дельты](delta-query-overview.md) [directoryObject](/graph/api/directoryobject-delta?view=graph-rest-beta) |
| Изменение      | 1.0 и бета-версия  | Альтернативная поведения для возврата изменения свойств только в ответ JSON для [пользователей](/graph/api/user-delta?view=graph-rest-1.0) и [групп](/graph/api/group-delta?view=graph-rest-1.0). |
| Дополнение    | v1.0   | Добавлена функция [дельта](/graph/api/directoryrole-delta?view=graph-rest-1.0) для [directoryRole](/graph/api/resources/directoryrole?view=graph-rest-1.0) для поддержки [исправлений с помощью запроса дельты](delta-query-overview.md). |

### <a name="directory-apis"></a>API каталогов

| **Тип изменения** | **Версия**   | **Описание**                          | 
| :-------------- | :------------ | :--------------------------------------- | 
| Дополнение | Бета-версия | Добавить свойство **licenseAssignmentStates** удостоверение [пользователя](/graph/api/resources/user?view=graph-rest-beta) для [Группы на основе лицензирования](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-licensing-whatis-azure-portal).|
| Дополнение | Бета-версия | Добавлен **licenseAssignmentState** ресурсов для [Группы на основе лицензирования](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-licensing-whatis-azure-portal).| 
| Дополнение | Бета-версия | Добавлена **assignedLicenses**, **licenseProcessingState**, **hasMembersWithLicenseErrors** и **membersWithLicenseErrors** свойства [группы](/graph/api/resources/group?view=graph-rest-beta) сущности для [Группы на основе лицензирования](https://docs.microsoft.com/azure/active-directory/fundamentals/active-directory-licensing-whatis-azure-portal).|

### <a name="microsoft-intune-apis"></a>API Microsoft Intune

|Тип изменения|Version|Описание|
|:---|:---|:---|
|Дополнение|v1.0|Добавить свойство **tenantLockdownRequireNetworkDuringOutOfBoxExperience** [windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-1.0) сущности|
|Дополнение|v1.0|Добавить свойство **v12_0** [iosMinimumOperatingSystem](/graph/api/resources/intune-apps-iosminimumoperatingsystem?view=graph-rest-1.0) сложный тип|
|Дополнение|Бета|Добавить свойство **lastReportAggregationDateTime** [deviceManagement](/graph/api/resources/intune-shared-devicemanagement?view=graph-rest-beta) сущности|
...
|Дополнение|Бета|Добавлено действие [enableLegacyPcManagement](/graph/api/intune-deviceconfig-devicemanagement-enablelegacypcmanagement?view=graph-rest-beta) на [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие [extendFeatureUpdatesPause](/graph/api/intune-deviceconfig-windowsupdateforbusinessconfiguration-extendfeatureupdatespause?view=graph-rest-beta) на [windowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие [extendQualityUpdatesPause](/graph/api/intune-deviceconfig-windowsupdateforbusinessconfiguration-extendqualityupdatespause?view=graph-rest-beta) на [windowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие [unassignUserFromDevice](/graph/api/intune-enrollment-windowsautopilotdeviceidentity-unassignuserfromdevice?view=graph-rest-beta) на [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [getAssignedRoleIdsForLoggedInUser](/graph/api/intune-rbac-devicemanagement-getassignedroleidsforloggedinuser?view=graph-rest-beta) на [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [getManagedDevicesWithAppFailures](/graph/api/intune-troubleshooting-user-getmanageddeviceswithappfailures?view=graph-rest-beta) на [пользователя](/graph/api/resources/intune-devices-user?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [managedDeviceEnrollmentAbandonmentSummary](/graph/api/intune-troubleshooting-reportroot-manageddeviceenrollmentabandonmentsummary?view=graph-rest-beta) на [reportRoot](/graph/api/resources/intune-deviceconfig-reportroot?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [managedDeviceEnrollmentAbandonmentDetails](/graph/api/intune-troubleshooting-reportroot-manageddeviceenrollmentabandonmentdetails?view=graph-rest-beta) на [reportRoot](/graph/api/resources/intune-deviceconfig-reportroot?view=graph-rest-beta) |
|Удаление|Бета|Свойство **subjectAlternativeNameType** удалены из сущности [androidForWorkCertificateProfileBase](/graph/api/resources/intune-deviceconfig-androidforworkcertificateprofilebase?view=graph-rest-beta)|
|Дополнение|Бета|Добавить свойство **subjectAlternativeNameType** [androidForWorkPkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-androidforworkpkcscertificateprofile?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [androidForWorkScepCertificateProfile](/graph/api/resources/intune-deviceconfig-androidforworkscepcertificateprofile?view=graph-rest-beta) добавлен свойства **certificateStore**, **customSubjectAlternativeNames** и **subjectAlternativeNameType**|
|Удаление|Бета|Свойство **subjectAlternativeNameType** удалены из сущности [androidWorkProfileCertificateProfileBase](/graph/api/resources/intune-deviceconfig-androidworkprofilecertificateprofilebase?view=graph-rest-beta)|
|Дополнение|Бета|Добавить свойство **subjectAlternativeNameType** [androidWorkProfilePkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-androidworkprofilepkcscertificateprofile?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [androidWorkProfileScepCertificateProfile](/graph/api/resources/intune-deviceconfig-androidworkprofilescepcertificateprofile?view=graph-rest-beta) добавлен свойства **certificateStore**, **customSubjectAlternativeNames** и **subjectAlternativeNameType**|
|Дополнение|Бета|Добавить свойство **legacyPcManangementEnabled** [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) сущности|
|Удаление|Бета|Свойство **pinRequiredOnLaunchInsteadOfBiometric** удалены из сущности [managedAppProtection](/graph/api/resources/intune-mam-managedappprotection?view=graph-rest-beta)|
|Дополнение|Бета|Добавить свойство **roleScopeTagIds** [managedDeviceMobileAppConfiguration](/graph/api/resources/intune-apps-manageddevicemobileappconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **applicationGuardEnabledOptions** [windows10EndpointProtectionConfiguration](/graph/api/resources/intune-deviceconfig-windows10endpointprotectionconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **selectedMobileAppIds** [windows10EnrollmentCompletionPageConfiguration](/graph/api/resources/intune-onboarding-windows10enrollmentcompletionpageconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавлен **engagedRestartDeadlineInDays**, **engagedRestartSnoozeScheduleInDays**, **engagedRestartTransitionScheduleInDays**, **autoRestartNotificationDismissal**, **scheduleRestartWarningInHours** и ** scheduleImminentRestartWarningInMinutes** свойства [windowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [windowsWifiConfiguration](/graph/api/resources/intune-deviceconfig-windowswificonfiguration?view=graph-rest-beta) добавлен свойства **preSharedKey** и **meteredConnectionLimit**|
|Дополнение|Бета|Добавить свойство навигации **intuneBrandingProfiles** [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавлена свойства **v6_0**, **v7_0**, **v7_1**, **v8_0**, **v8_1** и **v9_0** [androidMinimumOperatingSystem](/graph/api/resources/intune-apps-androidminimumoperatingsystem?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавить свойство **v12_0** [iosMinimumOperatingSystem](/graph/api/resources/intune-apps-iosminimumoperatingsystem?view=graph-rest-beta) сложный тип|
|Удаление|Бета|Свойство **runAsLoggedOnUser** удалены из [win32LobAppPowerShellScriptDetection](/graph/api/resources/intune-apps-win32lobapppowershellscriptdetection?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавить свойство **lastUpdateDateTime** [osVersionCount](/graph/api/resources/intune-devices-osversioncount?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавить свойство **lastUpdateDateTime** [windowsMalwareCategoryCount](/graph/api/resources/intune-devices-windowsmalwarecategorycount?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавить свойство **lastUpdateDateTime** [windowsMalwareExecutionStateCount](/graph/api/resources/intune-devices-windowsmalwareexecutionstatecount?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавить свойство **lastUpdateDateTime** [windowsMalwareNameCount](/graph/api/resources/intune-devices-windowsmalwarenamecount?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавить свойство **lastUpdateDateTime** [windowsMalwareStateCount](/graph/api/resources/intune-devices-windowsmalwarestatecount?view=graph-rest-beta) сложный тип|

### <a name="privileged-identity-management-apis"></a>API управления привилегированными пользователями

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Изменение | Бета | Изменение сущности [privilegedapproval](/graph/api/resources/privilegedapproval?view=graph-rest-beta) .|
| Дополнение | Бета | Добавлена [privilegedroleassignmentrequest](/graph/api/resources/privilegedroleassignmentrequest?view=graph-rest-beta) сущности и следующие методы и действия:<br> [перечисление](/graph/api/privilegedroleassignmentrequest-list?view=graph-rest-beta); <br> [создание](/graph/api/privilegedroleassignmentrequest-post?view=graph-rest-beta); <br> [Отмена](/graph/api/privilegedroleassignmentrequest-cancel?view=graph-rest-beta) <br> [Мои](/graph/api/privilegedroleassignmentrequest-my?view=graph-rest-beta) |
| Дополнение | Бета | Добавлены [обновления](/graph/api/privilegedrolesettings-update?view=graph-rest-beta) для [privilegedRoleSettings](/graph/api/resources/privilegedrolesettings?view=graph-rest-beta)|
| Удаление |Бета| Устаревшие [Self активации назначения ролей](/graph/api/privilegedrole_selfactivate?view=graph-rest-beta)|

### <a name="microsoft-teams-apis"></a>API Microsoft Teams

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Дополнение|Бета|Добавлена поддержка разрешения приложения [группы архива](/graph/api/team-archive?view=graph-rest-beta) и [unarchive группы](/graph/api/team-unarchive?view=graph-rest-beta) API-интерфейсы.|

### <a name="outlook-contacts"></a>Контакты Outlook

| **Тип изменения** | **Версия** | **Описание**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Удаление         | v1.0        | Это исправление в документации по: удалены свойство **флаг** из раздела сущности [контактов](/graph/api/resources/contact?view=graph-rest-1.0) . Свойство никогда не было выполнено в сущности **контактов** .|

### <a name="reports-apis"></a>API отчетов
| Тип изменения | Version | Описание                              |
|:------------|:--------|:-----------------------------------------|
| Дополнение    | v1.0    | Добавить свойство **ID сайта** [getSharePointSiteUsageDetail](/graph/api/reportroot-getsharepointsiteusagedetail?view=graph-rest-1.0). |

## <a name="september-2018"></a>Сентябрь 2018

### <a name="calls-and-online-meetings-api"></a>Звонки и API собрания по сети

| **Тип изменения** | **Версия** | **Описание**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Изменение          | Бета-версия        | Ресурс [приложения](/graph/api/resources/application?view=graph-rest-beta) были обновлены для добавления коллекции звонки. |
| Изменение          | Бета-версия        | [Операция](/graph/api/resources/operation?view=graph-rest-beta) ресурсов был обновлен для поддержки длительным звонков и собрания по API-интерфейсы. |
| Дополнение        | Бета-версия        | Добавлен ресурс [звонков](/graph/api/resources/call?view=graph-rest-beta) для управление звонками аудио/видео (изначально, в Майкрософт для групп), включая интерфейсы API для [создания вызова](/graph/api/application-post-calls?view=graph-rest-beta), [Извлечение звонка](/graph/api/call-get?view=graph-rest-beta), [Удаление (разъединения) вызов](/graph/api/call-delete?view=graph-rest-beta), [ответ на звонок](/graph/api/call-answer?view=graph-rest-beta), [отклонение Вызовите](/graph/api/call-reject?view=graph-rest-beta), [Перенаправление звонка](/graph/api/call-redirect?view=graph-rest-beta)и [Передача звонка](/graph/api/call-transfer?view=graph-rest-beta). Также мы добавили API-интерфейсы для поддержки [сценариев IVR](/graph/api/resources/calls-api-ivr-overview?view=graph-rest-beta): [воспроизведения в строке](/graph/api/call-playprompt?view=graph-rest-beta), [записи звонка](/graph/api/call-record?view=graph-rest-beta), [Отменить обработки мультимедиа](/graph/api/call-cancelmediaprocessing?view=graph-rest-beta)и [Подписка на коснитесь тона уведомлений](/graph/api/call-subscribetotone?view=graph-rest-beta). |
| Дополнение        | Бета-версия        | Добавлена [участника](/graph/api/resources/call?view=graph-rest-beta) ресурсов и интерфейсы API для управления участниками в вызовах аудио и видео и собраний, включая [Извлечение объект участника](/graph/api/participant-get?view=graph-rest-beta), [Настройка аудиомикшеру для участника](/graph/api/participant-configuremixer?view=graph-rest-beta), отключение звука [один](/graph/api/participant-mute?view=graph-rest-beta) или [все ](/graph/api/participant-muteall?view=graph-rest-beta)участников, [Извлечение списка участников](/graph/api/call-list-participants?view=graph-rest-beta) в вызова/собрания, а [Приглашение участников](/graph/api/participant-invite?view=graph-rest-beta) для вызова/собрания. |
| Дополнение        | Бета-версия        | Добавлены API-интерфейсы для приложений для управления и участвовать в вызовах и собраний, включая возможность [совместного доступа к контенту](/graph/api/call-changescreensharingrole?view=graph-rest-beta), [Отключение и включение звука самого](/graph/api/call-unmute?view=graph-rest-beta)и [обновлять метаданные, связанные с помощью вызова](/graph/api/call-updatemetadata?view=graph-rest-beta). |
| Дополнение        | Бета-версия        | Добавлена [звука группы маршрутизации](/graph/api/resources/audioroutinggroup?view=graph-rest-beta) ресурсов и интерфейсы API для управления закрытый звука маршрутов между участниками в многосторонней беседе, включая [Создание звука группы маршрутизации](/graph/api/call-post-audioroutinggroups?view=graph-rest-beta), [извлечение их список](/graph/api/audioroutinggroup-get?view=graph-rest-beta)и [обновление](/graph/api/audioroutinggroup-update?view=graph-rest-beta) и [Удаление](/graph/api/audioroutinggroup-delete?view=graph-rest-beta) их. |
| Дополнение        | Бета-версия        | Добавлен ресурсов [собрания по сети](/graph/api/resources/audioroutinggroup?view=graph-rest-beta) и интерфейсы API для управления группами Майкрософт собрания по сети. Вначале имеется только один API для собраний по сети, чтобы [получить объект собрание по сети](/graph/api/onlinemeeting-get?view=graph-rest-beta). Связанные ресурсов для [аудиоконференции сведения](/graph/api/resources/audioconferencing?view=graph-rest-beta) , связанные с собрания (например URL-связь, секретный код и номера телефонов) также добавлена. |
| Дополнение        | Бета-версия        | Многие из звонков и собрания интерфейсам API вступили время выполнения, поэтому ресурсы, необходимые для этих длительных операций были добавлены: [операций, связанных с вызова](/graph/api/resources/commsoperation?view=graph-rest-beta), [воспроизведения звуковых приглашений](/graph/api/resources/playpromptoperation?view=graph-rest-beta)и [записи](/graph/api/resources/recordoperation?view=graph-rest-beta).  |

### <a name="dynamics-365-business-central-api"></a>API центра Business Dynamics 365

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Дополнение        | Бета-версия          | Добавлены финансирование API-интерфейсы для центра бизнес Dynamics 365. Дополнительные сведения содержатся в разделе [Справочник по API финансирование](/graph/api/resources/dynamics-graph-reference?view=graph-rest-beta)

### <a name="microsoft-graph-data-connect"></a>Подключение данных Microsoft Graph

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Дополнение         | Не применимо| Появилась возможность доступа к данным Office 365 в пакетном режиме. Дополнительные сведения см [данные диаграммы Microsoft connect (Просмотр)](data-connect-overview.md).|

### <a name="microsoft-intune-apis"></a>API Microsoft Intune
|Тип изменения|Version|Описание|
|:---|:---|:---|
|Дополнение|v1.0|Добавлено действие [assign](/graph/api/intune-apps-manageddevicemobileappconfiguration-assign?view=graph-rest-1.0) для объекта [managedDeviceMobileAppConfiguration](/graph/api/resources/intune-apps-manageddevicemobileappconfiguration?view=graph-rest-1.0). |
...
|Дополнение|Бета|Добавлено действие [Назначение](/graph/api/intune-cirrus-officeclientconfiguration-assign?view=graph-rest-beta) на [officeClientConfiguration](/graph/api/resources/intune-cirrus-officeclientconfiguration?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие **updatePrioritie** на [officeClientConfiguration](/graph/api/resources/intune-cirrus-officeclientconfiguration?view=graph-rest-beta) коллекции |
...
|Дополнение|Бета|Добавлено действие **overrideComplianceState** на [managedDevice](/graph/api/resources/intune-devices-manageddevice?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие **getTargetedUsersAndDevices** на [deviceConfiguration](/graph/api/resources/intune-deviceconfig-deviceconfiguration?view=graph-rest-beta) коллекции |
|Дополнение|Бета|Добавлена функция [autopilotDeviceStream](/graph/api/intune-enrollment-importedwindowsautopilotdeviceidentityupload-autopilotdevicestream?view=graph-rest-beta) на [importedWindowsAutopilotDeviceIdentityUpload](/graph/api/resources/intune-enrollment-importedwindowsautopilotdeviceidentityupload?view=graph-rest-beta) |
|Дополнение|Бета|Добавить свойство **restrictedApps** [androidCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidcompliancepolicy?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **tokenCreationDateTime** [androidDeviceOwnerEnrollmentProfile](/graph/api/resources/intune-androidforwork-androiddeviceownerenrollmentprofile?view=graph-rest-beta) сущности|
|Удаление|Бета|Свойство **restrictedApps** удалены из сущности [androidForWorkCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidforworkcompliancepolicy?view=graph-rest-beta)|
|Удаление|Бета|Свойство **restrictedApps** удалены из сущности [androidWorkProfileCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidworkprofilecompliancepolicy?view=graph-rest-beta)|
|Изменение|Бета|Изменить следующие свойства для сущности [appleVpnConfiguration](/graph/api/resources/intune-deviceconfig-applevpnconfiguration?view=graph-rest-beta) :<br/>требуется, чтобы необязательно **enablePerApp** из<br/>|
|Дополнение|Бета|Сущность [defaultManagedAppProtection](/graph/api/resources/intune-mam-defaultmanagedappprotection?view=graph-rest-beta) добавлен свойства **disableProtectionOfManagedOutboundOpenInData** и **protectInboundDataFromUnknownSources**|
|Дополнение|Бета|Добавить свойство **microsoftStoreForBusinessPortalSelection** [deviceAppManagement](/graph/api/resources/intune-shared-deviceappmanagement?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **passcodeMinutesOfInactivityBeforeScreenTimeout** [iosCompliancePolicy](/graph/api/resources/intune-deviceconfig-ioscompliancepolicy?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **useOAuth** [iosEasEmailProfileConfiguration](/graph/api/resources/intune-deviceconfig-ioseasemailprofileconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавлен **kioskModeBlockVolumeButtons**, **classroomForceRequestPermissionToLeaveClasses**, **keychainBlockCloudSync**, **pkiBlockOTAUpdates**, **privacyForceLimitAdTracking**, ** enterpriseBookBlockBackup**, **enterpriseBookBlockMetadataSync**, **airPrintBlocked**, **airPrintBlockCredentialsStorage**, **airPrintForceTrustedTLS**, **airPrintBlockiBeaconDiscovery**, ** blockSystemAppRemoval** и **vpnBlockCreation** свойства для сущности [iosGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-iosgeneraldeviceconfiguration?view=graph-rest-beta)|
|Дополнение|Бета|Сущность [iosManagedAppProtection](/graph/api/resources/intune-mam-iosmanagedappprotection?view=graph-rest-beta) добавлен свойства **disableProtectionOfManagedOutboundOpenInData** и **protectInboundDataFromUnknownSources**|
|Дополнение|Бета|Добавить свойство **gatekeeperAllowedAppSource** [macOSCompliancePolicy](/graph/api/resources/intune-deviceconfig-macoscompliancepolicy?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [macOSGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-macosgeneraldeviceconfiguration?view=graph-rest-beta) добавлен свойства **keychainBlockCloudSync**, **airPrintBlocked**, **airPrintForceTrustedTLS** и **airPrintBlockiBeaconDiscovery**|
|Дополнение|Бета|Сущность [managedAppRegistration](/graph/api/resources/intune-mam-managedappregistration?view=graph-rest-beta) добавлен свойства **deviceModel** и **deviceManufacturer**|
|Дополнение|Бета|Добавить свойство **enabledForScopeValidation** [resourceOperation](/graph/api/resources/intune-rbac-resourceoperation?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **claimTokenManagementFromExternalMdm** [vppToken](/graph/api/resources/intune-onboarding-vpptoken?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **windows10AppsForceUpdateSchedule** [windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **deploymentProfileAssignmentDetailedStatus** [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [deviceManagement](/graph/api/resources/intune-shared-devicemanagement?view=graph-rest-beta) добавлен свойства навигации **deviceConfigurationConflictSummary** и **importedWindowsAutopilotDeviceIdentityUploads**|
|Дополнение|Бета|Добавить свойство навигации **intendedDeploymentProfile** [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta) сущности|
|Дополнение|Бета|Свойства **startLayoutTileSize** и **имя** добавляемого сложный тип [windowsKioskAppBase](/graph/api/resources/intune-deviceconfig-windowskioskappbase?view=graph-rest-beta)|
|Дополнение|Бета|Сложный тип [windowsKioskDesktopApp](/graph/api/resources/intune-deviceconfig-windowskioskdesktopapp?view=graph-rest-beta) добавляется свойства **desktopApplicationId** и **desktopApplicationLinkPath**|
|Удаление|Бета|Удален из [windowsKioskDesktopApp](/graph/api/resources/intune-deviceconfig-windowskioskdesktopapp?view=graph-rest-beta) сложный тип свойства **name**|
|Дополнение|Бета|Добавить свойство **disallowDesktopApps** [windowsKioskMultipleApps](/graph/api/resources/intune-deviceconfig-windowskioskmultipleapps?view=graph-rest-beta) сложный тип|
|Изменение|Бета|Изменять следующие свойства сложного типа [windowsKioskMultipleApps](/graph/api/resources/intune-deviceconfig-windowskioskmultipleapps?view=graph-rest-beta) .<br/>требуется, чтобы необязательно **startMenuLayoutXml** из<br/>|
|Дополнение|Бета|Добавлена свойства **v10_1607**, **v10_1703**, **v10_1709** и **v10_1803** [windowsMinimumOperatingSystem](/graph/api/resources/intune-apps-windowsminimumoperatingsystem?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавлен элемент **paloAltoGlobalProtect** тип перечисления [androidWorkProfileVpnConnectionType](/graph/api/resources/intune-deviceconfig-androidworkprofilevpnconnectiontype?view=graph-rest-beta)|
|Дополнение|Бета|Добавлен элемент **remoteLock** тип перечисления [deviceComplianceActionType](/graph/api/resources/intune-deviceconfig-devicecomplianceactiontype?view=graph-rest-beta)|
...
| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Дополнение|Бета|Добавлены API для [вкладки](/graph/api/resources/teamstab?view=graph-rest-beta).|
|Дополнение|Бета|Добавлены API-Интерфейс для [публикации приложений для вашей организации](/graph/api/resources/teamsapp?view=graph-rest-beta).|
|Дополнение|Бета|Добавлены приложения разрешения поддерживает получение [/teams/ {идентификатор}](/graph/api/team-get?view=graph-rest-beta). |
|Дополнение|Бета|Добавлен для поддержки разрешения приложения [/teams/ GET {идентификатор} аудио- и каналы](/graph/api/group-list-channels?view=graph-rest-beta). |
|Дополнение|Бета|Добавлены приложения разрешения поддерживает получение [/channels/ /teams/ {идентификатор} {идентификатор}](/graph/api/channel-get?view=graph-rest-beta). |
|Дополнение|Бета|Добавлены приложения разрешения поддерживать на [ПОМЕСТИТЬ /groups/ {идентификатор} / рабочей группы](/graph/api/team-put-teams?view=graph-rest-beta). |
|Дополнение|Бета|Добавлена поддержка разрешения приложения применить исправление [/teams/ {идентификатор}](/graph/api/team-update?view=graph-rest-beta). |
|Дополнение|Бета|Добавить приложение, которое поддерживает разрешения [создать канал](/graph/api/channel-post?view=graph-rest-beta), [канал обновление](/graph/api/channel-patch?view=graph-rest-beta)и [Удаление канала](/graph/api/channel-delete?view=graph-rest-beta). |
|Удаление|Бета| Удаленные isBlocks и installedState свойства из [teamsApp](/graph/api/resources/teamsapp?view=graph-rest-beta).|
|Изменение| Бета | Свойство context на [teamsApp](/graph/api/resources/teamsapp?view=graph-rest-beta) переименовано в distributionMethod.|

### <a name="outlook-mail"></a>Почта Outlook

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Дополнение        | 1.0 и бета-версия | Свойство **internetMessageHeaders** объекта [сообщения](/graph/api/resources/message?view=graph-rest-1.0) теперь доступен для чтения при создании сообщения. |


### <a name="project-rome-notifications-api"></a>Проект рим уведомлений API

| **Тип изменения** | **Версия** | **Описание**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Дополнение          | Бета-версия        | Добавить тип ресурсов [уведомления](/graph/api/resources/projectrome-notification?view=graph-rest-beta) . |
| Дополнение          | Бета-версия        | Добавлена [создание и публикация уведомлений] (аудио- и графике/api/projectrome_notification_post? представления = График rest бета-версии) API.|

### <a name="security-apis"></a>API безопасности

| **Тип изменения** | **Версия** | **Описание**              |
| :-------------- | :---------- | :--------------------------------------- |
| Дополнение        | Бета-версия       | Добавлена интерфейсы API показатель безопасного [безопасности API](/graph/api/resources/securescore-api-overview?view=graph-rest-beta), включая следующие ресурсы и операций:<br/>[secureScores](/graph/api/resources/securescores?view=graph-rest-beta) (и связанных с ними сущностей)<br/>[Список secureScores](/graph/api/securescores-list?view=graph-rest-beta)<br/>[secureScoreControlProfiles](/graph/api/resources/securescorecontrolprofiles?view=graph-rest-beta)<br/>[Список secureScoreControlProfiles](/graph/api/securescorecontrolprofiles-list?view=graph-rest-beta)<br/>[Обновление secureScoreControlProfiles](/graph/api/securescorecontrolprofiles-update?view=graph-rest-beta)


### <a name="onedrive-and-sharepoint-apis"></a>OneDrive и интерфейсы API SharePoint

| **Тип изменения** | **Версия** | **Описание**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Дополнение        | Бета-версия        | Добавлен аргумент **deferCommit** [createUploadSession](/graph/api/driveitem-createuploadsession?view=graph-rest-beta) действие на [driveItem](/graph/api/resources/driveitem?view=graph-rest-beta)|
| Дополнение        | Бета-версия        | Сложный тип [storagePlanInformation](/graph/api/resources/storageplaninformation?view=graph-rest-beta) добавлен |
| Дополнение        | Бета-версия        | Добавить свойство **storagePlanInformation** сложный тип [квоты](/graph/api/resources/quota?view=graph-rest-beta) |
| Дополнение        | Бета-версия        | Добавлено **следующее** свойство навигации для сущности [диска](/graph/api/resources/drive?view=graph-rest-beta) |
| Дополнение        | Бета-версия        | Добавлено действие [выполните](/graph/api/driveitem-follow?view=graph-rest-beta) на [driveItem](/graph/api/resources/driveitem?view=graph-rest-beta) |
| Дополнение        | Бета-версия        | Добавлена [Отменить подписку](/graph/api/driveitem-unfollow?view=graph-rest-beta) API |
| Дополнение        | Бета-версия        | Добавить свойство **hasPassword** сущности [разрешений](/graph/api/resources/permission?view=graph-rest-beta) |
| Дополнение        | Бета-версия        | Добавить свойство **preventsDownload** [sharingLink](/graph/api/resources/sharinglink?view=graph-rest-beta) сложный тип |
| Дополнение        | Бета-версия        | Сущность [sharedDriveItem](/graph/api/resources/shareddriveitem?view=graph-rest-beta) добавлен свойство навигации **разрешений** |
| Дополнение        | Бета-версия        | Сущность [columnDefinition](/graph/api/resources/columndefinition?view=graph-rest-beta) добавлен свойство **географического расположения** |
| Дополнение        | Бета-версия        | Сложный тип [geolocationColumn](/graph/api/resources/geolocationcolumn?view=graph-rest-beta) добавлен |
| Дополнение        | Бета-версия        | Сущность [driveItem](/graph/api/resources/driveitem?view=graph-rest-beta) добавлен свойство **аналитики** |
| Дополнение        | Бета-версия        | Добавить свойство **analytics** объект [сайта](/graph/api/resources/site?view=graph-rest-beta) |
| Дополнение        | Бета-версия        | Добавить свойство **аналитических** объектов [listItem](/graph/api/resources/listitem?view=graph-rest-beta) |
| Дополнение        | Бета-версия        | Добавлена функция **getActivitiesByInterval** [driveItem](/graph/api/resources/driveitem?view=graph-rest-beta) сущности |
| Дополнение        | Бета-версия        | Добавлена функция **getActivitiesByInterval** на объект [сайта](/graph/api/resources/site?view=graph-rest-beta) |
| Дополнение        | Бета-версия        | Добавлена функция **getActivitiesByInterval** на объектов [listItem](/graph/api/resources/listitem?view=graph-rest-beta) |
| Дополнение        | Бета-версия        | Добавлена [itemAnalytics](/graph/api/resources/itemanalytics?view=graph-rest-beta) сущности |
| Дополнение        | Бета-версия        | Добавлена [itemActivityStat](/graph/api/resources/itemactivity?view=graph-rest-beta) сущности |
| Дополнение        | Бета-версия        | Сложный тип [itemActionStat](/graph/api/resources/itemactionstat?view=graph-rest-beta) добавлен |
| Дополнение        | Бета-версия        | Сложный тип [accessAction](/graph/api/resources/accessaction?view=graph-rest-beta) добавлен |
| Дополнение        | Бета-версия        | Сложный тип [incompleteData](/graph/api/resources/incompletedata?view=graph-rest-beta) добавлен |
| Дополнение        | Бета-версия        | Добавить свойство **access** [itemActivity](/graph/api/resources/itemactivity?view=graph-rest-beta) сложный тип |
| Дополнение        | Бета-версия        | Сложный тип [itemActivity](/graph/api/resources/itemactivity?view=graph-rest-beta) добавляется свойство **location** |
| Дополнение        | v1.0        | Добавлено действие **предварительного просмотра** в сущности [driveItem](/graph/api/resources/driveitem?view=graph-rest-1.0) |
| Дополнение        | v1.0        | Сложный тип [itemPreviewInfo](/graph/api/resources/itempreviewinfo?view=graph-rest-1.0) добавлен |

## <a name="august-2018"></a>Август 2018

### <a name="delta-query"></a>Запрос на получение различий

| **Тип изменения** | **Версия** | **Описание**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Дополнение        | Бета-версия        | Возможность [запроса дельты](delta-query-overview.md) для следующих сущностей в Azure AD:<br/>[application](/graph/api/application-delta?view=graph-rest-beta)<br/>[directoryRole](/graph/api/directoryrole-delta?view=graph-rest-beta)<br/>[servicePrincipal](/graph/api/serviceprincipal-delta?view=graph-rest-beta) |

### <a name="directory-apis"></a>API каталогов

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Дополнение | v1.0 | Добавлено свойство isMultipleDataLocationsForServicesEnabled в ресурс [Organization](/graph/api/resources/organization?view=graph-rest-beta), позволяющее приложениям проверять, включена ли для клиента поддержка нескольких регионов. Добавлено свойство preferredDataLocation в ресурсы [user](/graph/api/resources/user?view=graph-rest-beta) и [group](/graph/api/resources/group?view=graph-rest-beta), позволяющее настраивать предпочтительное расположение данных для пользователя и группы.|
| Дополнение | v1.0 | Свойство добавлены [onPremisesProvisioningErrors](/graph/api/resources/onpremisesprovisioningerror?view=graph-rest-1.0) [пользователей](/graph/api/resources/user?view=graph-rest-1.0) и [группы](/graph/api/resources/group?view=graph-rest-1.0) сущности, представляющий ошибок синхронизации службы каталогов при синхронизации каталогов в локальной для Azure Active Directory при использовании Microsoft Синхронизация продукта (в том числе Azure AD подключение, DirSync и MIM и соединитель).|
| Дополнение | v1.0 | Добавить свойство [onPremisesExtensionAttributes](/graph/api/resources/onpremisesextensionattributes?view=graph-rest-1.0) удостоверение [пользователя](/graph/api/resources/user?view=graph-rest-1.0) , который содержит пятнадцать свойства атрибут настраиваемого расширения. Для пользователя с onPremisesSyncEnabled, этот набор свойств, создаются в локальной службе Active Directory и синхронизируются с Azure AD и доступен только для чтения. Для пользователей только в облаке, (где onPremisesSyncEnabled имеет значение false) эти свойства могут быть установлены во время создания или обновления.|
|Дополнение|v1.0|Добавлена свойства **onPremisesDomainName**, **onPremisesSamAccountName**и **onPremisesUserPrincipalName** удостоверение [пользователя](/graph/api/resources/user?view=graph-rest-1.0)|
...
|Дополнение|v1.0|Добавлена функция [managedDeviceEnrollmentTopFailures](/graph/api/intune-shared-reportroot-manageddeviceenrollmenttopfailures?view=graph-rest-1.0) для ресурса [reportRoot](/graph/api/resources/intune-shared-reportroot?view=graph-rest-1.0). |
|Добавление|v1.0|Добавить свойство **kioskModeBuiltInAppId** [iosGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-iosgeneraldeviceconfiguration?view=graph-rest-1.0) сущности|
|Дополнение|v1.0|Добавлен элемент **не назначено** тип перечисления [complianceStatus](/graph/api/resources/intune-shared-compliancestatus?view=graph-rest-1.0)|
|Дополнение|v1.0|Добавлен элемент **pushNotification** тип перечисления [deviceComplianceActionType](/graph/api/resources/intune-deviceconfig-devicecomplianceactiontype?view=graph-rest-1.0)|
|Дополнение|v1.0|Добавлен элемент **userAbandonment** тип перечисления [deviceEnrollmentFailureReason](/graph/api/resources/intune-troubleshooting-deviceenrollmentfailurereason?view=graph-rest-1.0)|
|Дополнение|v1.0|Добавлена члены **и **неправильно настроенный** ** тип перечисления [managedDevicePartnerReportedHealthState](/graph/api/resources/intune-devices-manageddevicepartnerreportedhealthstate?view=graph-rest-1.0)|
|Дополнение|v1.0|Добавлен элемент **assignedToExternalMDM** тип перечисления [vppTokenState](/graph/api/resources/intune-onboarding-vpptokenstate?view=graph-rest-1.0)|
||
...
|Дополнение|Бета|Добавлено действие [uploadDepToken](/graph/api/intune-enrollment-deponboardingsetting-uploaddeptoken?view=graph-rest-beta) на [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие [syncWithAppleDeviceEnrollmentProgram](/graph/api/intune-enrollment-deponboardingsetting-syncwithappledeviceenrollmentprogram?view=graph-rest-beta) на [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие [setDefaultProfile](/graph/api/intune-enrollment-enrollmentprofile-setdefaultprofile?view=graph-rest-beta) на [enrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие **importAppleDeviceIdentityList** на [importedAppleDeviceIdentity](/graph/api/resources/intune-enrollment-importedappledeviceidentity?view=graph-rest-beta) коллекции |
|Дополнение|Бета|Добавлено действие [updateDeviceProfileAssignment](/graph/api/intune-enrollment-enrollmentprofile-updatedeviceprofileassignment?view=graph-rest-beta) на [enrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие [shareForSchoolDataSyncService](/graph/api/intune-enrollment-deponboardingsetting-shareforschooldatasyncservice?view=graph-rest-beta) на [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие [unshareForSchoolDataSyncService](/graph/api/intune-enrollment-deponboardingsetting-unshareforschooldatasyncservice?view=graph-rest-beta) на [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Дополнение|Бета|Добавлено действие [assignUserToDevice](/graph/api/intune-enrollment-windowsautopilotdeviceidentity-assignusertodevice?view=graph-rest-beta) на [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [getRoleScopeTagsByResource](/graph/api/intune-rbac-devicemanagement-getrolescopetagsbyresource?view=graph-rest-beta) на [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [getRoleScopeTagsByIds](/graph/api/intune-rbac-devicemanagement-getrolescopetagsbyids?view=graph-rest-beta) на [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [getEncryptionPublicKey](/graph/api/intune-enrollment-deponboardingsetting-getencryptionpublickey?view=graph-rest-beta) на [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [exportMobileConfig](/graph/api/intune-enrollment-enrollmentprofile-exportmobileconfig?view=graph-rest-beta) на [enrollmentProfile](/graph/api/resources/intune-enrollment-enrollmentprofile?view=graph-rest-beta) |
|Дополнение|Бета|Добавлена функция [autopilotDeviceStream](/graph/api/intune-enrollment-importedwindowsautopilotdeviceidentityupload-autopilotdevicestream?view=graph-rest-beta) на [importedWindowsAutopilotDeviceIdentityUpload](/graph/api/resources/intune-enrollment-importedwindowsautopilotdeviceidentityupload?view=graph-rest-beta) |
|Удаление|Бета|Удаляется коллекция **uploadDepToken** |
|Удаление|Бета|Удален **syncWithAppleDeviceEnrollmentProgram** действия в коллекции [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) |
|Удаление|Бета|Удалена функция **getEncryptionPublicKey** на [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) коллекции |
|Дополнение|Бета|Добавить свойство **restrictedApps** [androidForWorkCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidforworkcompliancepolicy?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [androidForWorkGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-androidforworkgeneraldeviceconfiguration?view=graph-rest-beta) добавлен свойства **vpnAlwaysOnPackageIdentifier** и **vpnEnableAlwaysOnLockdownMode**|
|Удаление|Бета|Свойство **имя пакета** удалены из сущности [androidForWorkMobileAppConfiguration](/graph/api/resources/intune-apps-androidforworkmobileappconfiguration?view=graph-rest-beta)|
|Дополнение|Бета|Добавить свойство **restrictedApps** [androidWorkProfileCompliancePolicy](/graph/api/resources/intune-deviceconfig-androidworkprofilecompliancepolicy?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [androidWorkProfileGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-androidworkprofilegeneraldeviceconfiguration?view=graph-rest-beta) добавлен свойства **vpnAlwaysOnPackageIdentifier** и **vpnEnableAlwaysOnLockdownMode**|
|Дополнение|Бета|Добавить свойство **optInToDeviceIdSharing** [appleVpnConfiguration](/graph/api/resources/intune-deviceconfig-applevpnconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) добавлен свойства **tokenType**, **tokenName**, **syncedDeviceCount**, **defaultProfileDisplayName** и **dataSharingConsentGranted**|
|Дополнение|Бета|Добавить свойство **roleScopeTagIds** [deviceCompliancePolicy](/graph/api/resources/intune-deviceconfig-devicecompliancepolicy?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [deviceConfiguration](/graph/api/resources/intune-deviceconfig-deviceconfiguration?view=graph-rest-beta) добавлен свойства **roleScopeTagIds** и **supportsScopeTags**|
|Дополнение|Бета|Добавить свойство **windowsMalwareOverview** [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) сущности|
|Изменение|Бета|Изменить следующие свойства для сущности [iosCertificateProfileBase](/graph/api/resources/intune-deviceconfig-ioscertificateprofilebase?view=graph-rest-beta) :<br/>свойство **subjectAlternativeNameType** преобразовано из требуемого в необязательное.<br/>|
|Дополнение|Бета|Добавить свойство **restrictedApps** [iosCompliancePolicy](/graph/api/resources/intune-deviceconfig-ioscompliancepolicy?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [iosScepCertificateProfile](/graph/api/resources/intune-deviceconfig-iosscepcertificateprofile?view=graph-rest-beta) добавлен свойства **certificateStore** и **customSubjectAlternativeNames**|
|Дополнение|Бета|Добавить свойство **enforcedSoftwareUpdateDelayInDays** [iosUpdateConfiguration](/graph/api/resources/intune-deviceconfig-iosupdateconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [iosVpnConfiguration](/graph/api/resources/intune-deviceconfig-iosvpnconfiguration?view=graph-rest-beta) добавлен свойства **providerType**, **userDomain**, **strictEnforcement**, **cloudName** и **список исключений**|
|Дополнение|Бета|Добавлен **safariBlockAutofill**, **cameraBlocked**, **iTunesBlockMusicService**, **spotlightBlockInternetResults**, **keyboardBlockDictation**, **definitionLookupBlocked**, ** appleWatchBlockAutoUnlock**, **iTunesBlockFileSharing**, **iCloudBlockDocumentSync**, **iCloudBlockMail**, **iCloudBlockAddressBook**, **iCloudBlockCalendar**, **iCloudBlockReminders**, ** iCloudBlockBookmarks**, **iCloudBlockNotes**, **airDropBlocked**, **passwordBlockModification** и **passwordBlockFingerprintUnlock** свойства [macOSGeneralDeviceConfiguration](/graph/api/resources/intune-deviceconfig-macosgeneraldeviceconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [managedDevice](/graph/api/resources/intune-devices-manageddevice?view=graph-rest-beta) добавлен свойства **roleScopeTagIds**, **windowsActiveMalwareCount**, **windowsRemediatedMalwareCount**, **заметки** и **configurationManagerClientHealthState**|
|Дополнение|Бета|Добавить свойство **installStateDetail** [mobileAppInstallStatus](/graph/api/resources/intune-apps-mobileappinstallstatus?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавить свойство **roleScopeTagIds** [notificationMessageTemplate](/graph/api/resources/intune-notification-notificationmessagetemplate?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [officeSuiteApp](/graph/api/resources/intune-apps-officesuiteapp?view=graph-rest-beta) добавлен свойства **targetVersion** и **updateVersion**|
|Дополнение|Бета|Сущность [resourceOperation](/graph/api/resources/intune-rbac-resourceoperation?view=graph-rest-beta) добавлен свойство **ресурса**|
|Дополнение|Бета|Сущность [sharedPCConfiguration](/graph/api/resources/intune-deviceconfig-sharedpcconfiguration?view=graph-rest-beta) добавлен свойства **localStorage**, **setPowerPolicies** и **signInOnResume**|
|Дополнение|Бета|Добавить свойство **configurationManagerComplianceRequired** [windows10CompliancePolicy](/graph/api/resources/intune-deviceconfig-windows10compliancepolicy?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавлен **userRightsAccessCredentialManagerAsTrustedCaller**, **userRightsAllowAccessFromNetwork**, **userRightsBlockAccessFromNetwork**, **userRightsActAsPartOfTheOperatingSystem**, **userRightsLocalLogOn **, **userRightsBackupData**, **userRightsChangeSystemTime**, **userRightsCreateGlobalObjects**, **userRightsCreatePageFile**, **userRightsCreatePermanentSharedObjects**, ** userRightsCreateSymbolicLinks**, **userRightsCreateToken**, **userRightsDebugPrograms**, **userRightsRemoteDesktopServicesLogOn**, **userRightsDelegation**, **userRightsGenerateSecurityAudits **, **userRightsImpersonateClient**, **userRightsIncreaseSchedulingPriority**, **userRightsLoadUnloadDrivers**, **userRightsLockMemory**, **userRightsManageAuditingAndSecurityLogs**, ** userRightsManageVolumes**, **userRightsModifyFirmwareEnvironment**, **userRightsModifyObjectLabels**, **userRightsProfileSingleProcess**, **userRightsRemoteShutdown**, **userRightsRestoreData **, **userRightsTakeOwnership**, **userRightsRegisterProcessAsService**, **localSecurityOptionsMinimumSessionSecurityForNtlmSspBasedClients**, ** localSecurityOptionsMinimumSessionSecurityForNtlmSspBasedServers**, **lanManagerAuthenticationLevel** и **lanManagerWorkstationEnableInsecureGuestLogons** свойства [ windows10EndpointProtectionConfiguration](/graph/api/resources/intune-deviceconfig-windows10endpointprotectionconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [windows10GeneralConfiguration](/graph/api/resources/intune-deviceconfig-windows10generalconfiguration?view=graph-rest-beta) добавлен свойства **passwordMinimumAgeInDays**, **tenantLockdownRequireNetworkDuringOutOfBoxExperience** и **dataProtectionBlockDirectMemoryAccess**|
|Дополнение|Бета|Добавить свойство **extendedKeyUsages** [windows10PkcsCertificateProfile](/graph/api/resources/intune-deviceconfig-windows10pkcscertificateprofile?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [windows10VpnConfiguration](/graph/api/resources/intune-deviceconfig-windows10vpnconfiguration?view=graph-rest-beta) добавлен свойства **enableDnsRegistration** и **dnsSuffixes**|
|Дополнение|Бета|Добавить свойство **customSubjectAlternativeNames** [windows81CertificateProfileBase](/graph/api/resources/intune-deviceconfig-windows81certificateprofilebase?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [windowsAutopilotDeploymentProfile](/graph/api/resources/intune-enrollment-windowsautopilotdeploymentprofile?view=graph-rest-beta) добавлен свойства **extractHardwareHash** и **deviceNameTemplate**|
|Дополнение|Бета|Сущность [windowsAutopilotDeviceIdentity](/graph/api/resources/intune-enrollment-windowsautopilotdeviceidentity?view=graph-rest-beta) добавлен свойства **addressableUserName** и **userPrincipalName**|
|Дополнение|Бета|Добавить свойство **threatState** [windowsDeviceMalwareState](/graph/api/resources/intune-devices-windowsdevicemalwarestate?view=graph-rest-beta) сущности|
|Дополнение|Бета|Добавлен **qualityUpdatesPauseStartDateTime**, **featureUpdatesPauseStartDateTime**, **featureUpdatesRollbackWindowInDays**, **qualityUpdatesWillBeRolledBack**, **featureUpdatesWillBeRolledBack**, ** qualityUpdatesRollbackStartDateTime** и **featureUpdatesRollbackStartDateTime** свойства для сущности [windowsUpdateForBusinessConfiguration](/graph/api/resources/intune-deviceconfig-windowsupdateforbusinessconfiguration?view=graph-rest-beta)|
|Дополнение|Бета|Добавить свойство **trustedServerCertificateNames** [windowsWifiEnterpriseEAPConfiguration](/graph/api/resources/intune-deviceconfig-windowswifienterpriseeapconfiguration?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [depOnboardingSetting](/graph/api/resources/intune-enrollment-deponboardingsetting?view=graph-rest-beta) добавлен **defaultIosEnrollmentProfile**, **defaultMacOsEnrollmentProfile**, **enrollmentProfiles** и **importedAppleDeviceIdentities** свойства навигации|
|Дополнение|Бета|Добавить свойство навигации **roleScopeTags** [deviceAndAppManagementRoleAssignment](/graph/api/resources/intune-rbac-deviceandappmanagementroleassignment?view=graph-rest-beta) сущности|
|Дополнение|Бета|Сущность [deviceManagement](/graph/api/resources/intune-androidforwork-devicemanagement?view=graph-rest-beta) добавлен **advancedThreatProtectionOnboardingStateSummary**, **roleScopeTags** и **importedWindowsAutopilotDeviceIdentityUploads** свойства навигации|
|Дополнение|Бета|Добавить свойство **supportedDeviceTypes** [mobileAppIntentAndStateDetail](/graph/api/resources/intune-troubleshooting-mobileappintentandstatedetail?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавить свойство **hideEscapeLink** [outOfBoxExperienceSettings](/graph/api/resources/intune-enrollment-outofboxexperiencesettings?view=graph-rest-beta) сложный тип|
|Дополнение|Бета|Добавить элементы **zscalerPrivateAccess**, **f5Access2018**, **citrixSso** и **paloAltoGlobalProtectV2** тип перечисления [appleVpnConnectionType](/graph/api/resources/intune-deviceconfig-applevpnconnectiontype?view=graph-rest-beta)|
|Дополнение|Бета|Добавлен элемент **userAbandonment** тип перечисления [deviceEnrollmentFailureReason](/graph/api/resources/intune-troubleshooting-deviceenrollmentfailurereason?view=graph-rest-beta)|
|Дополнение|Бета|Добавлен элемент **Заблокированные** тип перечисления [enrollmentState](/graph/api/resources/intune-enrollment-enrollmentstate?view=graph-rest-beta)|
|Дополнение|Бета|Добавлен элемент **microsoft365ManagedMdm** тип перечисления [managementAgentType](/graph/api/resources/intune-devices-managementagenttype?view=graph-rest-beta)|
|Дополнение|Бета|Добавлен элемент **domainNameService** тип перечисления [subjectAlternativeNameType](/graph/api/resources/intune-deviceconfig-subjectalternativenametype?view=graph-rest-beta)|
|Дополнение|Бета|Добавлена **wpa2Personal** и **wpa2Enterprise** члены тип перечисления [wiFiSecurityType](/graph/api/resources/intune-deviceconfig-wifisecuritytype?view=graph-rest-beta)|
|Дополнение|Бета|Добавить элементы **enterpriseUnwantedSoftware**, **ransom** и **hipsRule** тип перечисления [windowsMalwareCategory](/graph/api/resources/intune-devices-windowsmalwarecategory?view=graph-rest-beta)|

### <a name="outlook-calendar"></a>Календарь Outlook

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Дополнение | Бета-версия | Добавлена [getSchedule](/graph/api/calendar-getschedule?view=graph-rest-beta) действие и [freeBusyError](/graph/api/resources/freebusyerror?view=graph-rest-beta), [scheduleInformation](/graph/api/resources/scheduleinformation?view=graph-rest-beta)и [scheduleItem](/graph/api/resources/scheduleitem?view=graph-rest-beta) сложные типы для поддержки [получения сведений о доступности, сведения о доступности для пользователей, списков рассылки и ресурсы в течение заданного периода времени](outlook-get-free-busy-schedule.md). |

### <a name="outlook-mail"></a>Почта Outlook

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Дополнение        | v1.0        | Добавлена поддержка [getMailTips](/graph/api/user-getmailtips?view=graph-rest-1.0) действие, которое необходимо получить любые подсказок для получателей. Добавлены следующие ресурсы: [automaticRepliesMailTips](/graph/api/resources/automaticrepliesmailtips?view=graph-rest-1.0), [подсказки](/graph/api/resources/mailtips?view=graph-rest-1.0), [mailTipsError](/graph/api/resources/mailtipserror?view=graph-rest-1.0). |

### <a name="reports-apis"></a>API отчетов
| Тип изменения | Version | Описание                              |
|:------------|:--------|:-----------------------------------------|
| Дополнение    | v1.0    | Добавить свойство **Активируется на компьютере общих** [getoffice365activationsuserdetail](/graph/api/reportroot-getoffice365activationsuserdetail?view=graph-rest-1.0). |
| Дополнение    | v1.0    | Добавить свойство **Общих активации компьютера** в [getoffice365activationsusercounts](/graph/api/reportroot-getoffice365activationsusercounts?view=graph-rest-1.0). |

### <a name="security-apis"></a>API безопасности

| **Тип изменения** | **Версия** | **Описание**              |
| :-------------- | :---------- | :--------------------------------------- |
| Дополнение        | Бета       | Добавлена **activityGroupName**, **cloudAppStates**, **надежности**и свойства **registryKeyStates** на [оповещение](/graph/api/resources/alert?view=graph-rest-beta ). |
|Удаление|Бета| Удаленные **activityGroupStates**, **applicationStates**, **malwareWasRunning**, **riskScore** и **Тип** свойства в [оповещении](/graph/api/resources/alert?view=graph-rest-beta ). |
|Изменение|Бета| Изменен тип **комментариев** из `String` для `String collection`и изменить тип **уровень серьезности** от `String` перечисление [alertSeverity](/graph/api/resources/alertseverityenumtype?view=graph-rest-beta) в [оповещении](/graph/api/resources/alert?view=graph-rest-beta). |
...
| Дополнение        | Бета       | Добавить свойство **fileHash** [fileSecurityState](/graph/api/resources/filesecuritystate?view=graph-rest-beta ).|
|Удаление|Бета| Удаленные **authenticodeHash256** и **sha256** свойства из [fileSecurityState](/graph/api/resources/filesecuritystate?view=graph-rest-beta). |
| Дополнение | Бета | Добавлена **свойством** [hostSecurityState](/graph/api/resources/hostsecuritystate?view=graph-rest-beta).|
| Дополнение | Бета | Добавлена **Категория** **семейства**свойств и **wasRunning** для [malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta).|
|Удаление|Бета| Свойство **псевдонимы** удален из [malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta). |
|Изменение|Бета| Переместить свойство **malwareWasRunning** в [оповещении](/graph/api/resources/alert?view=graph-rest-beta ) [malwareState](/graph/api/resources/malwarestate?view=graph-rest-beta) и переименован в **wasRunning**. |
| Дополнение        | Бета       | Добавлены **имя_приложения**, **destinationDomain**, **направление**, **domainRegisteredDateTime**, **localDnsName**, **natDestinationAddress**, **natDestinationPort**, **natSourceAddress**, ** natSourcePort**, **riskScore**, **состояние**и **urlParameters** свойства [networkConnection](/graph/api/resources/networkconnection?view=graph-rest-beta ).|
|Изменение|Бета| Свойство **uri** изменено на **destinationUrl** в [networkConnection](/graph/api/resources/networkconnection?view=graph-rest-beta ). |
| Дополнение        | Бета       | Добавить свойство **fileHash** к [процессу](/graph/api/resources/process?view=graph-rest-beta ).|
|Удаление|Бета| Удаленные **authenticodeHash256** и **sha256** свойства из [процесса](/graph/api/resources/process?view=graph-rest-beta ). |
| Дополнение        | Бета       | Добавлена **aadUserId**, **emailRole**, **isVpn**и свойств, **logonIp** [userSecurityState](/graph/api/resources/usersecuritystate?view=graph-rest-beta).|
|Изменение|Бета| Теперь свойство **logonIpAddress** **logonIp** в [userSecurityState](/graph/api/resources/usersecuritystate?view=graph-rest-beta). |
| Дополнение        | Бета       | Добавить свойство **wasRunning** [vulnerabilityState](/graph/api/resources/vulnerabilitystate?view=graph-rest-beta).|
|Удаление|Бета| Свойство **name** удален из [vulnerabilityState](/graph/api/resources/vulnerabilitystate?view=graph-rest-beta). |
...
| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
|Изменение|Бета|Обновленный [chatmessage](/graph/api/resources/chatmessage?view=graph-rest-beta) ресурсов|
...
| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Дополнение | Бета-версия | Добавить свойство **о ходе выполнения** в [sychronizationStatus](/graph/api/resources/synchronization-synchronizationstatus?view=graph-rest-beta) , чтобы разрешить клиентам для проверки хода выполнения задания синхронизации.|
...
### <a name="identity-and-access-apis"></a>Удостоверениями и доступом API-интерфейсы

| **Тип изменения** | **Версия**   | **Описание**                          |
| :-------------- | :------------ | :--------------------------------------- |
| Дополнение | Бета | Добавлена функция [дается обзор доступа](/graph/api/resources/accessreviews-root?view=graph-rest-beta) [Azure AD](/graph/api/resources/azure-ad-overview?view=graph-rest-beta). |
...
| Дополнение | v1.0 | Добавлена **ageGroup**, **legalAgeGroupClassification**и **ConsentRequiredForMinor** свойства для [пользователя](/graph/api/resources/user?view=graph-rest-1.0) ресурса
...
| Дополнение | Бета | Добавлена сущность [governanceResource](/graph/api/resources/governanceresource?view=graph-rest-beta), а также следующие методы и действия: <br> [Список](/graph/api/governanceresource-list?view=graph-rest-beta) <br> [Получение](/graph/api/governanceresource-get?view=graph-rest-beta)<br>|
...
|Удаление|Бета|Удалена функция exportMobileConfig](/graph/api/intune_corpenrollment_enrollmentprofile_exportmobileconfig?view=graph-rest-beta) на [enrollmentProfile](/graph/api/resources/intune-corpenrollment-enrollmentprofile?view=graph-rest-beta) |
...
|Добавление|1.0|Добавлены новые типы перечисления:<br/>[managedDeviceOwnerType](/graph/api/resources/intune-devices-manageddeviceownertype?view=graph-rest-1.0).<br/>|
...
| Дополнение        | Бета-версия          | Добавлены целевые параметры *siteCollectionId* и *siteId* для операций копирования. Пример: [CopyNotebook](/graph/api/notebook-copynotebook?view=graph-rest-1.0). |
...
| Изменение      | Бета    | Добавлены свойство навигации **eBook** для объекта [eBookGroupAssignment](/graph/api/resources/intune-books-ebookgroupassignment?view=graph-rest-beta). |
| Изменение      | Бета    | Добавлено свойство навигации **windowsProtectionState** для объекта [managedDevice](/graph/api/resources/intune-devices-manageddevice?view=graph-rest-beta). |
...
| **Тип изменения** | **Версия** | **Описание**                          |
| :-------------- | :---------- | :--------------------------------------- |
| Дополнение        | Бета-версия        | Добавление данных приложения в Microsoft Graph с помощью [расширений схемы](extensibility-overview.md#schema-extensions)  поддерживается для следующих ресурсов:<br/>administrative unit;<br/>calendar event;<br/>device;<br/>group;<br/>message<br/>organization;<br/>personal contact;<br/>post;<br/>user.<br/>Пример приведен в следующей статье:<br/>[Добавление пользовательских данных в ресурсы group с помощью расширений схемы (предварительная версия)](extensibility-schema-groups.md). |
| Дополнение        | Бета-версия        | Предоставлен альтернативный способ создания определения расширения схемы без подтвержденного личного домена .com. Подробности см. в разделе [Расширения схемы](extensibility-overview.md#schema-extensions). |
...
| Дополнение        | v1.0        | Добавлен метод **Search** для drive, который позволяет искать элементы на диске, а также общие элементы. |
...
| Дополнение        | Бета-версия        | Добавлена поддержка [getMailTips](/graph/api/user-getmailtips?view=graph-rest-beta) действие, которое необходимо получить любые подсказок для получателей. Добавлены следующие ресурсы: [automaticRepliesMailTips](/graph/api/resources/automaticrepliesmailtips?view=graph-rest-beta), [подсказки](/graph/api/resources/mailtips?view=graph-rest-beta), [mailTipsError](/graph/api/resources/mailtipserror?view=graph-rest-beta). |
...
|**Тип изменения**|**Версия**|**Описание**|
|:--------------|:-----------|:--------------|
|Существенные изменения|Бета-версия|Изменения findMeetingTimes API. Дополнительные сведения см в блоге [API findMeetingTimes Microsoft Graph обновления](https://dev.office.com/microsoft-graph-findmeetingtimes-api-update) . Это изменение действует 19 мая 2016.
...
| Дополнение        | Бета-версия          | Теперь Azure Active Directory используется в качестве резервного ресурса для запросов /beta/users/id/photo, когда приложению Outlook не удается обслужить запрос (например, когда у пользователя нет лицензии на почтовый ящик или у клиента нет подписки на Exchange Online). ПРИМЕЧАНИЕ. Этот резервный ресурс доступен и для запросов GET, и для запросов PATCH. |
