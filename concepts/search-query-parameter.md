...
Ресурсы Azure AD и их связи, которые являются производными от [directoryObject](/graph/api/resources/directoryobject), поддерживают параметр запроса `$search` только в расширенных запросах. Поиск использует разметку, которая выполняется путем извлечения слов из входной и выходной строки, использования пробелов, чисел, разного регистра и символов для разделения слов, как показано ниже.
