...
## <a name="delegated-permissions-application-permissions-and-effective-permissions"></a>Разрешения приложений, делегированные разрешения и действующие разрешения
В Microsoft Graph есть два типа разрешений: **разрешения приложений** и **делегированные разрешения**. 
...
_Действующие разрешения_ — это разрешения, которые приложение получает при отправке запросов в Microsoft Graph. Важно понимать разницу между разрешениями приложений или делегированными разрешениями, предоставленными приложению, и его действующими разрешениями при вызовах Microsoft Graph.
...
- В случае делегированных разрешений _действующими разрешениями_ приложения будут такие, которые включены одновременно как в делегированные (предоставленные приложению путем согласия), так и в назначенные вошедшему пользователю. У вашего приложения никогда не может быть больше привилегий, чем у вошедшего пользователя. В организациях привилегии вошедшего пользователя могут определяться политикой или членством в одной или нескольких ролях администратора. Дополнительные сведения о ролях администратора см. в статье [Назначение ролей администратора в Azure Active Directory](https://docs.microsoft.com/en-us/azure/active-directory/active-directory-assign-admin-roles).
  ...
  Допустим, приложению было предоставлено делегированное разрешение _User.ReadWrite.All_. Обычно это разрешение позволяет приложению просматривать и обновлять профиль любого пользователя в организации. Если вошедший пользователь является глобальным администратором, приложение сможет обновлять профиль любого пользователя в организации. Однако если вошедший пользователь не является администратором, то приложение сможет обновлять только профиль вошедшего пользователя. Оно не сможет обновлять профили других пользователей в организации, так как у пользователя, от имени которого может действовать приложение, нет таких привилегий.
  
- В случае разрешений приложений все предусмотренные ими привилегии будут включены в _действующие разрешения_ вашего приложения. Например, приложение с разрешением _User.ReadWrite.All_ может обновлять профиль любого пользователя в организации. 
...
Функции поиска пользователей и групп позволяют приложению найти любого пользователя и любую группу в каталоге организации с помощью запросов для набора ресурсов `/users` или `/groups` (например, `https://graph.microsoft.com/v1.0/users`). Эта возможность есть как у администраторов, так и у обычных пользователей. Однако ее нет у гостевых пользователей. Если пользователь вошел в приложение как гость, то в зависимости от предоставленных приложению разрешений оно может просматривать профиль определенного пользователя или определенной группы (например, `https://graph.microsoft.com/v1.0/users/241f22af-f634-44c0-9a15-c8cd2cea5531`). Тем не менее оно не может отправлять набору ресурсов `/users` или `/groups` запросы, которые могут возвращать несколько ресурсов. При наличии подходящих разрешений приложение может просматривать профили пользователей и групп, полученные с использованием ссылок в свойствах навигации (например, `/users/{id}/directReports` или `/groups/{id}/members`).
...
Это разрешение поддерживается только в приложениях, предназначенных для организаций.
...
## <a name="microsoft-intune-device-management-permissions"></a>Разрешения для управления устройствами Microsoft Intune
...
|   Разрешение    |  Отображаемая строка   |  Описание | Необходимость в согласии администратора |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _Files.Read_ |    Чтение файлов пользователя, а также файлов, которыми с ним поделились | Позволяет приложению считывать файлы вошедшего пользователя, а также файлы, которыми с последним поделились.| Нет |
...
| _Files.ReadWrite_ |   Полный доступ к файлам пользователя и к файлам, которыми с ним поделились | Позволяет приложению считывать, создавать, обновлять и удалять файлы вошедшего пользователя, а также файлы, которыми с последним поделились. | Нет |
...
|   Разрешение    |  Отображаемая строка   |  Описание | Необходимость в согласии администратора |
|:-----------------------------|:-----------------------------------------|:-----------------|:-----------------|
| _Files.Read.All_ | Чтение всех файлов, доступных пользователю (предварительная версия) | **Ограниченная поддержка в Microsoft Graph** <br/> Позволяет приложению считывать все файлы во всех семействах веб-сайтов в случаях, когда вход пользователя не предусмотрен (предварительная версия). | Да |
| _Files.ReadWrite.All_ | Полный доступ ко всем файлам, к которым может получать доступ пользователь (предварительная версия) | **Ограниченная поддержка в Microsoft Graph** <br/> Позволяет приложению считывать, создавать, обновлять и удалять все файлы во всех семействах веб-сайтов в случаях, когда вход пользователя не предусмотрен (предварительная версия). | Да |

### <a name="remarks"></a>Замечания

#### <a name="support-for-permissions-in-preview"></a>Поддержка разрешений в предварительной версии
**Делегированные разрешения**: 

- _Files.Read.Selected_ и _Files.ReadWrite.Selected_ пока не поддерживаются в Microsoft Graph. Для обеспечения обратной совместимости эти разрешения можно настраивать и включать в запросы на авторизацию, но Microsoft Graph не предоставляет никаких привилегий. Поддержку этих разрешений планируется добавить в будущем. 
- Files.ReadWrite.AppFolder_ поддерживается только для учетных записей Майкрософт. 

**Разрешения приложений**: 

- _Files.Read.All_ и _Files.ReadWrite.All_ в настоящее время не полностью поддерживаются в Microsoft Graph. Однако с этими разрешениями предоставляются некоторые привилегии. Полная поддержка ожидается в скором времени. 
...
Для разрешений приложений действуют некоторые ограничения на поддерживаемые API. Дополнительные сведения см. в статье, посвященной [известным проблемам](../concepts/known_issues.md).
...
### <a name="example-usage"></a>Примеры использования
#### <a name="delegated-and-application"></a>Разрешения приложений и делегированные разрешения
Следующие сценарии поддерживаются как для разрешений приложений, так и для делегированных разрешений:

* чтение всех событий, созданных для всех пользователей в клиенте (`GET /beta/identityRiskEvents`);
...
| _Mail.ReadWrite_ |    Доступ для чтения и записи к почте пользователя | Позволяет приложению создавать, читать, обновлять и удалять электронную почту в почтовых ящиках пользователя. Сюда не входит разрешение на отправку почты.| Нет |
...
| _offline_access_ |    Доступ к данным пользователя в любое время (предварительная версия) | Позволяет приложению считывать и обновлять данные пользователя, даже если он в настоящее время не использует приложение.| Нет |
| _openid_ |    Вход пользователей (предварительная версия) | Пользователи смогут входить в приложение с помощью своей рабочей или учебной учетной записи, а приложение сможет просматривать основные данные профилей пользователей.| Нет |
...
| _User.ReadWrite_ |    Доступ для чтения и записи к профилю пользователя | Разрешает приложению читать ваш профиль. Позволяет приложению обновлять данные профиля от вашего имени. | Нет |
...
- userPrincipalName.
 ...
 Делегированные разрешения _User.ReadWrite_ и _User.Readwrite.All_ позволяют приложению обновлять следующие свойства профиля для рабочих и учебных учетных записей:
...
С помощью разрешения _User.ReadWrite.All_ приложение может обновлять все объявленные свойства рабочих и учетных записей, кроме пароля.
