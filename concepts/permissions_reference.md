...
| **Задачи приложения, связанные с группой**  |  **Необходимые разрешения** |  **Строки разрешений** |
|:-------------------------------|:---------------------|:---------------|
| Приложению нужно разрешение считывать основные сведения о группе, такие как отображаемое имя и изображение (например, для отображения при выборе групп).  | _Group.Read.All_  | Считывание всех групп|
| Приложению нужно разрешение считывать все содержимое во всех общедоступных группах Office 365, в том числе файлы и беседы.  Ему также нужно разрешение отображать членов группы.  |  _Group.Read.All_, _User.ReadBasic.All_ | "Чтение всех групп", "Чтение базовых профилей всех пользователей" |
| Приложению нужно разрешение считывать и записывать все содержимое во всех общедоступных группах Office 365, в том числе файлы и беседы.  Кроме того, ему нужно разрешение на отображение и обновление членов группы (если вошедший пользователь является владельцем группы).  |     _Group.ReadWrite.All_, _User.ReadBasic.All_ |  "Чтение и запись всех групп", "Чтение базовых профилей всех пользователей" |
| Приложению нужно разрешение на то, чтобы позволить пользователю присоединяться к общедоступным группам Office 365. Оно позволит пользователю найти определенную группу, выбрать группу из списка, чтобы затем присоединиться к ней. Пользователь добавляется к любой выбранной им группе.    |   _Group.ReadWrite.All_, _User.Read_ | "Чтение и запись всех групп", "Вход в систему и чтение профиля пользователя" |
| Приложению нужно разрешение на создание группы с помощью Microsoft Graph. |    _Group.ReadWrite.All_ | Чтение и запись всех групп|
