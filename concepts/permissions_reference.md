...
> **Примечание**. В  личных учетных записяхFiles.Read и Files.ReadWrite также предоставляют доступ к файлам, используемым совместно с авторизовавшимся пользователем. 
...
| _Group.ReadWrite.All_ | Чтение и запись всех групп | Позволяет приложениям создавать группы, считывать и обновлять информацию о членстве в группах, а также удалять группы. Все эти операции могут выполняться приложением без авторизации пользователя. > **Примечание.**  Не все групповые API поддерживают доступ с помощью разрешений только для приложений. Примеры см. в разделе [Известные проблемы](../concepts/known_issues.md) .| Да |
...
> **Примечание.** При этом также необходимо разрешение _User.ReadBasic.All_ для чтения пользователя, чтобы добавить его как члена группы.
