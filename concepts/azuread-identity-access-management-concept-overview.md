...
Вы можете [управлять устройствами](https://docs.microsoft.com/en-us/azure/active-directory/device-management-introduction), зарегистрированными в организации. Устройства (ноутбуки, настольные компьютеры, планшеты и мобильные телефоны) зарегистрированы на пользователей. Устройства обычно создаются в облаке с помощью службы регистрации устройств или Microsoft Intune. Они используются в политиках условного доступа для многофакторной аутентификации.
...
### <a name="monitor-identity-risks-preview"></a>Риски, связанные с идентификатором монитор (Предварительная версия)
...
Azure AD использует адаптивные алгоритмы машинного обучения и эвристический анализ, чтобы определять аномалии, указывающие на возможную компрометацию учетных записей. Используя эти данные, Azure AD защиту защищает пользователей с помощью политик риска условного доступа и создает отчеты и оповещения о его обнаружения.

В настоящее время Microsoft Graph предоставляет легкий доступ к клиентам P2 Azure AD Premium [события риска запросов выявлено системой защиты удостоверения](/graph/api/resources/identityprotection-root?view=graph-rest-beta), включая события риска тип, уровень серьезности, дата, время, расположения, задействованной пользователя и многое другое. Клиенты затем можно использовать эти события в SIEM систем и безопасность приложений.
...
### <a name="manage-user-access-reviews-preview"></a>Управление Просмотр доступа пользователей (Предварительная версия)

Вы можете настроить доступ обзоры членство в группах и доступ к приложениям. [Дается обзор доступа](/graph/api/resources/accessreviews-root?view=graph-rest-beta) , представленные в P2 Azure AD Premium.

## <a name="api-reference"></a>Справочные материалы по API

Нужна Справка по API для этой службы?

- [Azure AD Управление удостоверениями и доступом API в Microsoft Graph версии 1.0](/graph/api/resources/azure-ad-overview?view=graph-rest-1.0)
- [Azure AD Управление удостоверениями и доступом API в бета-версии Microsoft Graph](/graph/api/resources/azure-ad-overview?view=graph-rest-beta)
