...
---
# <a name="use-outlook-to-organize-or-attend-meetings-online-preview"></a>Организация и посещение собраний по сети с помощью Outlook (предварительная версия)
...
Найдите свойства **allowedOnlineMeetingProviders** и **defaultOnlineMeetingProvider**, чтобы проверить, поддерживает ли [календарь](/graph/api/resources/calendar?view=graph-rest-beta) Outlook поставщика собраний по сети. В приведенном ниже примере календарь по умолчанию выполнившего вход пользователя поддерживает двух поставщиков, Microsoft Teams и Skype для бизнеса, и использует Microsoft Teams в качестве поставщика собраний по сети по умолчанию. 

### <a name="example-find-whether-a-calendar-supports-any-online-meeting-provider"></a>Пример. Узнайте, поддерживает ли календарь поставщика собраний по сети.
...
## <a name="create-and-enable-a-meeting-online"></a>Создание и включение собрания по сети

Вы можете создать собрание и разрешить участникам присоединиться к собранию по сети, задав значение `true` для **isOnlineMeeting** и изменив значение **onlineMeetingProvider** на одного из поставщиков, поддерживаемых родительским календарем. В приведенном ниже примере показано, как создать собрание в календаре по умолчанию выполнившего вход пользователя и разрешить участникам присоединиться к собранию с помощью Microsoft Teams. Отклик включает **событие** со сведениями о собрании по сети, указанными в свойстве **onlineMeeting**.

> [!NOTE]
> После включения собрания по сети Microsoft Graph задаст сведения о собрании в **onlineMeeting**. В дальнейшем вы не сможете изменить свойство **onlineMeetingProvider** или задать значение `false` для **isOnlineMeeting**, чтобы отключить собрание по сети. 

### <a name="example-create-and-make-meeting-available-as-an-online-meeting"></a>Пример. Создайте собрание и сделайте его доступным по сети
...
Участники и организаторы могут использовать свойство **isOnlineMeeting**, чтобы проверить, включено ли [событие](/graph/api/resources/event?view=graph-rest-beta) для онлайн-участия. Они могут использовать свойство **onlineMeetingProvider** для определения поставщика собрания, а **onlineMeeting** — для получения сведений о подключении, в том числе **joinUrl**. 
...
### <a name="example-get-online-meeting-information"></a>Пример. Получите сведения о собрании по сети
...
## <a name="update-a-meeting-to-enable-it-online"></a>Обновление собрания для включения по сети
Вы можете изменить существующее **событие**, чтобы сделать его доступным в качестве собрания по сети, задав значение `true` для **isOnlineMeeting** и изменив значение **onlineMeetingProvider** на одного из поставщиков собраний по сети, поддерживаемых родительским календарем. Отклик включает обновленное **событие** с соответствующими сведениями о собрании по сети, указанными в свойстве **onlineMeeting**.

> [!NOTE]
> После включения собрания по сети Microsoft Graph задаст сведения о собрании в **onlineMeeting**. В дальнейшем вы не сможете изменить свойство **onlineMeetingProvider** или задать значение `false` для **isOnlineMeeting**, чтобы отключить собрание по сети. 

### <a name="example-update-a-meeting-to-make-it-available-as-an-online-meeting"></a>Пример. Обновите собрание, чтобы сделать его доступным по сети.
