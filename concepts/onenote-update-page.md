...
> [!NOTE]
> При обновлении изображения на странице OneNote нельзя использовать ссылки www. Служба не будет пытаться скачать случайные ресурсы. Вместо этого образ должен быть частью запроса либо по URL-адресу данных изображения, либо по имени части составного запроса.
...
> [!NOTE]
> API удаляет все значения **идентификаторов** , определенные во входном [HTML-коде](onenote-input-output-html.md) запросов create-page и update-page.
...
> [!NOTE]
> Идентификаторы пользователей можно получить, выполнив запрос `https://graph.microsoft.com/v1.0/users`GET.
