...
либо на ресурс верхнего уровня: `/me/messages`, `/me/contacts`, `/me/events`, `users`, `groups`, `/communications/callRecords` или `/communications/presences`,

либо на определенный экземпляр ресурса: `users/{id}`, `groups/{id}`, `groups/{id}/conversations`, `sites/{site-id}/lists/{list-id}`
...
### <a name="teams-resource-limitations-preview"></a>Ограничения ресурсов Teams (предварительная версия)

Можно использовать одну действующую подписку на канал или чат для приложения.

Доступно не более 10000 активных подписок на организацию в беседах и каналах для всех приложений.
...
1. Клиенту необходимо правильно расшифровать `validationToken`, предоставленный на предыдущем этапе, и избегать любого HTML или JavaScript.
...
1. В течение 10 секунд первого этапа клиент должен предоставить ответ со следующими характеристиками:
    ...
    - Основной текст, включающий _расшифрованный_ маркер проверки.
