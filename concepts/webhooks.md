...
Если вы решите использовать **userPrincipalName** (UPN) на пути к ресурсу при оформлении подписки на ресурсы Outlook, такие как **сообщения**, **события** или **контакты**, запрос может быть не выполнен, если UPN содержит апостроф. Используйте ИД пользователей вместо их UPN, чтобы избежать этой проблемы. Например, вместо пути к ресурсу:
...
> **Примечание.** Если клиент имеет несколько подписок, которые отслеживают один ресурс и используют один URL-адрес уведомлений, Microsoft Graph может отправлять несколько уведомлений об изменениях, соответствующих разным подпискам, каждое из которых показывает идентификатор подписки. Уведомления об изменениях из запроса `POST` могут принадлежать разным подпискам.
