...
description: Отправка уведомлений о канале активности пользователям в Microsoft Teams с Teams приложения и microsoft graph.
author: RamjotSingh
...
Канал Microsoft Teams позволяет пользователям делить элементы, которые требуют внимания, оповещая их об изменениях. Вы можете использовать API уведомлений о подаче уведомлений о действиях в Microsoft Graph, чтобы распространить эту функцию на приложения. Это позволяет приложениям предоставлять более богатые впечатления и лучше привлекать пользователей, помогая поддерживать их в курсе изменений в инструментах и рабочего процессах, которые они используют.
...
API-API-каналов активности работают [с Teams приложением](/microsoftteams/platform/overview). Ниже приводится требование к отправке уведомлений о ленте действий:

- В манифесте Teams приложения должен быть добавлен ID приложения Azure AD в разделе`webApplicationInfo`. Подробные сведения см. в [схеме манифеста](/microsoftteams/platform/resources/schema/manifest-schema).
- Типы действий должны быть объявлены в разделе `activities` . Подробные сведения см. в [схеме манифеста](/microsoftteams/platform/resources/schema/manifest-schema).
- Приложение Teams должно быть установлено для получателя лично или в группе или в чате, [в который они](/graph/api/resources/chat?preserve-view=true) входит.[](/graph/api/resources/team?preserve-view=true) Дополнительные сведения см. [в Teams установки приложения](/graph/api/resources/teamsappinstallation?preserve-view=true).
...
> **Примечание:** Вы можете получить ошибку, если несколько Teams приложений в одной области (команда, чат или пользователь) используют одно и то же приложение Azure AD. Убедитесь, что вы используете уникальные приложения Azure AD.
...
>**Примечание:** `actor` это специальный параметр, который всегда принимает имя вызываемого. В делегировании `actor` звонков имя пользователя. В вызовах только для приложений оно принимает имя Teams приложения.
...
В этом примере показано, как можно отправить уведомление о ленте действий для новой задачи, созданной в чате. В этом случае Teams должно быть установлено в чате с ИД `chatId` `569363e2-4e49-4661-87f2-16f245c5d66a`, а пользователь должен быть частью чата.
...
Как повествуют в предыдущих примерах, вы можете ссылаться на различные аспекты группы или чата. Однако, если вы хотите связаться с аспектом, который не является частью команды или не представлен Microsoft Graph, или если вы хотите настроить имя, `topic` `text` вы можете установить источник и передать в настраиваемом значении для него. Кроме того, `webUrl` требуется, когда вы используете источник `topic` в качестве `text`.

В Yammer, показанных ранее, используется настраиваемая тема, так как Yammer ресурсы Майкрософт не поддерживаются Graph.
...
Microsoft Teams пользователи могут настраивать уведомления, которые они видят в своей ленте, в качестве баннера и так далее. Также можно настроить уведомления, созданные с помощью API-каналов активности. Пользователи могут выбрать, как они уведомлены с помощью параметров в Microsoft Teams. Teams приложения будут отображаться в списке для выбора пользователя, как показано на следующем скриншоте.
...
`Conflict`Ошибки в основном возникают, когда несколько Teams приложений, установленных в одной области (команда, чат, пользователь и так далее) имеют один и тот же appId `webApplicationInfo` Azure AD в разделе манифеста. Когда это произойдет, вы получите ошибку, такую как `Found multiple applications with the same Azure AD App ID 'Your AzureAD AppId'.`. Убедитесь, что вы используете уникальные приложения Azure AD для уникальных Teams приложений. Обратите внимание, что можно установить одно Teams в нескольких сферах (например, team + user).

## <a name="see-also"></a>См. также

* [Наилучшие методы использования уведомлений Microsoft Teams каналов активности](teams-activity-feed-notifications-best-practices.md)
