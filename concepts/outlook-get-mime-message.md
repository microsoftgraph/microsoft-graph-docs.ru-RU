...
- Если сообщение вложено в элемент Outlook или запись в группе, вы можете добавить сегмент `$value` к операции get-attachment с этим элементом или записью в группе.

В любом случае приложению должны быть предоставлены [разрешения на доступ](permissions-reference.md#mail-permissions) к элементу Outlook или записи в группе, чтобы применить операцию get-message или get-attachment. 
...
- `Content-Disposition` — предоставляет сведения о вложении, например стиль его презентации (`inline` или `attachment`), имена файлов, а также даты создания и последнего изменения.
...
Вы можете получить представление сообщения в формате MIME, добавление сегмента `$value` при [получении сообщения](/graph/api/message-get?view=graph-rest-beta): 
