...
description: Создавайте интерфейсы, которые легко перекрестно перекрестно используют веб-канал действий и API-интерфейсы ретранслятора устройств, чтобы снизить нагрузку на пользователей и обеспечить использование приложений.
...
* Чтение и запись действий пользователей со всех платформ при помощи [API веб-каналов активности](/graph/api/resources/activity-feed-api-overview)
...
Соберите идентификаторы всех приложений для определенных платформ, которые будут использовать [API Project Rome](/graph/api/resources/project-rome-overview).
...
> [!NOTE]
> Если вы используете Firebase для отправки push-уведомлений на устройства iOS с использованием учетных данных Android, необходимо указать учетные данные APNs как часть конфигурации приложения на нескольких устройствах. 
...
Вы можете найти существующие идентификаторы приложений или идентификаторы клиентов или подготовить новые, войдя на портал регистрации [приложений с помощью](https://apps.dev.microsoft.com) учетной записи разработчика. После входа вы сможете просмотреть идентификатор приложения или клиента для любого приложения. Поддерживаются как идентификаторы Live SDK (шестнадцатеричные значения), так и конвергированные идентификаторы приложений (GUID). Используйте тип платформы msa при добавлении идентификаторов, используемых для включения поддержки учетной записи Майкрософт или Azure AD, как показано в предыдущем примере.  

> [!NOTE]
> Если вы создаете приложение, которое поддерживает Azure AD пользователей, и не используете конвергентный идентификатор приложения, выданный через портал регистрации [приложений, необходимо](https://apps.dev.microsoft.com) указать GUID для идентификатора приложения Azure. Для такого идентификатора также следует задать тип платформы msa. 
...
Если ваше приложение использует API веб-каналов активности в Microsoft Graph, то значение узла необходимо передавать в свойстве **activitySourceHost**. Дополнительные сведения см. в статье [Тип ресурса activity](/graph/api/resources/projectrome-activity).
...
## <a name="maintain-your-cross-device-app-configuration"></a>Поддержка конфигурации приложения для нескольких устройств
