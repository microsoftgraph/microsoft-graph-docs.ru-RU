...
description: Некоторые приложения вызывают Microsoft Graph от своего имени, а не от имени пользователя. Как правило, эти приложения представляют собой фоновые службы или управляющие программы, которые работают на сервере без выполнившего вход пользователя.
...
Чтобы настроить службу и получить маркер из конечной точки платформы удостоверений Майкрософт, выполните указанные ниже основные действия. Ваша служба может использовать маркер для вызова Microsoft Graph от своего имени.
...
Когда используется поток предоставления учетных данных клиента OAuth 2.0, приложение проходит аутентификацию непосредственно в конечной точке `/token` платформы удостоверений Майкрософт, используя идентификатор приложения, назначенный службой Azure AD, и секрет клиента, созданный вами на портале.

## <a name="2-configure-permissions-for-microsoft-graph"></a>2. Настройка разрешений для Microsoft Graph

Для приложений, вызывающих Microsoft Graph от своего имени, Microsoft Graph предоставляет **специальные разрешения**. (Microsoft Graph также может предоставлять делегированные разрешения для приложений, вызывающих Microsoft Graph от имени пользователя).

Предварительная настройка этих разрешений выполняется при регистрации приложения. Разрешения для приложения всегда требуют согласия администратора. Администратор может согласиться предоставить эти разрешения на [портале Azure](https://portal.azure.com) во время установки приложения в организации. Вы также можете создать в приложении интерфейс регистрации, где администраторы могут соглашаться на предоставление заданных вами разрешений. Когда согласие администратора фиксируется в Azure AD, приложение может запрашивать маркеры, не требуя повторного согласия. Дополнительные сведения о разрешениях, доступных в Microsoft Graph, см. в [справочнике по разрешениям](./permissions-reference.md).

Настроить разрешения для приложения можно на [портале Azure для регистрации приложений](https://go.microsoft.com/fwlink/?linkid=2083908). Откройте страницу **Разрешения API** для приложения, нажмите кнопку **Добавить разрешение**, выберите **Microsoft Graph**, а затем выберите нужные разрешения в разделе **Разрешения приложения**.

На приведенном ниже снимке экрана показано диалоговое окно **Выбор разрешений** с разрешениями приложения в Microsoft Graph.

:::image type="content" source="./images/auth-v2/v2-application-permissions.png" alt-text="Диалоговое окно &quot;Выбор разрешений&quot; с разрешениями приложения в Microsoft Graph." border="true":::

> [!IMPORTANT]
> 
> Настройте минимальный набор разрешений, необходимый приложению, чтобы повысить его безопасность. Дополнительные сведения см. в статье [Повышение безопасности с помощью принципа предоставления минимальных прав](/azure/active-directory/develop/secure-least-privileged-access).
...
> [!IMPORTANT]
> 
> При изменении настроенных разрешений необходимо получить повторное согласие администратора. Изменения, внесенные на портале регистрации приложений, не вступят в силу, пока их не подтвердит администратор клиента.
...
|:--------------|:------------|:------------
| tenant        | Обязательный    | Клиент каталога, у которого требуется запрашивать разрешения. Значение может быть представлено в виде GUID или в формате понятного имени. Если вы не знаете, к какому клиенту относится пользователь, и хотите, чтобы он мог войти в любой клиент, используйте значение `common`.
...
| redirect_uri  | Обязательный    | URI перенаправления, на который должен отправляться ответ для обработки приложением. Этот URI должен совпадать с одним из URI перенаправления, зарегистрированных на портале. Он должен быть закодирован как URL-адрес и может содержать дополнительные сегменты пути.
...
На снимке экрана ниже представлен пример диалогового окна предоставления согласия, которое Azure AD отображает для администратора.

:::image type="content" source="./images/auth-v2/admin-consent.png" alt-text="Диалоговое окно согласия администратора." border="true":::
...
| Параметр     | Условие | Описание 
|:--------------|:----------|:------------
| tenant        | Обязательный  | Клиент каталога, у которого требуется запрашивать разрешения. Значение может быть представлено в виде GUID или в формате понятного имени.
...
```
Успешный ответ выглядит следующим образом (некоторые заголовки были удалены):
...
- [Выбор поставщика проверки подлинности Microsoft Graph на основе сценария](/graph/sdks/choose-authentication-providers?tabs=CS)
- [Узнайте, как создать веб-приложение, вызывающее Microsoft Graph от своего имени](/azure/app-service/scenario-secure-app-access-microsoft-graph-as-app).
