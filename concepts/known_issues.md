...
# Известные проблемы с Microsoft Graph
...
### Разрешения для групп и Microsoft Teams
...
### Команды в Microsoft Teams (предварительная версия)
...
### Каналы Microsoft Teams (предварительная версия)
...
### Цепочки и сообщения чата Microsoft Teams (предварительная версия)
...
### Установка свойства allowExternalSenders
...
### Контакты организации доступны только в бета-версии

В настоящее время поддерживаются только личные контакты. Контакты организации не поддерживаются в версии `/v1.0`, но их можно найти в версии `/beta`.
...
### Доступ к контактам через папку контактов в бета-версии
...
### Параметр comment для создания черновика
...
### Отслеживание изменений не поддерживается
...
### Одновременное создание ресурса и открытого расширения
...
### Для каждого экземпляра ресурса разрешено не более 100 значений свойства расширения схемы
...
### Не поддерживаются вложенные пакеты
...
### Все отдельные запросы должны быть синхронными
...
### Не поддерживается диалоговая обработка
...
### URI должны быть относительными
...
## Приложения Cloud Solution Provider

<a id="csp-apps-must-use-azure-ad-endpoint" class="xliff"></a>

### Приложения CSP должны использовать конечную точку Azure AD
...
### В некоторых клиентах предоставленные приложениям CSP разрешения не работают
...
## Функции, доступные только в REST API Office 365 или API Graph Azure AD
