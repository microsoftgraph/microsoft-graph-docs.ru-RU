...
Microsoft Teams и группы Office 365 имеют схожие функции. Все API групп можно использовать с командами за исключением того, что в настоящее время нельзя создать команду с помощью API "Создать группу".  Эта возможность появится в будущих выпусках API.
