...
1. **Предостанав служба хранилища Azure подключение** имени **AzureStorageConnectionString** и выберите **Далее**.
1. Нажмите **Готово**.
...
1. Добавьте следующий код в **класс EmailMetricsController** . Они будут использоваться для подключения к учетной записи **служба хранилища Azure,** которая содержит экспортные данные.
...
1. Добавьте следующий метод в **класс EmailMetricsController** . При этом будут указаны все blobs  `ProcessBlobEmails()` в указанном контейнере служба хранилища Azure указанной учетной записи и отправляться каждый метод, добавленный на последнем шаге.
...
1. В **диалоговом** окантове Add View установите имя **View** **Index**, оставьте оставшиеся элементы управления входом значениям по умолчанию и выберите **Добавить**.
...
1. Это приложение будет смотреть на данные электронной почты для электронных писем, извлеченных в **учетную запись Azure Blob служба хранилища** и отображать общее число получателей от каждого отправитель.
...
1. В **диалоговом** окантове Add View установите следующие значения и оставьте оставшиеся элементы управления входом значениям по умолчанию и выберите **Добавить**.
