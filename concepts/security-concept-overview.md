...
Автоматически отправляйте индикаторы угроз в решения Майкрософт по обеспечению безопасности, чтобы включить действия `alert`, `block` или `allow`. Применяйте непосредственно API безопасности Microsoft Graph или воспользуйтесь преимуществом интеграции с ведущими платформами аналитики угроз.
