...
1. Замените функцию **getProfilePhoto** приведенным ниже кодом. Этот код настраивает и отправляет запрос GET конечной точке */me/photo/$value*, а затем обрабатывает отклик. Обратите внимание, что фотографии профиля в настоящее время недоступны для учетных записей MSA.
...
1. Замените функцию **uploadFile** приведенным ниже кодом. Этот код настраивает и отправляет запрос PUT конечной точке */me/drive/root/children/mypic.jpg/content*. Если файл существует, этот запрос обновляет содержимое. В противном случае он создает файл и добавляет фотографию профиля. 
...
1. Замените функцию **getSharingLink** приведенным ниже кодом. Этот код настраивает и отправляет запрос GET конечной точке */me/drive/items/{идентификатор файла}/createLink* и обрабатывает результат (ссылку для общего доступа к файлу, которая будет включена в сообщение).
