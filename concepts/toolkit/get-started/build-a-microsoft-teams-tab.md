...
Самый простой способ создать приложение Teams — использовать [расширение набора средств Microsoft Teams](https://marketplace.visualstudio.com/items?itemName=TeamsDevApp.ms-teams-vscode-extension) для Visual Studio Code. Следуйте инструкциям по [настройке нового проекта Teams](https://docs.microsoft.com/microsoftteams/platform/toolkit/visual-studio-code-overview#set-up-a-new-teams-project). При **переходе**на экран **добавить возможности** выберите **вкладку**, а затем щелкните Личные.
...
Чтобы получить идентификатор клиента, необходимо [зарегистрировать приложение](https://docs.microsoft.com/graph/auth-register-app-v2) в Azure AD. Обязательно добавьте URL-адрес ngrok с полным путем к всплывающей странице проверки подлинности в URI перенаправления (например, `https://<YOUR_NGROK_URL>/auth.html` ).
