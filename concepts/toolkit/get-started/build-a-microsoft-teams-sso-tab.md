...
description: Начало создания вкладки Microsoft Teams SSO с помощью microsoft Graph набор средств.
...
6. Создание backend
7. Инициализация поставщика Teams MSAL2.
...
1. На левой области выберите **Azure Active Directory,** а затем выберите **регистрации приложений** в **статье Управление**.
...
1. Выберите **Новая регистрация**. На странице **Регистрация приложения** задайте необходимые значения следующим образом:
    ...
    - В статье Перенаправление **URI** установите первое падение и установите значение URL-адреса страницы auth, созданного на предыдущем шаге; например `Single Page Application` `https://myapp.ngrok.io/tabauth` . 
...
1. В **статье Управление** перейдите к **разрешениям API.** Выберите **Добавить разрешение** Microsoft Graph делегирования  >    >  разрешений, а затем добавьте следующие разрешения: `email` , , , , `offline_access` `openid` `profile` `User.Read` . Выберите **Добавить разрешения**.
...
1.  В **статье Управление**, перейдите к expose **aPI**. На верхней части страницы, рядом с `Application ID URI` , выберите **Набор**. Это создает API в виде: `api://{AppID}` . Обновите его, чтобы добавить поддомен; например, `api://myapp.ngrok.io/{appID}` .
...
1. Далее добавьте два клиентских приложения. Это для Teams и мобильных клиентов и веб-клиента. В разделе **Авторизованные клиентские приложения** выберите **Добавление клиентского приложения.** Заполните ИД клиента и выберите область, созданную вами. Затем выберите **приложение Добавить**. Сделайте это для следующих IDs:
...
В SSO-режиме убедитесь, что он должен быть указать на `sso-url`  /  `ssoUrl` API-поддержку.

# <a name="html"></a>[HTML](#tab/HTML)

Добавьте `mgt-teams-msal2-provider` htmL-код.
...
Замените с помощью клиентского ИД для приложения, замените полный или относительный путь на страницу auth и замените полный или относительный путь к службе `<YOUR_CLIENT_ID>` `auth-popup-url` `sso-url` backend.
...
```
Замените с помощью клиентского ИД для приложения, замените полный или относительный путь на страницу auth и замените полный или относительный путь к службе `<YOUR_CLIENT_ID>` `authPopupUrl` `ssoUrl` backend.
...
Вы можете добавлять компоненты в HTML, как обычно. Например, чтобы добавить `Person` компонент, добавьте следующий код в HTML.
...
Если все настроено правильно, будет отрисовка компонента без необходимости `Person` входа.
>[!IMPORTANT]
