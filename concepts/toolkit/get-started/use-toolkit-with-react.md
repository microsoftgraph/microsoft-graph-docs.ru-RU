...
> [!TIP]
> Вы также можете следовать этому учебнику в качестве интерактивного тура кода. Подробные сведения см. в [GitHub репо с проектом-началом](https://github.com/microsoftgraph/mgt-react-codetour).
...
Установите и `mgt-msal2-provider` пакет `mgt-element` npm, который содержит поставщика auth MSAL2.
...
1. В редакторе кода откройте **файл src/App.tsx** и `import` раздвяйте заявление компонента с **компонентом Agenda** .
