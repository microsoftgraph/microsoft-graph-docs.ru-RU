# <a name="integrate-microsoft-graph-security-api-alerts-with-ibm-qradar-siem-using-azure-monitor"></a>Интеграция оповещений API безопасности Microsoft Graph с SIEM при помощи Azure Monitor

Поставщиками безопасности Microsoft Graph можно управлять через одну конечную точку REST. Эту конечную точку можно настроить для [Azure Monitor](https://docs.microsoft.com/en-us/azure/monitoring-and-diagnostics/), который поддерживает соединители для нескольких продуктов SIEM. Шаги 1 и 2 инструкции, приведенной в этой статье, относятся ко всем соединителям Azure Monitor, поддерживающим получение информации через концентраторы событий. В этой статье описывается сквозная интеграция соединителя [QRadar](https://www.ibm.com/us-en/marketplace/ibm-qradar-siem)  SIEM.
...
3. [Загрузите и установите QRadar для использования оповещений системы безопасности](#step-3-download-and-install-the-qradar-to-consume-security-alerts).
...
1. В вашей подписке Azure (можно найти в разделе "Все службы") зарегистрируйте «microsoft.insights» (Azure Monitor) в качестве поставщика ресурсов.  
...
## <a name="step-3-download-and-install-the-qradar-to-consume-security-alerts"></a>Шаг 3: Загрузка и установка QRadar для использования оповещений безопасности

1. Загрузите и установите [IBM QRadar](https://www.ibm.com/us-en/marketplace/ibm-qradar-siem). **Версия 7.2.8 с исправлениями 7 или более поздней версии** для чтения событий из сервера-концентратора событий Microsoft Azure.
2. Выполните действия, описанные в статье [Настройка концентраторов событий Microsoft Azure для взаимодействия с IBM QRadar](https://www.ibm.com/support/knowledgecenter/SS42VS_DSM/t_dsm_guide_microsoft_azure_enable_event_hubs.html) для настройки вашего концентратора событий.
3. Наконец, выполните действия, описанные в статье [Настройка QRadar для сбора событий из концентраторов событий Microsoft Azure с использованием протокола концентраторов событий Microsoft Azure](https://www.ibm.com/support/knowledgecenter/SS42VS_DSM/t_logsource_microsoft_azure_event_hubs.html) для начала формирования оповещений безопасности.
  
 > **Примечание:** Интеграция Microsoft Azure с IBM QRadar поддерживает события, указанные в [спецификации Microsoft Azure DSM](https://www.ibm.com/support/knowledgecenter/SS42VS_DSM/c_dsm_guide_microsoft_azure_DSM_specs.html). В настоящий момент мы работаем над добавлением полной поддержки для оповещений API безопасности Microsoft Graph в IBM QRadar. В настоящее время вы сможете получать оповещения API безопасности Microsoft Security Graph и просматривать их в консоли IBM QRadar. Можно использовать [Редактор DSM](https://www.ibm.com/support/knowledgecenter/SS42VS_7.2.8/com.ibm.qradar.doc/c_qradar_adm_dsm_ed_overview.html) для включения анализа оповещений API безопасности Microsoft Graph.  
