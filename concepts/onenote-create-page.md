---
title: Создание страниц OneNote с помощью API OneNote
description: Создайте страницу OneNote, отправив запрос POST в конечную точку pages. Затем отправьте в тексте сообщения HTML-код, определяющий страницу.
...
> [!NOTE]
> Сведения о запросах POST, которые можно отправлять для создания разделов, групп разделов и записных книжек, см. в [интерактивном справочнике по REST](https://dev.onenote.com/docs).
...
При создании страниц в личной записной книжке пользователя Microsoft Graph также предоставляет конечные точки, которые можно использовать для создания страниц в стандартной записной книжке:
...
### <a name="use-the-sectionname-url-parameter"></a>Использование параметра *sectionName* URL-адреса
...
> [!NOTE]
> Ссылки, возвращаемые интерфейсом API на страницы в переименованном разделе, по-прежнему будут указывать на старые страницы.
...
- `<head>` и `<body>`
- `<title>` и `<meta>`, которые задают заголовок и дату создания страницы
- `<h1>` — `<h6>` для заголовков разделов
- `<p>` для абзацев
- `<ul>`, `<ol>` и `<li>` для списков и их элементов
- `<table>`, `<tr>` и `<td>`, включая вложенные таблицы
- `<pre>` для предварительно отформатированного текста (с сохранением пробелов и разрывов строк)
- `<b>` и `<i>` для полужирного текста и курсива
