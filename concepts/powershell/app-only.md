...
- Сертификат, который можно использовать в качестве учетных данных для приложения. Это может быть самозаверяемый сертификат или сертификат от органа.
- Необходимо [зарегистрировать приложение](/azure/active-directory/develop/app-objects-and-service-principals) в Azure AD, настроить его с помощью необходимых для сценария областей разрешений и общего ключа для сертификата.

### <a name="certificate"></a>Сертификат

Вам потребуется сертификат X.509, установленный в надежном хранилище пользователя на компьютере, где будет запускаться сценарий. Вам также потребуется общедоступный ключ сертификата, экспортируемый в формате .cer, .pem или .crt. Вам потребуется значение субъекта сертификата.
...
1. Откройте браузер и перейдите в центр [администрирования Azure Active Directory](https://aad.portal.azure.com) и войдите в систему с помощью администратора Microsoft 365 организации клиента.
...
- Субъект сертификата, загруженного на регистрацию приложения Azure AD.
