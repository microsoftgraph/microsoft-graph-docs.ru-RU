...
### <a name="files"></a>Файлы
Получение состояния диска на определенное время путем указания соответствующей метки времени, зашифрованной в формате URL-адреса. См. [пример](/graph/api/driveitem-delta#example-4-retrieving-delta-results-using-a-timestamp).
...
- Связывание одной или нескольких [внешних групп](/graph/api/resources/externalconnectors-externalgroup?view=graph-rest-beta&preserve-view=true) с [внешним подключением](/graph/api/resources/externalconnectors-externalconnection?view=graph-rest-beta&preserve-view=true). Например, внешняя группа, такая как бизнес-подразделение или рабочая группа, может определить разрешения на контент в источнике данных, представленном внешним подключением.
...
Ежемесячные обновления Intune для версии v1.0. В разделе [changelog](https://developer.microsoft.com/graph/changelog) установите фильтр **Дата** на сентябрь 2021 г. и выполните поиск раздела с таким заголовком.
...
### <a name="devices-and-apps--corporate-management"></a>Устройства и приложения | Корпоративное управление
Ежемесячные обновления Intune для бета-версии. В разделе [changelog](https://developer.microsoft.com/graph/changelog) установите фильтр **Дата** на сентябрь 2021 г. и выполните поиск раздела с таким заголовком.
...
- Общая доступность разрешений с согласием для конкретных ресурсов (RSC). Разрешения RSC позволяют владельцам групп предоставлять детально определенные согласия рабочим приложениям на доступ к конкретным данным команды и/или их изменение, например на чтение параметров команды или изменение названий, описаний и других параметров каналов.
