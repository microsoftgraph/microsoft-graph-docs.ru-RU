...
Поставщиками безопасности Microsoft Graph можно управлять через одну конечную точку REST. Эту конечную точку можно настроить для [Azure Monitor](https://docs.microsoft.com/en-us/azure/monitoring-and-diagnostics/), который поддерживает соединители для нескольких продуктов SIEM. Шаги 1 и 2 инструкции, приведенной в этой статье, относятся ко всем соединителям Azure Monitor, поддерживающим получение информации через концентраторы событий. В этой статье описывается сквозная интеграция [соединителя](https://splunkbase.splunk.com/)  Splunk SIEM.
...
Для начала необходимо создать пространство имен [концентраторов событий Microsoft Azure](https://docs.microsoft.com/en-us/azure/event-hubs/) и новый концентратор событий. Это пространство имен и концентратор событий будут пунктом назначения для всех оповещений системы безопасности в организации. Пространство имен концентраторов событий — это логическая группа концентраторов событий с одной и той же политикой доступа. Обратите внимание на несколько особенностей этого пространства имен и создаваемых вами концентраторов событий.
...
> **Примечание.** В настоящее время колонка параметров Azure Monitor Diagnostic не поддерживает конфигурацию ресурсов на уровне клиента. Оповещения API безопасности Microsoft Graph представляют собой ресурс клиентского уровня, требующий использования API диспетчера ресурсов Azure для настройки Azure Monitor на поддержку приема оповещений, которые поступают от системы безопасности вашей организации.

1. В подписке Azure зарегистрируйте "microsoft.insights" (Azure Monitor) в качестве поставщика ресурсов.  
> **Примечание.** Не регистрируйте «Microsoft.SecurityGraph» (API безопасности Microsoft Graph) в качестве поставщика ресурсов в подписке Azure, так как «Microsoft.SecurityGraph» представляет собой ресурс клиентского уровня, что описывается выше. Настройка на уровне клиента рассматривается в п. 6 ниже.
...
3. Подготовьте примерно такой файл JSON с запросом параметров диагностики:
    ...
    **"days":** число дней, на протяжении которых сообщения должны храниться в вашем концентраторе событий.
...
1. Эта интеграция поддерживает лишь развертывания [Splunk Enterprise](https://splunkbase.splunk.com/).
