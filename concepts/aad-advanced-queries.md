...
В следующей таблице обобщается поддержка операторов `$filter` свойствами всех объектов каталога.
...
- ![Требуются расширенные параметры запроса.](/graph/images/advanced-query-parameters/advanced.png) Для конкретного оператора `$filter` требуются *расширенные параметры запроса*, то есть:
