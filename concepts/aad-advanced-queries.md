...
> + Чтобы использовать расширенные возможности запросов в [пакетных запросах](json-batching.md), укажите заголовок **ConsistencyLevel** в свойстве **заголовков** запроса JSON.
...
+ `endsWith` Оператор поддерживается только расширенными запросами по **почте**, **userPrincipalName** и свойствами **proxyAddresses**.
