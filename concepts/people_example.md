...
```
### <a name="select-the-fields-to-return-in-a-filtered-response"></a>Выбор полей, которые необходимо возвратить в отфильтрованном отклике 
...
Указанный ниже поисковый запрос возвращает людей, релевантных для пользователя `/me`, у которых поле **displayName** содержит слово, начинающееся с буквы j.
...
```
### <a name="perform-a-fuzzy-search"></a>Выполнение нечеткого поиска 
Указанный ниже запрос выполняет поиск человека с именем Irene McGowen. Так как человек с именем Irene McGowan является релевантным для пользователя, выполнившего вход в систему, то будет возвращена информация об этом человеке.
