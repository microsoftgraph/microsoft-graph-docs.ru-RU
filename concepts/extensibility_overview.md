...
- Если у вас уже есть запоминающийся домен `.com`,`.net`, `.gov`, `.edu` или `.org`, который вы проверили с помощью своего клиента, вы можете использовать это доменное имя вместе с именем схемы, чтобы задать уникальное имя в следующем формате: \{_&#65279;доменноеИмя_\}\_\{_&#65279;имяСхемы_\}. Например, если у вас есть запоминающийся домен contoso.com, то вы можете задать его **идентификатор** `contoso_mySchema`.  Это предпочтительный способ.
