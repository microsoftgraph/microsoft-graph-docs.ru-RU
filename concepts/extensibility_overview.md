...
В приведенной ниже таблице показаны ресурсы, для которых поддерживаются открытые расширения и расширения схемы, а также версия таких расширений — общедоступная (в конечных точках /v1.0 и /beta) или только предварительная (в конечной точке /beta). 
