...
description: Подпишитесь на изменения ресурсов Outlook (создание, обновление и удаление) и измененные данные ресурсов в API Microsoft Graph и получайте уведомления через веб-перехватчики.
author: abheek-das
...
API Microsoft Graph позволяет подписаться на изменения в ресурсе, &mdash;включая создание, обновление или удаление ресурса,&mdash; и получать уведомления через веб-перехватчики. [Подписка](/graph/api/resources/webhooks) указывает желаемые типы изменений для отслеживания для определенного ресурса и включает URL-адрес конечной точки для получения уведомлений об этих изменениях.

Настройка подписки снижает накладные расходы, связанные с запросом и сравнением ресурсов для получения каких-либо изменений. При желании можно указать в запросе на подписку шифрование и включить в уведомление измененные данные ресурса, сохранив отдельный последующий вызов API для получения полезных данных ресурса.
...
- **resource**. Это свойство указывает URL-адрес ресурса. Обязательно используйте параметр запроса `$select`, чтобы явно указать свойства ресурса Outlook, которые необходимо включить в полезные данные уведомления.
  ...
  > [!NOTE]
  > Не включайте в URL-адрес `$top`, `$skip`, `$orderby`, `$select=Body,UniqueBody` и `$expand`, кроме **singleValueExtendedProperties** или **multiValueExtendedProperties**.
...
Ниже приводится пример расшифрованных полезных данных уведомления. Расшифрованные полезные данные соответствуют схеме [message](/graph/api/resources/message) Outlook. Полезные данные похожи на результаты, возвращаемые операцией [GET message](/graph/api/message-get). Однако полезные данные уведомления содержат только те свойства, которые указаны с помощью параметра `$select` в свойстве **resource** подписки. Полезные данные уведомлений для других ресурсов Outlook, таких как [contact](/graph/api/resources/contact) и [event](/graph/api/resources/event), следуют соответствующим схемам. 
...
> [!NOTE]
> Вызовы GET всегда возвращают текущее состояние ресурса. Если ресурс изменяется между моментом отправки уведомления и моментом извлечения ресурса, операция возвращает состояние ресурса при извлечении.
