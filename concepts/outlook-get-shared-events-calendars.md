...
description: Чтение событий в общем или делегированном календаре. Общий календарь или события можно получить непосредственно из почтового ящика владельца календаря или из почтового ящика получателя общего доступа.
author: juforan
...
> [!NOTE]
> Разрешения общего доступа (Calendars.Read.Shared или Calendars.ReadWrite.Shared) позволяют читать или записывать события в общем или делегированном календаре. Они не поддерживают [подписку на уведомления об изменениях](webhooks.md) элементов в таких папках. Чтобы настроить подписки на уведомления об изменениях событий в общем, делегированном или любом другом календаре пользователя или ресурса в клиенте, используйте разрешение приложения Calendars.Read.
...
В случае успешного выполнения вы получите отклик HTTP 200 OK и экземпляр объекта [calendar](/graph/api/resources/calendar), представляющий общий основной календарь Артема, в его почтовом ящике.
...
В случае успешного выполнения вы получите отклик HTTP 200 OK и экземпляр объекта [event](/graph/api/resources/event), определенный параметром `{id}` в основном календаре Артема, непосредственно из его почтового ящика.
...
В случае успешного выполнения вы получите отклик HTTP 200 OK и коллекцию экземпляров объекта [event](/graph/api/resources/event) в основном календаре Артема непосредственно из его почтового ящика.
...
- [API календаря](/graph/api/resources/calendar?view=graph-rest-1.0&preserve-view=true) в Microsoft Graph 1.0
