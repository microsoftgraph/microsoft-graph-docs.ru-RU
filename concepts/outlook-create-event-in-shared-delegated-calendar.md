...
description: Создание собраний в общем или делегированном календаре Outlook. Вы можете поделиться календарем с другими пользователями или предоставить делегату право действовать от вашего имени.
author: juforan
...
> [!NOTE]
> Войдя в систему как Анна, вы также можете получить делегированный календарь непосредственно из почтового ящика Артема, указав имя Артема и ярлык `calendar`, как в `GET https://graph.microsoft.com/v1.0/users/AlexW@contoso.OnMicrosoft.com/calendar`. Возвращенный код календаря соответствует почтовому ящику Артема. 
